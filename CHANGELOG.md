...
콘솔 세션 수명주기를 **수동 열기/닫기**로 전환. "내가 모르는 동안 터미널이 종료되어 작업이 끊기는"
문제를 근본 해결한다 — 사용자가 직접 "세션 종료"를 누르기 전까지는 유휴 타임아웃·호스트 메모리
압박 어느 경우에도 콘솔 세션/터미널을 자동으로 닫지 않는다.

### Added

- **콘솔 세션 수동 열기/종료 버튼** — 콘솔 헤더 우측에 `세션 열기`(제일 좌측) · `새 세션` ·
  `세션 종료`(새 세션 우측)를 배치. 상태칩(세션없음/대기중/응답중)을 함께 표시해 작업 중이 아닐
  때 "대기중"으로 보인다.
  - `세션 열기`: 세션을 시작한다 — 기존 세션이 있으면 이어서(resume), 없으면 새로 연다.
  - `세션 종료`: 현재 provider 의 세션을 두 시스템(stream-json + PTY)에서 종료하되 **session-id
    는 보존**(다음 "세션 열기"에서 resume). `새 세션`과 달리 대화를 버리지 않는다. 종료 후 같은
    프로젝트에 다른 provider 세션이 살아있으면 그 콘솔로, 없으면 프로젝트 목록(`/projects`)으로
    이동. (라우트 `POST /projects/{id}/console/close` — SSR 전용, 브라우저.)
- **설정 → 일반 "세션 자동 관리" 토글** (`security.autoManageSessions`, 기본 **꺼짐=수동**). 켜면
  v1.174.0 이하의 자동 회수 동작(유휴/메모리 압박 시 세션 자동 종료, session-id 보존 warm resume)
  으로 되돌린다. `ConfigHolder.current` 라이브 참조라 저장 즉시 반영(재시작 불필요).

### Changed

- **수동 모드(기본)에서 모든 자동 세션 종료 경로를 차단** — `ClaudeSessionManager` /
  `SubAgentSessionManager` 의 idle reaper(유휴 SIGTERM), `ConsoleTuiSessionManager` 의 idle
  reaper + **메모리 압박 회수**가 전부 no-op 이 된다. stall watchdog(turn 만 정리, 프로세스 유지)은
  회수가 아니므로 유지.
- **진입 시 자동 세션 실행 금지** — 프로젝트 콘솔 진입만으로 세션을 spawn 하지 않는다. 이미 살아있는
  세션이 있으면 자동 attach, 없으면 "세션 열기" 버튼(또는 프롬프트 전송)으로만 시작.
- **provider 전환 / 모델 변경 시 다른 provider 세션 보존** — 수동 모드에선 provider 전환이 이전
  provider PTY 를 죽이지 않아 "세션 종료 후 다른 provider 로 이동"이 성립한다. 모델 변경은 현재
  provider PTY 만 재시작(세션-id 보존).

### Wire

- **Wire/DB 무변경(브라우저 SSR 전용)** — `POST /projects/{id}/console/close` 는 admin SSR 라우트
  (`/console/new`·`/console/model` 과 동일 계열, `ApiPath` 미등록·`/api` 아님)로 브라우저에서만
  호출한다. `shared/` DTO·`ApiPath`·`WsFrame` 변경 없음. 신규 config 필드 `autoManageSessions` 는
  서버 내부(비-wire). **안드로이드 무영향** — 구버전 클라이언트 그대로 동작. (안드가 동일한 수동
  열기/종료 UX 를 원하면 별도 JSON API 를 추후 추가.)

### ⚠️ 주의

- 수동 모드의 "메모리 압박 완전 차단"은 대량 자동화 시 호스트 OOM 위험을 감수하는 선택이다(운영자
  결정). 위험하면 설정에서 "세션 자동 관리"를 켜면 이전 회수 동작으로 복귀한다.

## [1.174.0] - 2026-07-23

### Fixed

- **SwiftLint/SwiftFormat 설치 후에도 빌드환경에 "미설치"로 남던 문제 회수(잔여분)** — 근본 원인은
  v1.171.0 에서 이미 고친 **non-login SSH 셸의 Homebrew PATH 부재**였다(`ssh host 'swiftlint version'`
  이 `/opt/homebrew/bin` 을 못 봐 exit 127 → 미설치 오판; Xcode/simctl 은 `/usr/bin` 절대경로라 정상
  감지되어 "Xcode ✓ · SwiftLint ✗" 증상과 일치). 해당 수정은 v1.173.0 배포로 라이브 반영됨(운영 Mac
  에서 컨테이너→맥 프로브 재현 시 swiftlint 0.65.0 / swiftformat 0.62.1 / pod 1.17.0 정상 감지 확인).
  남은 UX 결함으로, 빌드환경 "Swift 도구 설치" 버튼으로 설치한 **직후**에도 `IosEnvComponentService`
  의 30초 TTL 스냅샷 캐시가 아직 유효해 카드가 잠시 "미설치"로 남았다. 설치 태스크 성공 시
  `IosEnvComponentService.refreshNow()` 로 캐시를 즉시 강제 갱신하도록 보강 — 설치 완료 후 페이지가
  카드를 다시 그릴 때 30초 지연 없이 바로 INSTALLED 를 반영한다.

### Wire

- **Wire/DB 무변경** — 서버 내부 캐시 갱신만. 안드로이드 무영향.

## [1.173.0] - 2026-07-23

진행중이던 운영 개선 작업(agent CLI 부재 가드 · Kotlin LSP 자원 관리 · flash i18n · 운영 문서)을
일괄 반영. 특히 맥(Apple Silicon) Docker 처럼 codex/opencode CLI 가 없거나 무거운 환경에서의 체감
지연·오작동을 줄인다.

### Changed

- **agent 상태 캡처를 CLI 부재 시 안전하게 스킵** — `codex`/`opencode` 바이너리가 PATH 에 없으면
  `CodexStatusService`/`OpenCodeAuthService`/`OpenCodeStatusService` 가 프로세스를 띄우지 않고 빈 결과를
  반환한다(`isCommandAvailable`/`isOpenCodeCommandAvailable`, PATH+PATHEXT 탐색). Codex usage 캡처는
  `cfg.enabled` 일 때만 수행하도록 순서를 조정 — Apple Silicon Docker Desktop 등에서 PTY+expect 기반
  캡처가 만들던 체감 지연 회피. `CODEX_CMD` env 로 codex 실행 파일 경로 override 가능(캡처 스크립트 포함).
- **Kotlin LSP 자원 관리** — `KotlinLspService` 에 idle reaper(기본 30분, `KOTLIN_LSP_IDLE_MINUTES`) +
  인스턴스 상한(기본 4, `KOTLIN_LSP_MAX_INSTANCES`, LRU eviction) 추가. 오래 유휴이거나 죽은 per-project
  language server 프로세스를 회수해 장기 구동 시 프로세스 누적을 방지.
- **flash 메시지 i18n 정리** — 프로젝트 목록/아카이브 flash 를 하드코딩 한국어 대신 메시지 키
  (`flash.project.created`/`deleted`/`archived`)로 통일(EN/KO).

### Docs

- `AGENTS.md` — 현재 등록된 운영 서버 3개(`codr-kr`, `aigoo.kr:32323`, `vibe.wody.work`) 기준으로 갱신.

### Wire

- **Wire/DB 무변경** — 서버 내부 동작/문서만. 새 엔드포인트·DTO·스키마 없음. 안드로이드 무영향.

## [1.172.0] - 2026-07-23

### Fixed

- **기존 iPhone 프로젝트에 iOS 빌드 지침이 없어 진행 불가하던 문제 회수** — iOS 빌드 파이프라인
  지침(CLAUDE.md 섹션 + `vibe-ios-build-flow` 스킬)은 **신규 프로젝트 생성 시점에만** 주입돼서,
  v1.169.0 이전에 만든 iPhone 프로젝트에는 스킬도 CLAUDE.md 섹션도 없었다. 이 때문에 콘솔의 AI 가
  "이 프로젝트를 어떻게 빌드/시뮬레이터 실행하는지" 모른 채 막혔다. 이제 iPhone 프로젝트 오버뷰(콘솔
  탭)를 열 때 `ProjectService.ensureIphoneProjectGuidance` 가 **누락된 기본 스킬/에이전트를 보강**하고,
  **CLAUDE.md 에 파이프라인 섹션 마커가 없으면 문서 끝에 비파괴 append** 한다(프로세스당 프로젝트 1회,
  덧쓰기 없음, iPhone 프로젝트에만 적용). 기존 프로젝트를 한 번 열기만 하면 다음 턴부터 AI 가 빌드
  절차를 인지한다.

### Changed

- **`ClaudeMdTemplate` iOS 파이프라인 섹션을 재사용 상수로 추출** — `IPHONE_BUILD_PIPELINE_SECTION`
  (+ 판정용 `IPHONE_PIPELINE_MARKER`). 신규 생성용 `CONTENT_IPHONE` 은 이 상수를 인라인 인터폴레이션해
  SSOT 를 유지하고, 기존 프로젝트 append 도 같은 본문을 쓴다.
- **Wire/DB 영향 없음** — SSR/파일시스템 보강만. 새 엔드포인트·DTO·스키마 없음. 안드로이드 클라이언트
  영향 없음(iOS 전용 서버 UX, 기존 wire 불변).

## [1.171.0] - 2026-07-23

### Fixed

- **`homebrew_missing` (및 swiftlint/swiftformat/pod 미발견) 회수** — `ssh host 'cmd'` 는 원격을
  **non-login 셸**로 돌려 `.zprofile`/`.bash_profile` 의 Homebrew PATH 를 로드하지 않아 `brew` 등을
  못 찾았다. `IosAgentCommandRunner.buildSshCommand` 가 모든 원격 명령 앞에 Homebrew 경로
  (`/opt/homebrew/bin`·`/usr/local/bin` + 표준)를 명시 prepend 하도록 수정. SwiftTools 설치·CocoaPods
  빌드·Homebrew 도구 프로브가 정상 동작. (운영 배포 시 관측된 실제 오류.)

### Added

- **Homebrew 원탭 설치 (맥 SSH 에이전트)** — 빌드환경 iPhone 연결 폼에서 preflight 가 Homebrew 미설치
  (`homebrewAvailable=false`)를 보고하면 **"Homebrew 원탭 설치"** 버튼 노출 → `POST /api/ios/homebrew/install`
  (`IosHomebrewInstallService`)이 공식 비대화식 설치 스크립트(`NONINTERACTIVE=1 … install.sh`)를 SSH
  agent 로 실행. Command Line Tools 없으면 `clt_required` 로 조기 안내. `IosPreflightDto.homebrewAvailable`
  프로브 추가(대시보드/연결 결과에 반영). (최초 설치는 passwordless sudo 전제 — 안 되면 `brew_install_failed`.)

### Changed

- **`ios.agent.xcodePath` → `DEVELOPER_DIR` 반영** — auto 가 아니면 원격 명령에 `env DEVELOPER_DIR=<path>`
  prepend 해 다중 Xcode 중 특정 버전 강제(그동안 저장만 되고 미사용이던 값 활성화).
- **archive/export 빌드 시 서명 프로파일 스테이징** — 서버별 `~/.vibe-coder-ios/<serverId>/signing/` 의
  `.mobileprovision` 을 빌드 준비 단계에서 맥의 `~/Library/MobileDevice/Provisioning Profiles/` 로 복사
  (있을 때만, best-effort) → 수동 서명 빌드가 프로파일을 찾게.

## [1.170.0] - 2026-07-23

### Added

- **대시보드 macOS 빌드 에이전트 연결상태 모니터 카드** — `ios.agent` SSH 가 구성된 경우에만
  대시보드 그리드에 표시(`AdminTemplates.renderMacAgentCard`). 대상(`user@host`) + 연결 상태 +
  Xcode + Simulator 를 색 뱃지로. 대시보드 SSR 을 막지 않도록 `GET /api/ios/preflight` 를 카드 JS 로
  라이브 조회(연결 시 즉시, 미연결 시 non-blocking) + 60초 폴링 + 수동 새로고침. `agent_unreachable`/
  `linux`(미연결) 구분 표시, "설정" 링크로 `/env-setup#iphone` 이동.

## [1.169.0] - 2026-07-23

### Added

- **iPhone 프로젝트에 iOS 빌드 파이프라인 학습 + 통합 워크플로 스킬 주입** — 신규 iPhone 프로젝트의
  로컬 `CLAUDE.md`(`ClaudeMdTemplate.CONTENT_IPHONE`)에 "vibe-coder iOS build pipeline — structure &
  procedure" 섹션 추가(SSH 연결·rsync 소스동기·빌드 트리거·시뮬레이터·xcresult/로그/스크린샷 디버깅
  루프). 더불어 `.claude/skills/vibe-ios-build-flow/SKILL.md` 를 주입(`ProjectToolingSeeder` +
  `IPhonePlatformEngine.defaultSkills`)해 **빌드→실행→검사→수정을 한 몸의 루프로** 다루도록 가이드.
  기존 프로젝트엔 영향 없음(생성 시 `notExists` 가드, 파일 미덮어씀).
- **연결 폼 Docker Desktop 호스트 자동 감지** — 컨테이너에서 `host.docker.internal` 이 해석되면
  "로컬 맥 감지" 안내 + host 프리필 유지, 미해석이면 "원격 맥 IP 입력" 안내(`renderIphoneConnectForm`).

### Changed

- **맥 작업공간 폴더명 `~/.vibe-coder-mac` → `~/.vibe-coder-ios`** (v1.167.0 에서 도입, 실사용 전 변경).
  서버별 격리는 그대로: `~/.vibe-coder-ios/<serverId>/<projectId>/`.

## [1.168.0] - 2026-07-23

### Added

- **iOS 빌드 트리거 UI (Build / Test / Archive / Export IPA)** — iPhone 프로젝트 rail 의 빌드 설정
  카드에 실행 버튼 추가(`ProjectTabsTemplate` + `project-tabs.js` initIosBuildRunCard). 기존 JSON API
  `POST /api/projects/{id}/ios/build/{debug|test|archive|export-ipa}` 를 호출하고 큐 등록 후 빌드
  상세 페이지로 링크. 원격 맥에서 xcodebuild 실행 → 산출물(.app/.ipa/xcresult)은 rsync 로 컨테이너에
  자동 회수(기존 백엔드). 이전엔 JSON API 만 있고 누를 버튼이 없었다.
- **시뮬레이터 원클릭 "Build & Run" + 현재화면 재캡처** — 시뮬레이터 rail 카드에 버튼 추가. Build &
  Run 은 iOS debug 빌드 → 빌드 상태 폴링(`GET /api/projects/{id}/builds/{buildId}`, TaskStatus)으로
  완료 대기 → (미부팅이면) boot → run(install+launch+screenshot)까지 한 번에. 재캡처는
  `POST /api/projects/{id}/ios/simulators/{udid}/screenshot`(v1.167.0) 로 현재 화면만 다시 떠 표시.

## [1.167.0] - 2026-07-23

### Added

- **iPhone 빌드 대행 맥 SSH 연결 — 비밀번호 원클릭 부트스트랩 (설정/빌드환경)** — Docker(리눅스
  컨테이너)로 설치한 어느 환경에서든, 빌드를 대신할 맥에 SSH 로 붙어 iPhone 빌드/시뮬레이터/서명/
  로그를 원격 실행할 수 있게 하는 첫 단계. 빌드환경(/env-setup) iPhone 섹션에 **host/port/user/
  비밀번호 입력 + "연결하기"** 폼 추가(`EnvSetupTemplates.renderIphoneConnectForm`). "연결하기"는
  신규 `POST /api/ios/agent-connect`(`IosAgentBootstrapService`)를 호출해:
  ① `sshpass -e ssh`(비번은 env 로만, argv/ps 미노출)로 맥 접속 →
  ② 컨테이너 공개키(`~/.ssh/id_ed25519.pub`)를 맥 `~/.ssh/authorized_keys` 에 설치 →
  ③ 키 인증(BatchMode) 검증 →
  ④ 맥에 `~/.vibe-coder-mac/<serverId>/` 작업공간 준비 후 `ios.agent` config 저장 →
  ⑤ preflight(Xcode/simctl/서명) 실행해 결과 표시. **비밀번호는 1회 부트스트랩에만 쓰이고
  저장/로그되지 않음**(이후 키 기반 SSH 로 동작). 로컬 맥(`host.docker.internal`)이든 원격 맥이든
  같은 폼으로 동일 동작.
- **여러 서버가 한 맥을 공유해도 충돌 없도록 서버별 작업공간 분리** — 컨테이너 SSH 공개키에서
  파생한 안정적·유일 `serverId`(`srv-<12hex>`)를 workspaceRoot(`~/.vibe-coder-mac/<serverId>`)에
  포함 → 기존 rsync sync/빌드/시뮬레이터 경로(`workspaceRoot/<projectId>/`)가 자동으로 서버 단위 격리.
- **시뮬레이터 현재 화면 단독 재캡처** — `POST /api/projects/{id}/ios/simulators/{udid}/screenshot`
  (`IosSimulatorRunService.capture`) — install/launch 재실행 없이 `simctl io booted screenshot` 만
  다시 떠 rsync 로 회수.

### Changed

- **Docker 이미지에 `sshpass` + `rsync` 설치** (`docker/Dockerfile`) — sshpass 는 비밀번호 부트스트랩,
  rsync 는 iOS 워크스페이스 동기·산출물/스크린샷 회수에 필수(이전엔 rsync 미설치라 iOS SSH 동기가
  실패할 수 있었음). §8.C 운영정책 변경 — 이미지 재빌드 필요.

### Wire change

- **Yes (additive)** — 신규 REST `POST /api/ios/agent-connect`, `POST /api/projects/{id}/ios/
  simulators/{udid}/screenshot` (`ApiPath` 상수 선등록). iOS 전용 서버 UX 엔드포인트라 구버전
  안드로이드 클라이언트 동작에 영향 없음. `vibe-coder-android` shared/ 동기는 안드가 iOS 기능을
  노출할 때만 필요(현재 불필요).

### 후속 (다음 증분)

- iOS 빌드/테스트/아카이브/Export IPA 트리거 버튼 UI(백엔드 `POST /api/projects/{id}/ios/build/*`
  는 이미 동작 — 빌드 산출물은 rsync 로 컨테이너에 자동 회수됨).
- 시뮬레이터 원클릭 파이프라인(빌드→부팅→설치→실행→스크린샷) UI.
- `xcodePath` → `DEVELOPER_DIR` 반영(다중 Xcode 선택).

## [1.166.0] - 2026-07-23

### Changed

- **Console TUI(xterm) 모바일 터치 제스처 재설계** — 통합 탭의 TUI 콘솔에서 손가락 제스처를
  정리했다. 스와이프는 native 세로 스크롤 전용(기존 매 touchmove 로깅 오버헤드 제거로
  매끄러워짐), 롱프레스(450ms)로 단어를 선택하면 앞/뒤 핸들이 뜨고 선택 상태에선 스크롤이
  잠긴다. 핸들을 드래그해 범위를 넓히고, 손가락이 화면 상/하단 16% 밴드에 오면 천천히
  auto-scroll 하며 선택을 확장한다. 손가락을 떼면 선택 위에 플로팅 '복사' 버튼이 뜬다(기존
  하단 textarea 편집 패널은 모바일에서 제거). 짧은 탭은 키보드 포커스(입력)로 유지하고,
  데스크톱 마우스 휠/드래그 선택은 xterm 기본 그대로다. 선택·단어·스크롤·하이라이트는 xterm
  공개 API(`select` / `getSelectionPosition` / `scrollLines` 등)로 구현하고, 픽셀↔cell 좌표
  변환만 `_core` 내부 API 를 얇은 헬퍼(`tuiCellFromPoint` / `tuiPointFromCell`)로 격리했다.
  `admin.css` / `sw.js` 캐시버스트 v1.166.0. Android 영향 없음(순수 SSR 프론트엔드).

## [1.165.0] - 2026-07-23

### Fixed

- **진행 중 Claude 콘솔 세션이 idle reaper 로 강제 종료되던 회귀 회수** — `reapIdleSessions`
  (30분)의 기준 `lastActivity` 가 프롬프트 전송·백그라운드 작업 이벤트에서만 갱신되고 일반
  assistant/tool 출력 프레임에서는 갱신되지 않아, 30분 넘게 걸리는 긴 turn 이 출력을 계속
  쏟아내는 중에도 `idle_terminated`(SIGTERM)로 강제 종료됐다. `handleStdoutLine` 이 매 콘솔
  출력 프레임마다 `lastActivity` 를 갱신하도록 해 "터미널 버퍼가 갱신되는 동안은 무조건
  running" 을 보장한다.

### Changed

- **콘솔 세션 turn 종료 판정에 활동 기반 3분 stall 안전장치 도입** — `reapIdleSessions` 를
  진행 중(busy)/유휴 분기로 재편했다. RESPONDING 인데 마지막 출력 후 `STALL_WATCHDOG_MS`(3분)
  동안 아무 프레임도 없으면 turn 만 정상 종료 처리한다(프로세스는 유지 — 완료 통지 누락/무출력
  stall 의 백스톱). busy + 최근 출력이면 idle reap 대상에서 제외, 유휴 세션은 기존 30분
  warm-keep 을 유지한다. 기존 백그라운드 대기(WAITING) 전용 watchdog(`finalizeStalledBgTurn` /
  `BG_SUSPEND_WATCHDOG_MS`)을 busy 전체로 일반화했다(`finalizeStalledTurn` / `STALL_WATCHDOG_MS`).
  정상 완료(Done)는 즉시 종료(프롬프트 자동화·상태칩 지연 없음)로 유지. Android 영향 없음(내부 로직).

## [1.164.0] - 2026-07-22

### Added

- **빌드환경(/env-setup)에 iPhone(macOS) 섹션 추가 (ROADMAP Phase 9)** — 맥에 설치했는데
  빌드환경 페이지에 iPhone 준비 항목이 하나도 없던 격차를 회수했다. `SetupComponent` 에
  `xcode`, `xcode-clt`, `ios-simulator-runtime`, `swift-tools`, `cocoapods` 5종을 추가하고,
  별도 "iPhone (macOS)" 섹션 + preflight 요약 배너(모드/Xcode/simctl/런타임/서명)로 렌더한다.
  Xcode/CLT/Simulator runtime 은 자동 설치 불가라 감지 + 안내 명령(App Store /
  `xcode-select --install` / `xcodebuild -downloadPlatform iOS`)만 제공하고, SwiftLint/
  SwiftFormat 만 `POST /env-setup/swift-tools/install`(Homebrew, 기존 `IosSwiftToolsInstallService`
  재사용)로 자동 설치한다. Linux Docker 단독에서는 명령을 전혀 실행하지 않고(`mac_required`)
  "Mac 필요 + 원격 Mac agent 연결" 안내만 표시한다.
- **iPhone 빌드환경 감지 캡슐화 (`IosEnvComponentService`)** — env/admin 계층이 `xcrun`/
  `xcodebuild`/`brew` 를 직접 실행하지 않고 `ios` 패키지 서비스에 위임한다(ROADMAP §18.3).
  진단 SSOT 는 `IosPreflightService` + `xcrun simctl list runtimes` / SwiftLint / SwiftFormat /
  CocoaPods version probe 이며 30초 캐시로 `detectAll` 의 5개 호출이 명령을 한 번만 spawn 한다.
- **iPhone 프로젝트 UI 진입점 완결** — 프로젝트 생성 폼의 iPhone 선택 아래에 "macOS 필요"
  안내 + 빌드환경 링크, iPhone 프로젝트 개요 rail 에 "iPhone 빌드환경" 카드
  (`PlatformUiCapabilities.showIosBuildEnvLink`), 대시보드에 Mac 설치인데 Xcode 미준비 시
  준비 힌트 배너를 추가했다. 모두 `/env-setup#iphone` 로 연결된다.

### Changed

- **기본 문서 MCP `android-docs` → `mobile-docs` 리네이밍** — OpenCode 기본 MCP 설정의
  문서 검색 MCP 를 key `android-docs`/패키지 `@siamakerlab/android-docs-mcp-server` 에서
  key `mobile-docs`/패키지 `mobile-docs-mcp` 로 정렬했다(원본 리포가 `mobile-docs-mcp` 로
  리네이밍·재게시됨). node22 npx 실행 구조·bin(`docs-mcp-server`)은 upstream 호환 유지.
  `mobile-docs-mcp@latest` 는 npm 게시본 **0.2.0** 으로 해석되어 바로 동작한다(Android
  Kotlin/Java/Flutter/Gradle 문서 today, iOS Swift/DocC/SPM 로드맵).
- **운영 요청 경로의 무거운 진단/프로세스 누적 완화** — iPhone 빌드환경 스냅샷은 캐시가
  없거나 만료되어도 HTTP 요청 스레드에서 원격 Mac probe 를 기다리지 않고 fallback/stale 값을
  반환한 뒤 백그라운드에서 갱신한다. OpenCode usage 갱신은 `providers list` 중복 실행을 제거해
  상태 갱신당 CLI spawn 1회를 줄였다. Kotlin LSP 는 프로젝트별 프로세스를 무기한 보존하지 않고
  30분 idle / 최대 4개 LRU 기준으로 회수한다(`KOTLIN_LSP_IDLE_MINUTES`,
  `KOTLIN_LSP_MAX_INSTANCES` 로 조정 가능).

### Notes

- **Wire 무변경 / Android 호환 영향 없음** — 신규 REST 는 SSR 전용
  `POST /env-setup/swift-tools/install` 하나뿐이고, iPhone 감지는 기존 `/api/ios/preflight` /
  `/api/ios/swift-tools/install` 를 재사용한다. `SetupComponent` 목록 확장은 동적 직렬화라
  구버전 클라이언트에 additive 하위호환. DTO/`ApiPath`/`WsFrame` 변경 없음. MCP 리네이밍도
  서버 내부 OpenCode 설정 변경이라 Android wire 영향 없음.

## [1.163.0] - 2026-07-22

### Added

- **아카이브/백업 시 대화 이력(conversation_turns) 포함** — 프로젝트 아카이브·백업 tar
  안에 `conversations.jsonl`(1행=1turn)로 대화 이력을 함께 담고, 복원(unarchive /
  restoreFromTar) 시 원본 `id/ts/turnIdx/provider/agent/memo/star` 까지 그대로 되살린다.
  이전에는 아카이브 시 `ProjectService.delete` cascade 로 `conversation_turns` 가 삭제되고
  복원해도 돌아오지 않았다. `ConversationTurnRepository` 에 배치 스트리밍 `exportForProject`
  와 원본 필드 보존 `restoreRows` 를 추가해 수만 건 이력도 메모리 상한을 유지하며 이동한다.
  구 아카이브(파일 없음)·대화 없던 프로젝트는 no-op 으로 하위호환.

### Changed

- **Docker slim 이미지 arm64 정식 게시 정책 전환** — `siamakerlab/vibe-coder-server:1.162.5`
  및 `:latest` 를 `linux/amd64,linux/arm64` 멀티아키 매니페스트로 게시한다. Apple
  Silicon(M1/M2/M3/M4) Mac 사용자는 같은 compose 파일로 native arm64 이미지를 받으며,
  설치/유지보수 문서의 amd64-only 기본 정책을 제거했다. Dockerfile 의 Gradle builder
  stage 는 `$BUILDPLATFORM` 으로 고정해 multi-arch 빌드 때 arm64 QEMU 컴파일을 반복하지
  않게 했다.
- **운영 대상 문서 정리** — 내부 운영 지침에 추가 운영 접속 대상
  `wody@aigoo.kr -p 32323` 을 기록하고, `vibe.wody.work` / 예전 compose 경로가 현재
  운영 기준이 아님을 명시했다. 주 운영 서버와 동일 인증 정책을 사용하되,
  비밀번호·토큰·키 값은 문서에 남기지 않는다.
- **운영 서버 3개 기준 문서화** — 현재 등록된 운영 서버를 `codr-kr`,
  `aigoo.kr:32323`, `vibe.wody.work` 총 3개로 정리했다. `vibe.wody.work` 는 현재 PC 로컬
  운영 대상으로 다시 문서화했으며, 실제 compose 경로와 프록시 포트는 배포/점검 직전 현재 PC
  상태로 확인한다.
- **Platform engine / tooling profile 기반 분리** — 프로젝트 타입을 `kotlin`, `flutter`,
  `iphone` 3종으로 확장하고, 각 타입이 별도 `ProjectPlatformEngine` 과
  `PlatformToolingProfile` 을 통해 지역 `CLAUDE.md`, 기본/조건부/opt-in MCP, Skill,
  Agent 구성을 소유하도록 정리했다. 신규 iPhone 프로젝트는 SwiftUI starter 파일과
  iPhone 전용 지역 `CLAUDE.md` 를 생성한다.
- **iPhone/macOS preflight API 추가** — `GET /api/ios/preflight` 가 Linux Docker 단독,
  MacBook 로컬 Xcode, `xcrun simctl` device type 상태를 구분해 반환한다. Linux 단독에서는
  Xcode 명령을 실행하지 않고 `mac_required` 로 차단 근거를 제공하며, macOS 에서는 iPhone/iPad
  Simulator device type, SDK/Xcode 버전, codesigning identity 를 best-effort 로 조회한다.
- **iPhone/macOS agent 설정 API 추가** — `ios.agent` config 섹션과
  `GET/POST /api/ios/agent-config` 를 추가했다. `local`/`ssh` 모드, host/port/user,
  agent workspace root, Xcode path 를 저장하며, SSH agent 활성화 시 필수 필드를 검증한다.
  `VIBECODER_IOS_AGENT_*` 환경변수 override 도 지원한다.
- **iPhone/macOS agent SSH bridge 하위 계층 추가** — iPhone preflight 가 `ios.agent`
  SSH 모드를 사용해 Linux 서버에서도 원격 Mac의 Xcode/SDK/Simulator device type 을 점검할
  수 있게 했다. agent 명령은 `IosAgentCommand` allowlist 로만 생성하며, workspace 동기화는
  `rsync -az --delete --dry-run` plan 을 먼저 만들고 `.git`, `.gradle`, `.vibecoder`,
  build output, DerivedData 를 제외하도록 제한했다. command runner 는 stdout/stderr 를
  실행 중 병렬 drain 해 큰 `xcodebuild` 로그에서도 pipe backpressure deadlock 을 피하고,
  SSH exit 255 는 `agent_unreachable` 로 분리한다.
- **iPhone build job 모델 추가** — 기존 build queue/log hub/history 흐름에 iOS variant
  `ios-debug`, `ios-test`, `ios-archive`, `ios-export-ipa` 를 추가했다. iPhone 프로젝트는
  macOS local/SSH agent 설정을 통과한 경우에만 `xcodebuild` 기반 build/test/archive/export
  job 을 큐에 넣을 수 있고, Android APK/AAB 빌드 endpoint 는 계속 차단된다.
- **iPhone build 취소 처리 보강** — iOS build 실행도 공용 `ProcessRunner` 경로를 사용해
  실행 중 `xcodebuild`/SSH bridge 로그를 stream drain 하며, build cancel 신호나 coroutine
  cancel 이 들어오면 자식 프로세스를 강제 종료하도록 맞췄다. 기존 Gradle/Flutter 경로도
  coroutine cancel 시 orphan process 가 남지 않도록 동일 러너를 보강했다.
- **iPhone export-ipa 산출물 경로 표준화** — `XcodeBuildRequest` 와 `XcodeArtifactFinder`
  를 추가해 iOS build 산출물을 `.vibecoder-ios-build/<variant>` 로 격리하고, `ios-export-ipa`
  는 archive 후 `xcodebuild -exportArchive` 를 실행해 생성된 `.ipa` 를 기존 artifact 저장
  흐름에서 찾도록 했다. SSH agent 모드는 원격 Mac 의 산출물 디렉터리를 서버 프로젝트
  workspace 로 다시 rsync 한다.
- **플랫폼 엔진 기반 build toolchain 선택** — `BuildService` 의 projectType별
  `when` 분기를 제거하고, `KotlinPlatformEngine` / `FlutterPlatformEngine` /
  `IPhonePlatformEngine` 이 각자 Gradle/Flutter/Xcode toolchain 선택을 소유하도록
  `PlatformBuildToolchains` 를 추가했다.
- **ProjectService platform engine 경유 확대** — clone projectType 감지, placeholder
  packageName/bundle id 보정, moduleName 감지, starter scaffold, 프로젝트 CLAUDE.md 생성,
  목록 version/icon 조회를 `ProjectPlatformEngine` 경유로 이동했다. Kotlin/Flutter/iPhone 별
  engine 책임을 회귀 테스트로 고정해 Android/Flutter 전용 detector 가 iPhone 흐름에 섞이지 않게 했다.
- **프로젝트 UI capability engine화** — 프로젝트 목록 타입 배지/Play 링크, iPhone Xcode build
  settings rail, Simulator rail, iPhone quick prompt 노출 여부를 `ProjectPlatformEngine.uiCapabilities()`
  로 이동했다. SSR template 은 projectType 문자열 분기 대신 engine capability snapshot 을 받아 렌더한다.
- **iPhone clone 감지 / starter 보강** — clone 타입 감지가 루트 `.xcodeproj`/`.xcworkspace`
  뿐 아니라 nested `project.pbxproj`, iOS platform 이 명시된 `Package.swift`, iOS 앱 단서가
  있는 `Info.plist` 를 `iphone` 후보로 판정한다. 신규 iPhone starter 는 SwiftUI app
  lifecycle, `ContentView`, bundle id `Info.plist`, 최소 Xcode project, XCTest scaffold 를
  함께 생성한다.
- **iPhone quick prompt 추가** — iPhone 프로젝트 콘솔 quick bar 에 SwiftUI 화면 구현과
  Xcode build 점검 prompt 를 조건부로 노출하고, 신규 프로젝트 템플릿 카탈로그에
  `iphone-swiftui-basic` starter prompt 를 추가했다.
- **Xcode build 실패 로그 분류 1차** — iOS build 실패 시 최근 로그 200줄만 bounded buffer 로
  스캔해 `project_missing`, `workspace_missing`, `scheme_missing`, `profile_missing`,
  `certificate_missing`, `swift_compile_failed`, `simulator_unavailable`, `test_failed` 를
  build log 에 요약한다. API wire shape 는 바꾸지 않는다.
- **Xcode build 실패 → 콘솔 수정 요청 연결** — 분류된 iOS build 실패 요약을 build 실패
  메시지에 보존하고, iOS 실패 상세 화면에서 현재 TUI console provider 로 buildId, variant,
  failure summary, build log 경로를 보내는 수정 요청 버튼을 추가했다. API wire shape 는
  변경하지 않는다.
- **Xcode shared scheme 감지 보강** — iOS build command 생성 시 `.xcscheme` 파일을 우선
  감지한다. `.xcworkspace` 는 함께 있는 `.xcodeproj/xcshareddata/xcschemes` 도 조회하며,
  다중 scheme 을 추론할 수 없으면 파일명 기반 오동작 대신 명확한 오류를 반환한다.
- **프로젝트별 Xcode build/signing 설정 UI 추가** — iPhone 프로젝트 우측 rail 에 scheme,
  Debug/Release configuration, bundle id, Team ID, export method, signing style,
  provisioning profile 지정 폼을 추가했다. 설정은 프로젝트 소스 루트의
  `.vibecoder-ios-build-settings.properties` 에 저장되고 이후 Xcode build/test/archive/export
  명령과 `exportOptions.plist` 생성에 반영된다.
- **원격 Mac iOS artifact 보존/정리 정책 보강** — SSH macOS agent 빌드 전 원격
  `.vibecoder-ios-build/<variant>` 디렉터리를 정리하고 필요한 `exportOptions.plist` 를 원격에
  다시 생성한다. 빌드 실패 시에도 원격 `.xcresult`/`.xcarchive`/`.dSYM` 산출물을 best-effort 로
  로컬 artifact root 에 회수해 실패 분석과 보존 흐름이 성공 빌드에만 의존하지 않게 했다.
- **iOS result bundle 보존** — iOS build/test/archive 후 `.xcresult`, `.xcarchive`, `.dSYM`
  bundle 을 zip 으로 압축해 기존 artifact 저장소에 `ios-xcresult`, `ios-xcarchive`,
  `ios-dsym` 타입으로 등록한다. 실패 빌드에서도 bundle 이 남아 있으면 best-effort 로 보존한다.
- **`.xcresult` 실패 summary 추출** — iOS build 실패 시 `xcrun xcresulttool get object --legacy
  --path ... --format json` 을 우선 사용하고 기존 `get --path` 를 fallback 으로 사용해 result
  bundle JSON 에서 test failure / compiler issue summary 를 추출한다. 추출된 요약은 build 실패
  메시지와 콘솔 수정 요청 prompt 에 함께 포함된다.
- **Build 실패 원인 kind 구조화** — `builds.failure_kind` nullable 컬럼과 `BuildDto.failureKind`
  nullable 필드를 추가했다. iOS/Xcode 분류 실패는 `swift_compile_failed`, `scheme_missing`,
  `profile_missing` 같은 kind 로 저장되어 JSON API 와 SSR build detail 에 표시된다.
- **iPhone Simulator inventory API 추가** — `GET /api/ios/simulators` 와
  `IosSimulatorListDto`/`IosSimulatorDto` 를 추가했다. Linux 단독에서는 `mac_required` 로
  Simulator UI 를 비활성화하고, MacBook local 또는 SSH macOS agent 에서만
  `xcrun simctl list -j devices available` allowlist 명령으로 iPhone/iPad simulator UDID,
  runtime, state 를 조회한다.
- **iPhone Simulator boot/shutdown API 추가** — `POST /api/ios/simulators/{udid}/boot`,
  `POST /api/ios/simulators/{udid}/shutdown` 과 `IosSimulatorActionDto` 를 추가했다. UDID 는
  안전한 문자 집합 또는 `booted` 별칭만 허용하고, Linux 단독에서는 `mac_required` 로 차단한다.
- **iPhone Simulator run API 추가** — `POST /api/projects/{id}/ios/simulators/{udid}/run` 과
  `IosSimulatorRunDto` 를 추가했다. 최신 `ios-debug` simulator `.app` 을 설치하고 bundle id 로
  launch 한 뒤 `.vibecoder-ios-build/simulator/latest-screenshot.png` 를 캡처한다. SSH macOS
  agent 모드에서는 원격 screenshot 을 서버 workspace 로 다시 rsync 한다.
- **iPhone Simulator 최근 로그 API 추가** — `GET /api/projects/{id}/ios/simulators/{udid}/logs` 와
  `IosSimulatorLogsDto` 를 추가했다. MacBook local 또는 SSH macOS agent 에서
  `simctl spawn <UDID> log show --last 5m --predicate 'process == "<bundleId>"'` 로 앱 로그를
  최근 200줄까지 제한 조회한다.
- **iPhone Simulator 웹 rail 1차 추가** — iPhone 프로젝트 우측 rail 에 Simulator 카드,
  device 선택, boot/shutdown, install+launch+screenshot, 최근 앱 로그 조회 버튼을 추가했다.
  screenshot preview 는 cookie 인증 SSR route 로 프로젝트 내부 최신 캡처 파일만 노출한다.
- **iPhone Simulator 실시간 로그 stream 추가** — `GET /ws/projects/{id}/ios/simulators/{udid}/logs`
  를 추가하고 웹 rail 의 Stream 버튼에 연결했다. MacBook local 또는 SSH macOS agent 에서
  `xcrun simctl spawn <UDID> log stream --predicate 'process == "<bundleId>"'` 를 실행하며,
  Linux 단독은 기존 Simulator 기능과 동일하게 `mac_required` 로 차단한다.
- **iPhone UI test 산출물 수집 보강** — `ios-test` job 의 `.xcresult` 번들을 기존 build
  artifact 저장소에 zip으로 보관하고, result bundle 내부 screenshot/image attachment 를 최신
  5개까지 별도 artifact 로 저장한다. 실패 요약은 기존 `xcresulttool` 파서와 함께 유지된다.
- **Flutter iOS target opt-in 추가** — Flutter 프로젝트 루트의
  `.vibecoder-flutter-targets.properties` (`targets=iphone` 또는 `targets=android,iphone`) 로
  iPhone build target 을 켤 수 있게 했다. 기본은 기존처럼 Android-only 이며, iPhone target 이
  켜진 경우 macOS agent에서 `flutter create --platforms=ios .`,
  `flutter build ios --debug --simulator --no-codesign`, `flutter build ipa --release` 를 실행한다.
  Simulator 실행은 Flutter debug 산출물 `build/ios/iphonesimulator/*.app` 도 찾는다.
- **iPhone macOS agent 요구사항 문서 추가** — `docs/iphone-macos-agent-requirements.md` 에
  MacBook 로컬 설치, 원격 macOS SSH agent, Xcode/Simulator, Apple Developer 계정, signing,
  App Store Connect API key 준비 항목을 공식 Apple 문서 링크와 함께 정리했다.
- **Swift 파일 미리보기/편집 하이라이트 추가** — Project file browser 가 `.swift` 를
  `text/x-swift` 로 인식하고 CodeMirror 5 Swift mode 를 로드해 Swift/SwiftUI 파일을
  iPhone 프로젝트에서 바로 읽고 편집할 수 있게 했다.
- **iPhone quick prompt 확장** — iPhone 프로젝트 콘솔 quick bar 에 SwiftUI 레이아웃,
  Xcode build, signing, Simulator, TestFlight 점검 프롬프트를 모두 노출한다.
- **Swift symbol scan 추가** — 기존 `/symbols` regex scanner 가 `.swift` 파일을 스캔하고
  Swift `struct`/`class`/`protocol`/`enum`/`func`, SwiftUI `View` conformer 를 찾아
  파일 에디터 line link 로 연결한다.
- **전역 CLAUDE.md seed 공통화** — `container-global-claude.ko.md` / `.en.md` 기본 seed 에서
  플랫폼별 빌드/UI/signing/store/simulator 세부 지침을 제거하고, 모바일 공통 운영·보안·문서화
  원칙만 남겼다. 플랫폼별 세부 규칙은 프로젝트 생성 시 선택된 engine 이 지역 `CLAUDE.md` 로
  주입한다.
- **전역 기본 MCP 축소** — `McpCatalog.defaultInstallIds` 를 플랫폼 중립 4종
  (`context7`, `memory`, `sequentialthinking`, `time`)으로 제한했다. `playwright`,
  `mobile-mcp`, release/publish 계열은 projectType별 tooling profile 의 조건부/opt-in 도구로
  이동했다.
- **프로젝트 로컬 Skill/Agent seed 플랫폼 분리** — 신규/clone 프로젝트 생성 시 선택된
  `ProjectPlatformEngine.toolingProfile()` 의 기본 Skill/Agent 만 `.claude/skills` 와
  `.claude/agents` 에 누락분으로 생성한다. 기존 사용자 파일은 덮어쓰지 않으며,
  Kotlin/Flutter/iPhone 간 default tooling 이 섞이지 않도록 회귀 테스트를 추가했다.
- **build variant 지원 판정 engine화** — `BuildService` 의 Kotlin/Flutter/iPhone 직접 타입
  비교를 제거하고, 각 `ProjectPlatformEngine.supportsBuildVariant()` 가 Android 계열
  APK/AAB와 iOS Xcode variant 지원 여부를 소유하도록 정리했다. Mac agent 필요 여부는 iOS
  variant 실행 환경 검증으로 분리했다.
- **App Store Connect key secret store 추가** — iPhone/TestFlight release 준비용
  `AppStoreConnectKeyStore` 를 추가해 ASC key id, issuer id, `.p8` private key 를 workspace
  sidecar 의 `.vibecoder/secrets/app-store-connect` 아래에 저장한다. 디렉터리는 0700,
  key/metadata 파일은 0600 권한을 best-effort 로 적용하고, API 응답에는 private key 본문을
  반환하지 않는다. TestFlight precheck 는 MCP 환경값과 이 secret store 둘 다 인정한다.
- **App Store Connect 연결 진단 추가** — 저장된 ASC `.p8` key 로 ES256 JWT 를 서버 내부에서
  생성하고 App Store Connect `/v1/apps` 를 read-only 조회하는
  `GET /api/ios/app-store-connect/diagnostics` 를 추가했다. 선택 `bundleId` query 를 주면
  app id/name 매칭 여부까지 확인하며, private key 본문은 응답하지 않는다.
- **TestFlight upload job 이력 추가** — iPhone build detail 에 TestFlight 업로드 카드를 다시
  노출하고, 업로드 trigger 시 `testflight_upload_jobs` row 를 생성해 `queued` →
  `uploading` 또는 `failed` 상태를 저장한다. 서버가 관측 가능한 prompt 전송 단계는 DB에
  기록하고, 실제 App Store Connect processing/accepted 판정은 콘솔 provider/MCP 결과를
  사용자가 확인하는 흐름으로 남긴다.
- **TestFlight 업로드 실패 정규화 추가** — provider/MCP 결과 수신부가 upload job 상태를
  갱신할 수 있도록 status update API 를 추가하고, 알려진 Transporter/App Store Connect
  실패 문구를 `invalid_provisioning`, `duplicate_build_number`, `missing_compliance`,
  `invalid_icon_screenshot_metadata`, `authentication_failed`, `invalid_ipa` 로 정규화한다.
- **TestFlight ASC 상태 polling 추가** — active upload job(`uploading`/`processing`)은
  App Store Connect `/v1/builds` 를 주기적으로 조회해 Apple `processingState` 를 내부 상태로
  전이한다. `PROCESSING` 은 `processing`, `VALID` 은 `accepted`, `FAILED`/`INVALID` 는
  `failed(asc_processing_failed)` 로 기록한다. ASC key 미설정, 인증 실패, build 미발견은
  기존 job 상태를 덮지 않고 다음 주기에 재시도한다.
- **TestFlight build number 자동 증가 정책 추가** — upload prompt 생성 시 Xcode
  `CURRENT_PROJECT_VERSION`, `Info.plist` `CFBundleVersion`, Flutter `pubspec.yaml`
  build metadata 중 가장 큰 정수를 읽어 다음 build number 를 제안한다.
- **iPhone signing status/profile 검증 추가** — local Mac 또는 SSH macOS agent 에서
  codesigning identity 와 installed provisioning profile metadata 를 조회해 bundle id/team id
  매칭, 만료 여부, profile 누락/불일치 blocked reason 을 빌드 전에 확인할 수 있게 했다.
  `.mobileprovision` 원문은 반환하지 않고 UUID/name/team/expiry/bundle metadata 만 반환한다.
- **iPhone keychain import API 추가** — admin 전용 `POST /api/ios/keychain/import` 로
  Mac local 또는 SSH macOS agent 에 `.p12` signing certificate 를 import 한다. 전용 keychain
  생성, unlock, `security import`, `set-key-partition-list`, user keychain search list 등록을
  수행하고, 응답에는 password 를 반환하지 않는다.
- **iPhone Swift tools 선택 설치 API 추가** — admin 전용
  `POST /api/ios/swift-tools/install` 로 Mac local 또는 SSH macOS agent 에서 Homebrew 기반
  SwiftLint/SwiftFormat 을 선택 설치한다. Homebrew 가 없으면 `homebrew_missing` 으로 차단한다.
- **iPhone Signing 상태 rail 추가** — iPhone 프로젝트 우측 rail 에 `Signing Status` 카드를
  추가해 `GET /api/projects/{id}/ios/signing-status` 응답 하나를 기준으로 certificate identity,
  provisioning profile uuid/name/expiration, bundle/team match, blocked reason, warnings 를
  갱신 표시한다. Kotlin/Flutter 프로젝트에는 노출하지 않는다.
- **Xcode signing failureKind 정규화 보강** — expired profile, bundle/team mismatch,
  keychain locked, untrusted/missing certificate 패턴을 `profile_expired`,
  `profile_bundle_mismatch`, `profile_team_mismatch`, `keychain_locked`,
  `certificate_missing` 으로 분류한다.

### Removed

- **Factory Wizard / supervisor 기능 제거** — 프로젝트별 supervisor slot, `/wizard` SSR,
  `/api/projects/{id}/wizard*` 및 `/api/wizard/projects` JSON API, 관련 WebSocket frame,
  `.supervisor` 초기화/quality gate/planner/provider supervisor turn 경로, 전역
  `CLAUDE.md` 자동 supervisor 지침 seed, 프로젝트/콘솔/오버뷰 Wizard UI 를 제거했다.
- **레거시 세션/turn 제한 배선 제거** — `/settings` 의 Claude 동시 turn 상한 UI와 저장
  로직을 제거하고, Claude/Codex/OpenCode 상주 세션 캡과 Claude turn gate 런타임 적용을
  비활성화했다. 과거 외부 `server.yml` 은 읽을 수 있도록 키만 호환 필드로 남기며, 새
  AI/빌드 작업 시작 및 idle TUI 회수 판단은 `ResourceGuard` 메모리 정책 한 곳에서만 수행한다.
- **빌드환경 provider 인증 경로 단일화** — 빌드환경 카드 순서를 Claude, Codex, OpenCode
  순서로 고정하고 Claude 는 웹 OAuth 옵션 0, Codex 는 device-code, OpenCode 는 z.ai API key
  한 경로만 노출한다. Claude credentials 업로드/API-key 및 Codex access-token/API-key
  대체 로그인 UI와 대응 JSON API/DTO/경로 상수를 제거했다.

### Fixed

- **ResourceGuard soft pressure 차단 완화** — 운영에서 24GB cgroup 기준 사용률 82~87%,
  여유 3~4GB 상태에서도 console TUI 시작이 503으로 막히던 문제를 수정했다. soft 임계값은
  연결 끊긴 idle TUI 세션 정리 트리거로만 사용하고, 새 AI/빌드 작업 차단은 hard 임계값
  또는 최소 여유 메모리 미만일 때만 수행한다. 기본값도 soft 88%, hard 96%,
  min-free 768MB 로 완화했다.
- **운영 pids limit 부족으로 인한 CLI 오진 완화** — Claude/Codex/OpenCode TUI와 MCP
  서버가 동시에 떠 있는 운영 환경에서 컨테이너 PIDs가 2048 한도에 근접해 `fork`/스레드
  생성이 `EAGAIN`으로 실패하고, 이 여파로 `claude --version` 진단이 "CLI 미설치"로
  표시되던 문제를 완화했다. 서버 컨테이너 기본 pids limit 을 8192로 올리고, Claude
  버전 확인 비정상 종료는 미설치가 아닌 경고로 분류하며, 이미 살아있는 TUI 세션은
  진단 실패 배너로 차단하지 않는다.
- **Project console TUI 메모리/부팅/브라우저 payload 정리** — provider-native TUI PTY가
  무기한 상주하지 않도록 연결 끊긴 idle 세션 기본 reaper(`security.consoleTuiIdleTimeoutMinutes`
  120분)를 추가하고, 새 TUI/빌드 시작 제한은 `ResourceGuard` 메모리 정책 한 곳에서만
  판단하도록 정리했다. prompt history ingest 는 project/provider 당 최신 poller 하나만
  유지하고, OpenCode transcript 적재는 `time_created` offset 기반 증분 조회로 바꿔
  프롬프트마다 SQLite 전체 assistant row 를 재파싱하지 않게 했다. Codex MCP user-scope 동기화는 동일 JSON marker 가 있으면 remove/add
  CLI 호출을 건너뛰며, 콘솔 SSR 에서 더 이상 쓰지 않는 신규 이미지 첨부 모달/FileReader/canvas
  JS 와 xterm serialize/web-links addon 로드를 제거했다. 콘솔 history 초기/더보기 조회는
  `COUNT(*) + OFFSET` 대신 최신 N개 직접 조회로 바꿨다.
- **TUI 직접 입력 prompt history 오염 원인 차단** — 기존에는 xterm.js `onData`
  raw PTY 입력 스트림을 서버가 프롬프트로 추론해 기록했기 때문에, 마우스/터치 스크롤,
  focus, bracketed paste, ANSI/OSC/SS3 escape sequence, private-use 글리프가 최근 프롬프트
  히스토리에 섞일 수 있었다. PTY 입력 전달과 히스토리 기록 의도를 분리해, 브라우저가
  실제 제출 텍스트로 판정한 값만 `recordPrompt` 로 명시할 때 DB에 기록하도록 바꿨다.
  방어적으로 서버/클라이언트 양쪽에 escape artifact 와 private-use 노이즈 필터도 유지한다.
- **전역 CLAUDE.md 기본 seed / 운영본 갭 정리** — 운영 서버의 `/home/vibe/.claude/CLAUDE.md`
  사용자 편집본은 보존하면서 잘못된 Codex 경로(`/home/vibe/.config/codex/...`)를 실제
  `/home/vibe/.codex/...` 기준으로 정정했다.
- **OpenCode/GLM z.ai 강제 모드의 기본 capability 인식 갭 보강** — 운영 점검 결과 Claude 와
  Codex MCP 는 user-scope 로 동기화되어 있었지만, OpenCode/GLM z.ai 강제 모드가
  `OPENCODE_CONFIG_HOME` 을 격리 디렉토리로 바꾸면서 일반 `opencode.jsonc` 의
  `skills.paths` 와 기본 MCP 블록을 읽지 못했다. 강제 config 생성 시 전역
  `/home/vibe/.claude/skills` 경로와 무키/무비용 기본 MCP(`memory`, `sequentialthinking`,
  `context7`, `playwright`, `mobile-mcp`, `android-docs`)를 포함하도록 수정했다.
- **Codex 전역 기본 스킬 seed 보강** — 기존에는 기본 skill/agent seed 가
  Claude 전역 `.claude/skills` / `.claude/agents` 중심이라 Codex 의 `.codex/skills` 에
  신규 기본 스킬이 일부 누락될 수 있었다. 서버 시작 시 기본 스킬을 Codex
  전역 skills registry 에도 누락분만 seed 한다. Claude sub-agent 정의는 Claude Code
  네이티브 기능이므로 Codex/OpenCode 에서는 전역 CLAUDE/AGENTS 지침과 worker prompt 로
  용도만 전달한다.

### Added

- **Resource guard / Docker memory hard limit** — Claude/Codex/OpenCode TUI, Gradle,
  Flutter 같은 메모리 사용량이 큰 자식 프로세스가 호스트 PC 전체를 멈추게 하지 않도록
  compose 기본값에 서버/PG 컨테이너 cgroup memory/pids limit 을 추가했다. 서버 내부에는
  cgroup 메모리 사용률을 감시하는 `ResourceGuard` 를 추가해 soft 임계치에서는 연결 끊긴
  idle TUI 세션을 먼저 종료하고, hard 임계치 또는 최소 여유 메모리 미만에서 새 AI/빌드
  작업 시작을 보류한다.
  운영자는 `.env` 의 `VIBECODER_SERVER_MEMORY_LIMIT`,
  `VIBECODER_RESOURCE_MEMORY_SOFT_PERCENT`, `VIBECODER_RESOURCE_MEMORY_HARD_PERCENT`,
  `VIBECODER_RESOURCE_MIN_FREE_MEMORY_MB` 로 조정할 수 있다.
- **Project console TUI-only migration(breaking for legacy JSON console)** — 프로젝트 콘솔에 xterm.js 기반 접이식
  TUI 패널을 추가하고, provider CLI 를 project-scoped PTY 로 실행하는 REST/WS 배선을
  신설했다. `/terminal` 의 일반 bash 세션과 분리된 `ConsoleTuiSessionManager` 가
  project/user/provider 단위 세션을 유지하며, 기존 JSON console runtime 은 더 이상
  prompt 실행 경로로 쓰지 않는다. 기존 `.../claude/console/prompt|interrupt` endpoint 도
  동일 TUI dispatcher 로 들어가며, `PromptRequestDto.images` 는 TUI 제약상
  `images_unsupported` 로 거부된다. 신규/유지 endpoint:
  `POST|GET /api/projects/{id}/console/tui/session`,
  ...
  `WS /ws/projects/{id}/console/tui/{sessionId}`.
  TUI prompt/compact endpoint 는 user turn 을 conversation DB 에 기록한 뒤 같은 텍스트를
  bracketed paste 로 provider TUI stdin 에 주입한다. `console/tui/mode` 는 과거 토글
  호출 호환용으로 남지만 서버는 항상 TUI enabled 를 반환한다.
- **Codex TUI history ingest(additive)** — Codex provider TUI 세션도 `~/.codex/state_*.sqlite`
  의 `threads.rollout_path` 에서 현재 project directory 의 rollout JSONL 을 찾아 assistant
  `response_item` / `output_text` 를 `conversation_turns` 에 best-effort 로 적재한다.
  prompt 전 offset baseline 을 잡아 기존 Codex 히스토리가 한꺼번에 import 되지 않게 했다.
- **기본 MCP 업그레이드 보강** — `McpCatalog.defaultInstallIds` 는 first-run 뿐 아니라
  기존 설치의 이미지 업그레이드 시에도 누락분을 user-scope 에 보강 등록한다. `android-docs` /
  `gradle-agp-docs` capability 는 기본 MCP 인 `context7` 등록도 documentation source 로
  인식한다.
- **무키/무비용 기본 MCP 공통화** — 기본 MCP 는 `context7`, `memory`,
  `sequentialthinking`, `time` 처럼 API key 와 유료 외부 API가 필요 없고 플랫폼 중립인
  항목으로 제한한다. Git hosting, browser/mobile automation, release/publish 계열은
  projectType별 tooling profile 의 조건부/opt-in 항목으로 유지한다.

### Wire change

- **프로젝트 타입/도구 profile wire 추가(additive)** —
  `ProjectTypes.IPHONE` 을 추가하고 `ProjectDto.projectType` /
  `RegisterProjectRequestDto.projectType` 허용 값에 `iphone` 을 포함했다.
  `GET /api/projects/{id}/tooling-profile` 과 `PlatformToolingProfileDto` 를 추가해
  클라이언트가 projectType별 기본/조건부/opt-in MCP, Skill, Agent, forbidden tool 목록을
  조회할 수 있게 했다. 구버전 클라이언트는 기존 `kotlin` 기본값으로 계속 동작한다.
- **iPhone preflight wire 추가(additive)** — `GET /api/ios/preflight` 와 `IosPreflightDto` 를
  추가했다. 이후 additive 필드 `codesigningIdentities` 를 더해 macOS signing preflight 결과를
  함께 반환한다. 구버전 클라이언트에는 영향 없고, iPhone/macOS 기능 노출 판단이 필요한
  클라이언트는 이 endpoint 를 사용해야 한다.
- **iPhone agent config wire 추가(additive)** — `GET/POST /api/ios/agent-config` 와
  `IosAgentConfigDto` 를 추가했다. iPhone 원격 Mac agent 설정 UI/클라이언트는 이 endpoint 로
  `ios.agent` 설정을 조회/저장해야 한다.
- **App Store Connect key wire 추가(additive)** — `GET/POST /api/ios/app-store-connect-key`,
  `IosAppStoreConnectKeyDto`, `IosAppStoreConnectKeySaveRequestDto` 를 추가했다. 응답은 key id,
  issuer id, private key 존재 여부와 저장 경로 metadata 만 반환하며 `.p8` 본문은 반환하지 않는다.
  구버전 클라이언트에는 영향 없고, iPhone/TestFlight release UI가 필요하면 이 endpoint 를 사용한다.
- **App Store Connect diagnostics wire 추가(additive)** —
  `GET /api/ios/app-store-connect/diagnostics?bundleId=<id>` 와
  `IosAppStoreConnectDiagnosticDto` 를 추가했다. 구버전 클라이언트에는 영향 없고,
  TestFlight release UI는 ASC 인증 상태와 bundle id/app id 조회 결과를 이 endpoint 로 표시할 수 있다.
- **TestFlight upload wire 추가(additive)** —
  `POST /api/projects/{id}/builds/{buildId}/testflight-upload`,
  `GET /api/projects/{id}/testflight/uploads`,
  `POST /api/projects/{id}/testflight/uploads/{jobId}/status`, `TestFlightUploadRequestDto`,
  `TestFlightUploadStatusUpdateRequestDto`, `TestFlightUploadResponseDto`,
  `TestFlightUploadJobDto` 를 추가했다. 구버전 클라이언트에는 영향 없고, iPhone release UI는
  이 endpoint 로 upload trigger, 이력 표시, 상태 갱신을 사용할 수 있다.
- **iPhone signing status wire 추가(additive)** —
  `GET /api/projects/{id}/ios/signing-status`, `IosSigningStatusDto`,
  `IosSigningProfileDto` 를 추가했다. 구버전 클라이언트에는 영향 없고, iPhone release UI는
  이 endpoint 로 signing blocked reason/profile expiry/team/bundle match 를 표시해야 한다.
- **iPhone keychain import wire 추가(additive)** —
  `POST /api/ios/keychain/import`, `IosKeychainImportRequestDto`,
  `IosKeychainImportDto` 를 추가했다. 구버전 클라이언트에는 영향 없고, iPhone signing 관리 UI가
  `.p12` import/unlock 작업을 제공하려면 이 endpoint 를 사용한다.
- **iPhone Swift tools install wire 추가(additive)** —
  `POST /api/ios/swift-tools/install`, `IosSwiftToolsInstallRequestDto`,
  `IosSwiftToolsInstallDto` 를 추가했다. 구버전 클라이언트에는 영향 없고, iPhone 품질 도구 설치
  UI가 필요하면 이 endpoint 를 사용한다.
- **iPhone simulator inventory wire 추가(additive)** — `GET /api/ios/simulators` 와
  `IosSimulatorListDto` / `IosSimulatorDto` 를 추가했다. MacBook local 또는 SSH macOS agent
  에서 사용 가능한 iPhone/iPad Simulator 목록과 UI 노출 가능 여부를 조회한다.
- **iPhone simulator control wire 추가(additive)** —
  `POST /api/ios/simulators/{udid}/boot`,
  `POST /api/ios/simulators/{udid}/shutdown` 과 `IosSimulatorActionDto` 를 추가했다.
  전역 Simulator boot/shutdown 제어용 endpoint 이다.
- **iPhone simulator run wire 추가(additive)** —
  `POST /api/projects/{id}/ios/simulators/{udid}/run` 과 `IosSimulatorRunDto` 를 추가했다.
  최신 debug simulator `.app` 설치, bundle id launch, screenshot 캡처 결과를 반환한다.
- **iPhone simulator logs wire 추가(additive)** —
  `GET /api/projects/{id}/ios/simulators/{udid}/logs` 와 `IosSimulatorLogsDto` 를 추가했다.
  iPhone 프로젝트의 bundle id 기준 최근 Simulator 앱 로그를 bounded response 로 반환한다.
- **iPhone build wire 추가(additive)** —
  `POST /api/projects/{id}/ios/build/debug`,
  `POST /api/projects/{id}/ios/build/test`,
  `POST /api/projects/{id}/ios/build/archive`,
  `POST /api/projects/{id}/ios/build/export-ipa` 를 추가했다. 응답은 기존 `BuildDto` 이며,
  build logs 는 기존 `/ws/builds/{buildId}/logs` 흐름을 사용한다.
- **Factory Wizard / supervisor wire 제거(breaking)** —
  `shared/ApiPath.kt` 의 wizard endpoint, `FactoryWizardDtos.kt`,
  `WsFrame.FactoryWizardStatusChanged`, `WsFrame.FactoryWizardTimelineEvent` 를 제거했다.
  Android 클라이언트가 해당 API/DTO/frame 을 참조한다면 shared 동기화와 UI/API 제거가 필요하다.
- **`WsFrame.TerminalInput.recordPrompt` 추가(additive)** — TUI PTY raw input(`data`)과
  prompt history 기록 대상 텍스트를 분리하기 위해 nullable `recordPrompt` 필드를 추가했다.
  구버전 클라이언트가 이 필드를 보내지 않아도 서버 decode 는 그대로 동작한다. Android 가
  TUI 직접 입력 히스토리 기록을 원하면 `TerminalInput` 송신 시 제출 텍스트만 이 필드에 넣어야 한다.

## [1.162.1] — 2026-07-17

### Fixed (v1.162.1)

- **foldable 에뮬레이터 슬롯 2개가 폴더블로 안 만들어지던 문제 회수 — 5개 슬롯 전부 정상 폼팩터** —
  `EmulatorService.buildSlots` 의 foldable/fold7 슬롯 deviceProfile 이 `pixel_fold` 였는데,
  이 값은 현재 cmdline-tools(11076708)의 `avdmanager list device` 목록에 **없어서(INVALID)**
  AVD 생성이 실패하고 `ensureAvd` 의 기본 프로파일 fallback(pixel_6, phone)으로 잘못
  만들어졌다. 유효한 fold device id 로 교체:
    - foldable → `"7.6in Foldable"` (id 62)
    - fold7 (Foldable wide) → `"8in Foldable"` (id 63)
  - 유효성 확인: `pixel_6`/`pixel_7_pro`/`pixel_tablet` 은 VALID, `pixel_fold` 만 INVALID 였다.
  - 이제 5개 슬롯(Phone/Large phone/Tablet/Foldable/Foldable wide) 모두 정상 폼팩터 +
    google_apis(v1.162.0) 이미지로 생성 → 전부 스크린샷 촬영 가능.
  - Android/wire 영향 없음(서버 내부 상수).

## [1.162.0] — 2026-07-17

### Changed (v1.162.0)

- **에뮬레이터 시스템 이미지 google_atd → google_apis 교체 — 스크린샷/화면 검토 활성화** —
  `/emulator` 헤드리스 에뮬레이터에서 `adb screencap`(= mobile-mcp `mobile_take_screenshot`)
  이 항상 검은 화면만 반환하던 문제의 근본 회수. 원인은 시스템 이미지가 `google_atd`
  (Automated Test Device)였던 것 — ATD 이미지는 CPU/메모리 절감을 위해 **하드웨어 렌더링을
  비활성화**하며, Google 공식 문서상 스크린샷처럼 렌더링에 의존하는 작업은 지원되지 않는다.
  v1.73.0/v1.160.0 이 "adb 로그분석 전용 경량"으로 의도적으로 ATD 를 골랐으나, 화면 검토
  용도와 정면 충돌했다.
  - **이미지 교체**: `EmulatorService`(기본 systemImage), `EnvSetupService`(감지 우선순위),
    `docker/doctor/manifest.yml`(설치 대상), i18n(EN/Ko), `docker/README.md` 를
    `system-images;android-35;google_apis;x86_64` 로 정렬. `-no-window`/`swiftshader_indirect`
    는 원인이 아니어서 유지(일반 이미지 + swiftshader 는 CI 스크린샷 표준 조합).
  - **AVD stale 재생성**: 이미지만 바꿔도 구 `google_atd` 로 만들어진 기존 AVD 가 `ensureAvd`
    의 `avdExists` skip 때문에 재사용돼 검은 화면이 재발한다. `avdImageMatches`(config.ini
    `image.sysdir.1` 대조) 불일치 시 `deleteAvd` 후 재생성하도록 보강 — 배포 후 새 이미지로
    자동 승격.
  - **폴백**: `FALLBACK_ATD_IMAGE`(구 상수 `LEGACY_GOOGLE_APIS_IMAGE` 대체) — google_apis 가
    아직 설치 안 됐고 google_atd 만 남은 구 환경에서 최소 동작(logcat/설치)만 유지, 스크린샷은
    이미지 재설치 후 가능.
  - **운영 마이그레이션**: 서버 배포 후 `/env-setup` 에서 "안드로이드 에뮬레이터" 재설치(또는
    `vibe-doctor android`)로 google_apis 이미지를 받아야 한다. 기존 AVD 는 자동 재생성됨.
    mobile-mcp 는 이미 MCP 카탈로그(v1.95.0)에 있어 프로젝트 콘솔에서 바로 사용 가능.
  - Android/wire 영향 없음(서버 내부 + 빌드환경 변경).

## [1.161.2] — 2026-07-17

### Fixed (v1.161.2)

- **소프트키보드 모드에서 콘솔 입력창(textarea)이 "반토막" 나던 문제 회수 (v1.161.1 후속)** —
  태블릿/모바일에서 소프트키보드가 올라오면 프롬프트 입력창의 높이가 우측 버튼 컬럼의 위 절반
  (마이크+이미지 버튼) 높이에서 잘려, 아래 절반(끼어들기~전송 버튼 옆)이 빈 공간으로 낭비됐다.
  - 원인: v1.161.1 이 로그 공간 확보를 위해 키보드 모드 textarea 에 `max-height: 64px` 를
    걸었는데, 이 값이 마이크+이미지 2개 버튼 높이 수준이라 textarea 가 거기서 clamp 됐다.
    입력 영역 row 는 `align-items:stretch` 이고, 우측 버튼 컬럼(voice/image/interrupt/send,
    `flex-shrink:0`)이 이미 row 높이를 지배하므로, textarea 는 그 높이(=전송 버튼 하단)까지
    늘어날 수 있었는데도 `max-height` 가 이를 막았다.
  - `body.console-keyboard .prompt-form textarea { max-height: none }` 로 변경 → textarea 가
    우측 버튼 컬럼 전체 높이(전송 버튼 하단)까지 stretch. 버튼 컬럼이 이미 row 높이를 결정하고
    있어 로그 공간을 추가로 잠식하지 않고, 낭비되던 아래 절반만 회수한다.
  - 태블릿 뷰포트(720×560, `body.console-keyboard`)로 실측: 수정 전 textarea 높이 64px ·
    전송 버튼 하단까지 108px 빈 공간 → 수정 후 172px · 빈 공간 0px(전송 버튼 하단과 정확히 정렬).
  - Android/wire 영향 없음(SSR CSS 만).

## [1.161.1] — 2026-07-16

### Fixed (v1.161.1)

- **모바일/태블릿 콘솔 소프트키보드 겹침 완전 회수 (v1.161.0 후속)** — v1.161.0 에서 iframe
  `.layout` 이 실제 iframe 높이를 벗어나던 문제는 고쳤으나, 실기(태블릿 가로)에서 여전히
  콘솔 로그가 입력창(textarea)을 덮는 겹침이 남아 "텍스트창이 콘솔 뒤로 가려짐" 상태였다.
  실서버 iframe DOM 을 태블릿 뷰포트로 실측한 결과, 진짜 원인은 키보드 모드에서
  `console-log` 의 `min-height:72px` 가 flex 축소를 막아, 좁은 세로에서 console-log-wrap 이
  압축돼도 console-log 가 wrap 밖으로 넘쳐 입력창을 약 55px 덮던 것이었다.
  - `body.console-keyboard .console-log { min-height: 0 }` + `.console-log-wrap { overflow: hidden }`
    로 로그가 남는 세로만 차지(내부 스크롤)하게 하고 넘침을 이중 차단.
  - 좁은 세로에서 입력폼이 로그 공간을 과점하지 않도록 keyboard 모드 textarea `max-height`
    96→64, 입력창 아래 힌트 줄(전송/줄바꿈 안내) 접기.
  - 실서버(vibe.codr.kr) 콘솔 iframe 소프트키보드 상황(부모 `--app-viewport-height` 축소)을
    시뮬해 측정: 수정 전 겹침 55px → 수정 후 -2px(겹침 없음).
  - Android/wire 영향 없음(SSR CSS 만).

### Changed (v1.161.1)

- **'프로젝트 설정' 드롭다운을 헤더 우상단 → 우측 개요 카드 안으로 이동 (사용자 요청)** —
  통합 탭 헤더의 `⚙ 프로젝트 설정` 드롭다운(메타데이터 + 편집/zip/env-files 링크 +
  archive/delete 액션)을 우측 오버뷰 rail 의 '개요' 카드 하단으로 옮겨, 프로젝트 정보와 설정
  액션을 한 곳에 모았다. 드롭다운 메뉴는 좁은 rail 폭에 맞게 `min-width` 제거 + 카드 폭 정렬.
  - Android/wire 영향 없음(SSR HTML/CSS 만).

## [1.161.0] — 2026-07-13

### Changed (v1.161.0)

- **Codex 사용량 5h 게이지 파싱 보강** — 운영(codr.kr)에서 Codex 사용량 pill/card 가 주간(7d)
  만 표시되는 원인은 파서가 `/status` TUI 의 세션 한도 라벨을 `5h limit:` 고정 문자열로만
  인식한 점이다. Codex CLI 출력이 `5-hour limit:` / `Session limit:` 류로 바뀌면
  `weeklyUsagePercent` 만 채워지고 `sessionUsagePercent` 는 null 이 되어 UI 가 7d 만 렌더했다.
  - `parseCodexUsageCapture` 가 `5h`/`5-hour`/`5 hour`/`session`/`hourly` 라벨을 모두
    세션(5h) 한도로 분류.
  - 여러 `* limit` 줄 중 weekly 로 분류된 줄과 별도의 알 수 없는 limit 줄이 있으면, 그 줄을
    보수적으로 session fallback 으로 사용.
  - `/usage` slash command 는 "usage limit reset 사용" 동작일 수 있어 자동 캡처에 추가하지 않음.
  - Android/wire 영향 없음(DTO 구조 변경 없이 서버 파싱 로직만 보강).

- **한글 UI quota reset 시각 표시 통일** — Claude/Codex 의 5시간/주간 reset 값이
  `Resets 10:49pm (Asia/Seoul)` / `11:03 on 30 Jun` / `22:49` 같은 free-form 으로 내려와
  한글 UI 에서 날짜 없이 보이던 문제를 회수. GLM 처럼 ISO 로 내려오는 값과 함께 같은
  `fmtQuotaReset` / sidebar `formatReset` 경로에서 `7월 18일 22:49` 형식으로 표시한다.
  시간만 있는 값은 다음 도래 시각(이미 지났으면 다음 날)으로 해석.
  - Android/wire 영향 없음(표시 포맷만 변경).

- **프로젝트 키스토어 업로드 4파일 set 보강** — 프로젝트 키스토어 생성은 release/debug
  keystore 와 release/debug properties 를 만들지만, 업로드 폼/라우트는 기존 3파일
  (release / debug / release properties)만 받던 불일치를 수정. 업로드에서
  `debugProperties` multipart 필드를 추가로 받아 `<pkg>-debug-keystore.properties` 로
  staging 하고, `storeFile=/home/vibe/keystores/<pkg>-debug.keystore` 로 정규화한다.
  - 업로드 후 배치 프롬프트도 debug properties 를 이동 목록에 포함하고, debug signingConfig 는
    `<pkg>-debug-keystore.properties` 가 있으면 우선 사용하도록 안내.
  - Android/wire 영향 없음(SSR multipart 필드 추가, 기존 필드/경로 유지).

- **자동 콘솔 프롬프트 provider 고정 제거** — 프로젝트 키스토어 탭과 전역 Keystores 페이지의
  서명 적용/AdMob 적용/업로드 후 배치 프롬프트, 스토어 자산의 런처 아이콘 적용, Play/TestFlight
  업로드 위임, 품질/접근성 fix 요청, 패키지명 변경 후 코드 리네임, action registry 의
  `kind=prompt` 전송이 `ClaudeSessionManager` 로 직접 전송되던 하드코딩을 제거하고,
  `AgentRouter` 를 통해 해당 프로젝트 콘솔에서 현재 선택된 provider(Claude/Codex/OpenCode)로
  전송하도록 변경.
  - 유휴 판정도 선택 provider 의 busy 상태를 기준으로 맞춰, 선택 provider 와 다른 콘솔 상태가
    버튼 활성/전송 가드를 좌우하지 않게 했다.
  - Android/wire 영향: DTO/경로 변경 없음. 기존 Android `POST /api/projects/{id}/quality/fix`
    호출은 그대로 동작하지만, 수정 프롬프트 전송 대상이 Claude 고정이 아니라 프로젝트의 현재
    선택 provider 로 바뀐다.

- **docker-compose 예시 문서 정리** — README / Docker Hub README / docker README / 고급 기능
  문서의 compose 예시를 현재 `docker/compose.yml` 기준으로 갱신. `~/.codex`, `~/.opencode`,
  `~/.android` bind mount 와 현재 GitHub raw URL(`vibe-coder-server`)을 반영하고, 오래된
  `:full` emulator 이미지 예시는 현재 KVM device mount 예시로 교체.

- **Claude/Codex/OpenCode 홈 디렉토리 영속성 정리** — 이미지 업데이트/컨테이너 재생성 후
  웹앱에서 설치한 AI provider 관련 `~/.xxx` 하위 상태가 사라지지 않도록 compose/Dockerfile/
  entrypoint 기준을 명시화했다.
  - `docker/compose.yml`: `dev-tools/codex:/home/vibe/.codex` 마운트 추가. 기존 Claude
    `~/.claude` 와 OpenCode `~/.opencode` 마운트와 함께 세 provider 의 홈 디렉토리 상태를
    모두 bind mount 로 보존.
  - `docker/Dockerfile` + 서버 spawn env: `CODEX_HOME=/home/vibe/.codex` 로 Codex 기본 경로를
    native `~/.codex` 로 정리. v1.145.0~v1.160.x 의 `/home/vibe/.config/codex` 상태는
    entrypoint 가 새 `~/.codex` 볼륨이 비어 있을 때 1회 복사해 로그인/설정을 보존.
  - `entrypoint.sh`: 신규 `~/.codex`/`~/.opencode` bind mount 권한 보정 및 Codex
    `AGENTS.md` → `~/.claude/CLAUDE.md` symlink 경로 갱신.
  - 운영 반영(codr.kr): 운영 compose 의 `vibe-coder-server` 서비스에
    `${VIBE_DATA_ROOT:-./vibe-coder-data}/dev-tools/codex:/home/vibe/.codex` 와
    `${VIBE_DATA_ROOT:-./vibe-coder-data}/dev-tools/opencode:/home/vibe/.opencode` 가 모두
    있는지 확인 후 재생성.
  - Android/wire 영향 없음(Docker/entrypoint/env/documentation 변경만 — `ApiPath`/DTO/`WsFrame`
    무변경).

### Fixed (v1.161.0)

- **모바일/태블릿 콘솔 소프트키보드 레이아웃 겹침·하단 공백 회수** — 통합 탭(ProjectTabs) 안에
  `<iframe>` 으로 임베드된 콘솔에서, 모바일/태블릿 소프트키보드가 올라오면 콘솔 로그 박스와
  입력창(textarea)이 세로로 겹쳐 입력한 글자가 로그에 가려지고, 입력창 높이가 낮아지며 하단에
  큰 공백이 생기던 문제.
  - 원인: iframe 안에서는 `window.visualViewport.height` 가 iframe 의 실제 렌더 높이가 아니라
    "소프트키보드에 가려지지 않고 실제로 보이는 부분"을 반영한다. 콘솔이 이 값으로 자체 계산한
    `--app-viewport-height`(v1.145.0 도입)가 iframe 실제 높이보다 작아지면(태블릿 가로에서 iframe
    하단이 키보드에 가릴 때) `.layout` 이 iframe 보다 작아져 하단 공백 + 내부 flex 압축(로그·입력창
    겹침)이 발생했다. 부모 ProjectTabs(`project-tabs.js`)가 이미 iframe 크기를 키보드에 맞춰
    관리하므로 iframe 콘솔의 자체 뷰포트 계산은 중복·충돌이었다.
  - 수정: (1) `admin.css` — embed 문서(`body.pt-embed .layout`)는 자체 visualViewport 대신 iframe
    실제 높이(`height:100%`)를 그대로 채운다. (2) 콘솔 JS(`syncVisualViewport`)는 embed(`pt-embed`)
    시 `--app-viewport-height` 설정을 건너뛴다(부모가 유일 소유자). (3) `body.console-page .layout`
    의 grid row 를 `minmax(0,1fr)` 로 명시 stretch → 콘텐츠가 적어도 `.content` 가 layout 을 꽉
    채워 `console-log`(flex:1)가 남는 세로를 흡수하고 입력창/하단 바가 바닥에 붙는다.
  - Playwright 로 태블릿 가로(680×620) + iframe 뷰포트 축소 상황을 재현: 수정 전 로그·입력창
    19px 겹침 + 하단 공백, 수정 후 겹침 0 + 공백 제거 확인.
  - Android/wire 영향 없음(SSR HTML/CSS + 인라인 JS 만 변경 — `ApiPath`/DTO/`WsFrame` 무변경).

## [1.160.4] — 2026-07-11

### Fixed (v1.160.4)

- **opencode 영속 볼륨 첫 설치 시 `mkdir: Permission denied` 회수** — v1.160.2 에서 추가한
  `dev-tools/opencode:/home/vibe/.opencode` 볼륨의 호스트 디렉토리를 Docker 가 **root 소유**로
  생성하는데, `entrypoint.sh` 의 1회성 chown 목록에 `/home/vibe/.opencode` 가 빠져 있어 컨테이너
  vibe 사용자가 설치(`curl … | bash` → `/home/vibe/.opencode/bin`)하지 못했다. chown 목록에
  `/home/vibe/.opencode` 추가.
  - 운영 서버(codr-kr)는 v1.160.3 배포 후 1회 수동 chown 으로 이미 정상화됨(호스트 디렉토리
    소유권 영구 유지) — 본 수정은 **신규/재생성 볼륨**에서 자동 처리되도록 하는 완결 조치.
  - Android/wire 무영향(entrypoint·운영 전용).

## [1.160.3] — 2026-07-11

### Fixed (v1.160.3)

- **빌드환경 opencode — z.ai API key 등록/설치가 "connected 후 진행 없음 → 56초 뒤 끊김" 회수** —
  근본 원인은 **대화형 `opencode providers login` 이 헤드리스 컨테이너에서 hang** 하는 것.
  `spawnOpenCodeLogin` 이 `runStreamingCommand`(타임아웃 없는 `waitFor`)로 `opencode providers
  login` 을 spawn 하는데, 이 CLI 는 stdin/TTY 대기라 컨테이너에서 종료되지 않고 멈춘다. 그 결과
  `projectId="env-setup"` 태스크 큐 워커를 영구 점유 → 이후 **API key 등록·설치 태스크가 큐에서
  대기만 하다 실행되지 못함**(auth.json 미생성). 각 태스크 로그창은 "connected" 만 뜨고 WS idle
  타임아웃(~56s)에 닫혔다.
  - `EnvSetupService.spawnOpenCodeLogin` + SSR 라우트 `POST /env-setup/opencode-login/start` +
    카드의 "OpenCode 로그인" 버튼(`renderOpenCodeLoginAction`) 제거. 미사용 i18n 키
    (`env.action.opencodeLogin`, `env.action.opencodeLoginConfirm`) 정리.
  - z.ai coding plan 은 **API key 직접 입력(auth.json)** 이 정답이라 그 경로만 남기고 기본 노출
    (`<details open>`). 다른 provider 의 대화형 로그인은 카드의 `docker exec … opencode providers
    login` 터미널 안내로 유도(기존 유지).
  - Android/wire 영향 없음(SSR 라우트·서버 내부만 — `ApiPath`/DTO/`WsFrame` 무변경; 제거한
    `/env-setup/opencode-login/start` 는 SSR 전용 hardcoded 경로였고 `ApiPath` 에 없었다).

## [1.160.2] — 2026-07-11

### Fixed (v1.160.2)

- **빌드환경 opencode — 설치해도 "미설치"로 표시 + API key 입력 불가 회수** — 근본 원인은
  **PATH 누락**. opencode 공식 install 스크립트는 `/home/vibe/.opencode/bin` 에 바이너리를
  설치하는데, 이 경로가 서버 JVM 프로세스의 PATH 에 없어(Dockerfile 은 `.local/bin` 만 추가)
  detect 의 `opencode --version`(`EnvSetupService`)이 실패 → `SetupComponent.OPENCODE` 가
  MISSING 으로 오탐. 그 결과 카드가 "미설치"로 뜨고, API key 입력 폼은 `status == INSTALLED`
  일 때만 렌더돼(`renderOpenCodeLoginAction`) 같이 사라졌다. 바이너리·설치 자체는 정상.
  - `EnvSetupService.resolveOpenCodeCmd()` / `OpenCodeSessionManager.defaultOpenCodeCmd()` 가
    `OPENCODE_CMD` env → 설치 절대경로(`/home/vibe/.opencode/bin/opencode`, 실행 가능 시) →
    PATH(`opencode`) 순으로 resolve. detect 도 `resolveOpenCodeCmd()` 를 사용하도록 변경.
  - `docker/Dockerfile`: `ENV PATH=/home/vibe/.opencode/bin:${PATH}` 추가(터미널/spawn 통일).

### Changed (v1.160.2)

- **opencode 설치 영속 볼륨 추가** — `/home/vibe/.opencode` 가 볼륨이 아니어서 이미지
  업데이트/컨테이너 재생성 시 설치(~188MB)가 사라지던 잠재 결함 회수(opencode.sh 의 "영속"
  주석도 정정). `docker/compose.yml` 에 `dev-tools/opencode:/home/vibe/.opencode` 마운트 추가.
  auth.json 은 기존대로 `.local` 볼륨(`~/.local/share/opencode`)에 저장돼 영속.
  - ⚠️ **운영 반영 절차**: 운영 compose(`/home/wody/docker/vibe-codr-kr/compose.yml`)에 위
    볼륨을 추가하고 `docker compose up -d --force-recreate`. 재생성 시 임시 레이어의 기존
    설치가 사라지므로 빌드환경 카드에서 opencode 를 **한 번 재설치**하면 이후 영속된다.
  - Android/wire 영향 없음(SSR·Dockerfile·compose 내부 변경만 — `ApiPath`/DTO/`WsFrame` 무변경).

## [1.160.1] — 2026-07-08

### Changed (v1.160.1)

- **오버뷰 우측 rail `/compact` 버튼 — 확인 다이얼로그 제거** — 운영자 요청. 버튼을 누르면
  `window.confirm` 없이 **즉시** 콘솔로 `/compact` 를 전송한다. `project-tabs.js` 의 confirm 분기
  삭제 + `ProjectTabsTemplate` 버튼의 `data-confirm` 속성/미사용 i18n 키(`tabs.rail.compact.confirm`)
  정리. 정적 JS 변경이라 `project-tabs.js?v` (→1.160.1) + `sw.js` `CACHE_VERSION`(→v1.160.1) 동반 bump.

## [1.160.0] — 2026-07-08

### Fixed (v1.160.0)

- **디버그 빌드가 보관된 디버그 키스토어 지문을 쓰도록 — 전용 디버그 properties 도입** —
  `assembleDebug` 가 키스토어 폴더의 `<pkg>-debug.keystore` 를 안 쓰고 **릴리즈와 같은
  지문을 쓰거나 AGP 가 임의 `~/.android/debug.keystore` 를 만들던** 현상 회수. 근본 원인은
  디버그 전용 서명 config 파일이 없어 `loadSigning(debug=true)` 이 릴리즈 properties 의
  비번/alias 를 재사용(`{alias}-debug` 추측)한 것. 운영 키스토어 75개 분석 결과 디버그 alias 가
  `key0` / `key0-debug` / `androiddebugkey` / `<name>` / `<name>-debug` 등 제각각이라 추측이
  다수에서 빗나가 서명 실패/자동 debug.keystore 생성을 유발했다.
  - **신규 파일 규약 `<pkg>-debug-keystore.properties`** (SSOT) — 디버그 키스토어의 실제
    storeFile/storePassword/keyAlias/keyPassword 를 명시. `KeystoreService.create()` 가
    릴리즈 properties 와 함께 자동 생성(`keyAlias=key0-debug`).
  - **`loadSigning(debug=true)` 우선순위 개편** — ① 전용 디버그 properties 가 있으면 그 값을
    그대로 사용(추측 없음) → ② 없을 때만 레거시 fallback(릴리즈 비번 + `resolveDebugAlias`).
    `resolveDebugAlias` 는 `{base}-debug`→`{base}`→**키스토어 내 단일 key entry alias** 순으로
    개선(표준 `androiddebugkey`/`key0` 도 커버). 디버그 키스토어가 없으면 여전히 null(릴리즈
    fallback 금지, AGP 표준 debug).
  - **지문 열람(`fingerprints`)** 도 디버그 자격증명(전용 properties 우선)으로 읽어 비번/alias 가
    릴리즈와 달라도 정확히 표시.
  - `keystoreFiles`/`delete`/`renamePackage` 가 새 디버그 properties 를 아카이브/삭제/이름변경에
    포함.
  - 회귀 테스트: `KeystoreServiceDebugPropertiesTest`(4) + 기존 `KeystoreServiceLoadSigningTest`(7).
  - **운영 마이그레이션(이번 배포와 별개, 직접 수행)** — 운영 키스토어 폴더의 기존 디버그 키스토어
    73개를 `keytool -list` 로 분석해 `<pkg>-debug-keystore.properties` 를 직접 생성. **키스토어
    파일/지문은 일절 변경 없음**(생성 후 SHA-256 원본 대조로 검증). 비번 미상 2개
    (`notecloudy`, `postflow`)는 위조 금지 원칙상 건너뜀 — 운영자 수동 조치 필요.

### Notes (v1.160.0)

- **Wire change: No.** `KeystoreService` 는 서버 내부(빌드 서명 + `/settings/keystores` SSR) —
  DTO/ApiPath/WsFrame 불변. vibe-coder-android 동기 불필요.
- **글로벌 `~/.claude/CLAUDE.md`** 키스토어 규약에 `<pkg>-debug-keystore.properties` +
  "디버그 서명 = 반드시 보관된 디버그 키스토어 사용" 정책 추가.
- **후속 권고** — 업로드 흐름(`stageUploaded`/`buildKeystorePlacementPrompt`)은 아직 디버그
  properties 를 자동 생성하지 않는다(개선된 `resolveDebugAlias` fallback 으로 대부분 동작하나,
  업로드 디버그 키스토어의 비번이 릴리즈와 다르면 전용 properties 가 필요). 차기 사이클 후보.

## [1.159.1] — 2026-07-08

### Added (v1.159.1)

- **프로젝트 목록에 Google Play 스토어 링크** — 프로젝트 목록 행의 **패키지명과
  개발언어(Kotlin/Flutter) 배지 사이**에 Play 스토어 아이콘 링크를 추가. 탭하면
  새 탭에서 `https://play.google.com/store/apps/details?id=<packageName>` (해당 앱의
  Play 스토어 페이지)을 연다. 셀은 프로젝트 행 네비게이션과 겹치지 않게 project
  href 로 감싸지 않고 `target="_blank"` + `rel="noopener noreferrer"` +
  `stopPropagation`. packageName(applicationId)은 항상 존재하므로 모든 행에 표시
  (미출시 앱은 Play 스토어가 "찾을 수 없음"을 보여줌 — 링크는 항상 유효).
  - `WebProjectTemplates.projectsPage` 행 렌더 + 빈 목록 colspan 5→6 +
    i18n `projects.playstore.title`(EN/Ko).
  - **Wire change: No.** SSR 템플릿만 수정 — 라우트/DTO/ApiPath/WsFrame 불변,
    `ProjectDto` 그대로. vibe-coder-android 동기 불필요.

## [1.159.0] — 2026-07-08

### Added (v1.159.0)

- **SSH 서버 카드 — vibe 키 접속 provisioning + 접속 정보 표시** — sshd 는 이미
  `PasswordAuthentication no` + `PubkeyAuthentication yes` + `AllowUsers vibe` 로
  키 전용이지만, 카드에 **접속에 쓸 키를 등록/발급할 방법이 없고 접속 정보도 표시되지
  않아** 실제로는 접속이 불가능했다(vibe 비밀번호는 설정할 수 없음). 이번에 빌드환경
  `/env-setup` SSH 카드에서 두 가지 provisioning 경로 + 접속 명령을 제공한다.
  - **① 내 공개키 등록** — 내 SSH 공개키를 붙여넣어 `~/.ssh/authorized_keys` 에 추가.
    개인키는 서버에 저장되지 않아 가장 안전. 등록된 키는 fingerprint 목록으로 보이고
    개별 삭제 가능.
  - **② 접속용 키 발급** — 키가 없는 사용자를 위해 서버가 전용 키쌍(`access_ed25519`)을
    생성하고 공개키를 authorized_keys 에 자동 등록. 개인키를 카드에서 다운로드
    (`vibe-access`) 해 `ssh -i vibe-access -p <port> vibe@<host>` 로 접속. 재발급 시
    이전 접속 키는 자동 de-authorize.
  - **접속 정보 패널** — user=`vibe` · port · host(요청 Host 헤더 기반) + 복사용 `ssh`
    명령. 미설치/미등록 시 경고 안내.
  - fingerprint 는 OpenSSH 와 동일한 `SHA256:base64(sha256(blob))` 를 순수 JVM 으로
    계산(열람/검증에 ssh-keygen 불필요, 발급만 ssh-keygen 사용). 공개키는 알고리즘
    allowlist + wire-format 검증으로 손상/가짜 키 거부.
  - 신규: `SshAccessService`(authorized_keys 관리 + 접속 키 발급/다운로드), SSR 라우트
    `POST /env-setup/ssh-server/authorized-keys/add|remove`,
    `POST /env-setup/ssh-server/access-key/generate`,
    `GET /env-setup/ssh-server/access-key/download`. 회귀 테스트
    `SshAccessServiceTest`(7 케이스 — fingerprint OpenSSH 일치·dedupe·삭제·검증·발급).

### Changed (v1.159.0)

- **`docker/doctor/lib/ssh-server.sh` — git 클라이언트 키 자동 시드 제거** — 예전엔
  설치 시 authorized_keys 가 비어 있으면 vibe 의 git push 키(`id_ed25519.pub`)를 자동
  복사했으나, 그 개인키는 컨테이너 밖으로 나가지 않아(게다가 push 전용) 접속에 쓸 수
  없는 유령 항목이었다. 이제 `.ssh` 700 / authorized_keys 600 만 보장하고, 인증키
  등록/발급은 위 SSH 카드가 담당한다. 설치 후 authorized_keys 가 비어 있으면 카드로
  안내하는 경고를 출력.

### Wire change (v1.159.0)

- **No.** 신규 라우트는 모두 admin SSR(`/env-setup/ssh-server/*`) 로 `ApiPath` 밖이며
  Android 가 호출하지 않는다. 컴포넌트는 동적 리스트라 `ComponentStateDto` 불변 →
  DTO/WsFrame 무변경. **vibe-coder-android 동기 불필요.**

## [1.158.10] — 2026-07-08

### Fixed (v1.158.10)

- **모바일 콘솔 하단 UI 겹침 — 진짜 원인(레이아웃) 회수** — v1.158.9(캐시버스트)로도
  해결되지 않아 실제 CSS 레이아웃 버그로 재특정. 키보드가 올라온 모바일에서 입력창
  아래 컨트롤 행(빠른 프롬프트/템플릿/에이전트/도구 바)이 **세로 0 높이로 짓눌려**
  텍스트가 겹치던 문제.
  - **근본 원인**: 콘솔은 `body.console-page .content` 가 flex column 이고
    `.layout` 이 `height: var(--app-viewport-height)` + `overflow:hidden` (키보드 시
    JS 가 visualViewport 높이로 축소, v1.145.0). console-keyboard 컴팩트 규칙이 하단
    행에 `overflow-y:hidden` 을 주는데, **overflow 가 visible 이 아니면 flex 아이템의
    automatic minimum size 가 0** 이 되어 세로 공간 부족 시 짓눌리고 내용이 잘렸다.
  - **수정**: console-keyboard 하단 행(`#quick-prompts`/`.console-template-row`/
    `.console-agent-row`/`.console-bottom-bar`)에 `flex-shrink:0` 부여 → 자기 높이
    유지, 세로 압축은 `console-log-wrap`(flex:1, min-height:0)만 흡수.
  - Playwright 모바일 뷰포트로 실제 flex 구조 재현 → 수정 전 짓눌림 재현/수정 후
    정상(392px 극단 + 460px 실제 키보드 높이 양쪽) 확인.
  - admin.css 변경이므로 `?v`(AdminTemplates/EnvSetupTemplates) + sw.js
    `CACHE_VERSION` → `1.158.10` 동반 bump.

## [1.158.9] — 2026-07-07

### Fixed (v1.158.9)

- **모바일 콘솔 하단 UI 겹침(빠른 프롬프트/템플릿/에이전트/도구 바) 회수** —
  키보드가 올라온 모바일에서 입력창 아래 영역이 세로 높이 없이 겹쳐 렌더링되던
  문제. 원인은 **admin.css 캐시버스트 stale**: v1.158.0·v1.158.3에서 admin.css 를
  변경했으나 `AdminTemplates` 의 `?v` 가 `1.157.0` 에 멈춰 있었고, SW `CACHE_VERSION`
  도 `v1.137.4` 로 방치돼 모바일 브라우저가 구버전 CSS(컴팩트 레이아웃 도입 이전)를
  박제. 최신 admin.css 자체는 정상임을 정적 재현으로 확인.
  - `AdminTemplates`/`EnvSetupTemplates` 의 admin.css `?v` → `1.158.9`.
  - `sw.js` `CACHE_VERSION` → `vibe-coder-v1.158.9` (구 SW 캐시 activate 시 정리).
  - `?v` 자산은 SW fetch 핸들러가 우회(network passthrough)하고 브라우저 HTTP
    캐시(URL 기준)에 맡기므로, `?v` 값 갱신이 캐시버스트 SSOT. admin.css 변경 시
    `?v`+`CACHE_VERSION` 동반 bump 규율 재확인.
  - CSS/HTML 로직 변경 없음 — 순수 캐시버스트 정합화. 기존 세션은 재접속(또는 SW
    갱신) 시 최신 CSS 수신.
...
- **프롬프트 템플릿 + 프롬프트 입력창 송신 한도를 문서 수준으로 상향** — 긴
  스펙/문서 형태의 프롬프트를 그대로 저장·송신할 수 있도록 두 축을 함께 100K 로
  통일.
  - **프롬프트 템플릿 본문**: `PromptTemplateStore.MAX_BODY_LEN` 16,000 → 100,000자.
    `/prompts` SSR 폼(rows 10→20, `resize:vertical`)과 콘솔 ▼ 편집 모달
    (`prompt-templates.js`, rows 9→18)의 textarea `maxlength` 도 동반 상향.
  - **프롬프트 입력창(콘솔·채팅·서브에이전트)**: 서버 송신 한도
    `MAX_PROMPT_BYTES` 32KB → 100,000 byte. Claude/Codex/OpenCode/SubAgent 4개
    세션 매니저 공통. 콘솔 입력 textarea(`WebProjectTemplates`)와 서브에이전트
    콘솔(`SubAgentRoutes`) `maxlength` 32768 → 100000.
  - `maxlength`(char)는 서버 UTF-8 byte 한도와 같은 숫자로 두는 기존 컨벤션 유지 —
    서버가 byte 기준 최종 게이트(한국어 등 multi-byte 는 서버에서 더 일찍 거절).
  - lint 수정 프롬프트 자동생성(`QualityRoutes`)의 24KB 예산은 UX 상 실용 상한이라
    보수적으로 유지(주석만 최신화).
  - 기존 데이터/워크플로에 영향 없음(한도 확대는 하위호환). Android 클라이언트는
    `shared/` wire 무변경 — 다만 안드에서도 100K 규모 프롬프트를 보내려면 자체
    입력 textarea/검증 한도를 동일하게 상향해야 함(§ 아래 호환성 메모).

## [1.158.7] — 2026-07-06

### Fixed (v1.158.7)

- **GLM 세션/주간 사용량 뒤집힘 재수정** — `GlmQuotaService.parseQuota` 가
  `nextResetTime` 오름차순으로 session/weekly 를 판별했으나, 주간 윈도우가
  리셋 임박 시 nextResetTime 이 세션보다 빨라져 라벨이 뒤바뀌는 결함.
  `unit` × `number` 로 윈도우 크기(초)를 계산해 정렬하도록 변경 —
  작은 윈도우(5h) = session, 큰 윈도우(7d) = weekly 로 항상 올바르게 분류.

## [1.158.0] — 2026-07-06

### Added (v1.158.0)

- **사이드바 GLM(z.ai) 사용량 모니터링 바** — Z.AI monitor API
  (`/api/monitor/usage/quota/limit`) 를 직접 호출하여 coding plan 사용량 백분율·
  리셋 시각·총 토큰 사용량을 사이드바에 Claude/Codex 와 동일한 pill 형태로 표시.
  API key 는 opencode auth.json (`zai-coding-plan.key`) 에서 자동 읽기.
  백그라운드 120s 폴링 + 클라이언트 60s 폴링.

### Fixed (v1.158.0)

- **debug 빌드 signing alias 불일치 수정** — `KeystoreService.loadSigning(debug=true)` 가
  properties 의 `keyAlias` 를 그대로 주입하여, debug keystore 의 `{alias}-debug` alias 와
  불일치하는 결함. `sqlite-viewer` debug 빌드 7회 연속 실패의 원인.
  `resolveDebugAlias()` 가 Java KeyStore API 로 debug keystore 를 열어 `{alias}-debug`
  존재를 검증 — 있으면 사용, 없으면 원래 alias 로 fallback(`create()` 기존 키스토어 호환).
  `fingerprints()` debug 지문 조회에도 동일 적용.
  `create()` 의 debug keystore alias 를 `key0` → `key0-debug` 로 통일.

### Wire change (v1.158.0)

- `shared/ApiPath.kt` 에 `SERVER_GLM_QUOTA = "/api/server/glm-quota"` 추가.
- `shared/dto/ConsoleDtos.kt` 에 `GlmUsageDto` 추가 (신규 DTO).
- additive (하위호환). 구버전 안드 클라이언트 영향 없음. Android 에서 GLM quota 를
  쓰려면 `shared/` 동기화 필요.
...
### Wire change (v1.157.2)

- 없음. 서버 내부 빌드 signing injection 로직 수정. **Android 영향 없음.**

## [1.157.1] — 2026-07-05

### Changed (v1.157.1)

- **사이드바 에뮬레이터 슬롯 표시** — 에뮬레이터 pill 이 5개 슬롯 점을 표시.
  초록=대기(booted, lease 없음), 노랑=busy(lease 보유 또는 부팅 중), 회색=종료 상태.

### Wire change (v1.157.1)

- 없음. 기존 `/api/emulators` 응답을 사이드바에서 사용하도록 바꾼 UI 변경.
  **Android 영향 없음.**
...
- **멀티 에뮬레이터 pool** — `/emulator` 가 최대 5개 헤드리스 AVD 슬롯을 관리.
  serial 은 `emulator-5554`/`5556`/`5558`/`5560`/`5562` 로 고정하고, 시작 시
  `emulator -port <port>` 를 명시해 동시 실행 경합을 방지.
- **디바이스 다양화 프리셋** — phone / large phone / tablet / foldable / foldable-wide
  슬롯을 제공. 기본 시스템 이미지는 헤드리스 자동 테스트에 적합한
  `system-images;android-35;google_atd;x86_64` 로 전환하고, 기존 `google_apis` 이미지는
  호환 fallback 으로 인정.
- **프로젝트별 에뮬레이터 lease** — `/api/projects/{projectId}/emulator/lease` 로 프로젝트가
  에뮬레이터를 점유/해제할 수 있음. lease 는 `<workspace>/.vibecoder/emulator-leases.tsv`
  에 저장되고 4시간 TTL 로 만료.
- **provider별 모델 catalog 동적화** — Claude/Codex/GLM 모델 선택 UI 와
  `/api/projects/{projectId}/claude/console/settings` 의 `availableModels` 가 공용
  `ModelCatalogService` 를 사용. GLM(OpenCode)은 `opencode models zai-coding-plan` 결과를
  캐시하고, Claude/Codex는 CLI가 모델 목록 명령을 제공하지 않는 동안 fallback 목록에 현재
  저장/적용 모델을 합쳐 표시.

### Wire change (v1.157.0)

- `shared/ApiPath.kt` 에 server-internal endpoint 추가:
  `GET /api/emulators`, `POST /api/emulators/{id}/start`,
  `POST /api/emulators/{id}/stop`, `GET|POST|DELETE /api/projects/{projectId}/emulator/lease`.
- 기존 `GET /api/emulator/status` 는 하위호환 shape 를 유지하면서
  `runningCount`/`bootedCount`/`max` 를 additive 로 추가.
- `ConsoleSettingsDto` shape 는 변경 없음. `availableModels` 내용만 provider별 catalog 기반으로
  바뀜.
- Android 앱 기존 버전은 그대로 동작(additive). Android 에서 멀티 에뮬레이터 관리/lease 를
  쓰려면 `shared/ApiPath.kt` 동기화와 새 API UI 반영 필요.

## [1.156.3] — 2026-07-05

### Fixed (v1.156.3)

- **OpenCode(GLM) project dir 고정** — `opencode run` 실행 시 OS cwd 뿐 아니라
  CLI 의 `--dir <projectRoot>` 도 함께 전달. GLM 세션이 `postgres-client` 같은 프로젝트
  콘솔에서 현재 폴더를 컨테이너 WORKDIR(`/workspace`)로 오인하던 문제를 방지.
- **OpenCode filesystem MCP 자동 재생성 중단** — 사용자가 filesystem MCP 를 제거해도 서버가
  프로젝트 루트에 `opencode.json` 을 자동 생성하며 다시 등록하던 동작을 제거. 기존에 이미
  생성된 프로젝트별 `opencode.json` 은 사용자가 명시적으로 둔 파일일 수 있어 자동 삭제하지 않음.

### Wire change (v1.156.3)
...
- **프로젝트별 filesystem MCP 제한** — OpenCode spawn 전에 프로젝트 루트에
  `opencode.json` 자동 생성. filesystem MCP(`@modelcontextprotocol/server-filesystem`)
  마운트 경로를 `/workspace`(글로벌)에서 `<projectRoot>`로 override.
  사용자가 이미 `opencode.json`을 두면 건드리지 않음. GLM 세션이 프로젝트 디렉토리
  외부 파일에 접근하지 못하도록 격리.

### Wire change (v1.156.2)
...
- **OpenCode/Codex HOME 환경변수 강제 설정** — 컨테이너가 root 로 실행되면
  `HOME=/root` 가 이미 존재해 `putIfAbsent` 가 무시되고, spawn 되는 opencode/codex
  프로세스가 `/root/.local/share/opencode/auth.json`(없음)을 찾아 z.ai 인증 실패 →
  "Unexpected server error" 로 모든 GLM 세션이 즉시 실패하는 치명적 회귀.
  `putIfAbsent` → `put` 강제 설정으로 수정. 4개 파일:
  `OpenCodeSessionManager` / `OpenCodeAuthService` / `CodexSessionManager` / `CodexStatusService`.
  XDG_DATA_HOME 도 명시적으로 `/home/vibe/.local/share` 로 설정.

### Wire change (v1.156.1)

- 없음. **Android 영향 없음.**

## [1.156.0] — 2026-07-05

### Phase 4 — 콘솔 UX 패리티 + 사이드 이슈

Provider 3축의 오케스트레이션 패리티(Phase 0~3) 완료 후, **콘솔 메시지 표현**과
**provider별 UI 가시성** 갭을 최소화. GLM(OpenCode) 콘솔이 Claude와 동일한 도구 렌더링
품질을 갖도록 정규화 계층 추가. `ROADMAP.md §6` 갭 분석 작성.

#### Added (v1.156.0 — Phase 4 + 사이드 이슈)

- **M4.1 OpenCode 도구 정규화** — `normalizeOpenCodeToolName`(소문자→파스칼: `read`→`Read`,
  `todowrite`→`TodoWrite`, `ls`→`Glob`) + `normalizeOpenCodeToolInput`/`camelToSnakeKey`
  (`filePath`→`file_path`). `handleEvent` ToolStarted/Completed 적용. 단위 테스트 14건.
- **M4.4 image-btn provider 분기** — `WebProjectTemplates` 이미지 첨부 버튼을 Claude만 노출.
  Codex/GLM은 `images_unsupported` 에러 회피. JS 바인딩은 기존 null-safe 가드로 대응.
- **GLM 표기** — `AgentProvider.displayName` "OpenCode"→"GLM" (z.ai coding plan 강제 모드 연동).
- **effort(`--variant`) 콤보박스** — 모델 셀렉터 옆 reasoning effort(max/high/minimal),
  `.vibecoder/opencode-variant` 영속, 기본 max.
- **OpenCode maxResidentSessions /settings UI** — ConfigLoader(v1.150.0) env를 `/settings`
  페이지 + `.env.example` + `server.yml` 주석으로 노출 (Claude/Codex와 대칭 fieldset).
- **/env-setup OpenCode 카드** — `vibe-doctor opencode` 설치 + z.ai API key 입력(`auth.json` 직접 작성).
- **사이드바 사용량 pill 접기/펼치기** — 대시보드 usage 카드 토글.

#### Fixed (v1.156.0)

- **M4.3 ToolCompleted output 버그** — `handleEvent`가 `event.input`을 `ToolResult`에 전달하던 것을
  `event.output`로 정정. 콘솔에 도구 입력이 아닌 결과가 표시됨.

#### 조사 완료 (구현 연기, 정책적)

- **M4.2 thinking/reasoning** — `OpenCodeJsonParser`(Unknown 폴백)/`CodexJsonParser`(null 스킵)
  reasoning 처리 현황 조사. z.ai API 응답 형식 미확정 → 형식 확정 시 별도 구현.
- **M4.3 assistant 스트리밍** — `opencode run` 1회성 exec → 부분 text 스트리밍 불가 (CLI 한계).
- **M4.4 provider 전환 UX(HTMX)** — future work (독립적 큰 작업).

### Wire change (v1.156.0)

- 없음 (서버 내부 렌더링/UI 분기 + 버그 수정). **Android 영향 없음.**

## [1.155.0] — 2026-07

### Phase 0~3 로드맵 완결 (v1.155.0)

Provider 3축(Claude/Codex/OpenCode) 기능 패리티 + z.ai coding plan 강제 로드맵 종결.
`ROADMAP.md` §0.1 갭 표를 residual gap(provider CLI 자체 한계) 기준으로 재작성.

- **Phase 0** (v1.146.0): provider 추상화 기반 — `AgentSessionManager` turn hook + 자동화/예약 provider 무관화.
- **Phase 1** (v1.147.0~v1.149.0): Codex 기능 갭 최소화 — 자동화/예약/사용량알림, 5상태 busy + rate-limit 자동재개, 부팅 reconcile + thinking 노이즈 제거.
- **Phase 2** (v1.150.0~v1.152.0): OpenCode 1등 시민 승격 — 세션 매니저(독립 구현), status/login/quota, 콘솔 UI(모델 셀렉터/usage 카드/CSS).
- **Phase 3** (v1.153.0~v1.155.0): z.ai coding plan 강제(OPENCODE_CONFIG_HOME 격리 + 모델 whitelist) + OpenCode rate-limit 자동재개 + 종합 정리.

**residual gap (CLI 제약)**: 서브에이전트(Codex)/이미지·vision(Codex; OpenCode inline DTO)/
auto-compaction(Codex; OpenCode 는 CLI 자체)/MCP strict/백그라운드 작업 카드/토큰 자동갱신/
thinking 전용 렌더링(형식 미확정)/사용량 임계치 알림(OpenCode usage % 미노출).

### Wire change (v1.155.0)

- 없음 (문서 정리만). **Android 영향 없음.**

## [1.154.0] — 2026-07

### Added (v1.154.0 — Phase 3 Milestone 3.2: OpenCode 고급 패리티)

- **OpenCode rate-limit 자동 재개** — 프로세스 crash(stderr)에서 rate-limit 신호 감지 시 지수
  백오프(30/60/120/240/300초, 최대 5회)로 같은 sessionID 에 "이어서 진행" 프롬프트 재전송.
  `CodexSessionManager`(M1.2)/`ClaudeSessionManager`(v1.99.0) 와 동일 정책 — `fireTurnDone`
  미호출로 자동화 폭주 차단.
- **rate-limit / usage-limit 분류 순수 함수** — `isOpencodeRateLimitMessage` /
  `isOpencodeUsageLimitMessage` (Codex 패턴 차용). crash 경로에서 3-way 분기(rate-limit → 재개 /
  usage-limit → STOPPED+interrupt / 진짜 crash).
- **`startPromptLocked(isAutoResume)`** — rate-limit 자동 재개 시 rate-limit 카운터와 turn-active
  마커를 유지.

### 조사/제약 (v1.154.0)

- **auto-compaction**: opencode CLI 가 컨텍스트 관리를 자체 수행. 서버가 별도 /compact 를 보낼
  필요 없음 — step_finish 의 tokens 로 컨텍스트 사용량을 표시만(`ConsoleContextUsage`).
- **서브에이전트**: opencode 의 `task` 도구가 서브에이전트 역할. 이미 `tool_use` 이벤트로 처리됨
  (사전 조사 tool usage 에서 `task` 확인). 별도 서브에이전트 시스템 불필요.
- **사용량 임계치 이메일 알림**: opencode CLI 가 usage % 게이지를 노출하지 않아(M2.2 제약) 임계치
  transition 알림 불가. z.ai API 직접 연동(Phase 3 이후) 시 확장.
- **부팅 reconcile**: M2.1(v1.150.0) 에서 이미 구현 — `opencode-turn-active` 마커 + sessionID resume.

### Wire change (v1.154.0)

- 없음. **Android 영향 없음.**

## [1.153.0] — 2026-07

### Added (v1.153.0 — Phase 3 Milestone 3.1: z.ai coding plan 강제 모드)

- **OpenCode provider z.ai 고정** — `opencode.zai.enforceCodingPlan: true` 시 OpenCode provider
  가 항상 z.ai coding plan 구독 경로만 사용. 다른 provider(anthropic/openai/자체 LLM) 우회 차단.
  - **OPENCODE_CONFIG_HOME 격리**: `OpenCodeSessionManager` 가 spawn 시 서버 통제 디렉토리
    (`<workspace>/.opencode-zai-enforced`)를 `OPENCODE_CONFIG_HOME` 으로 설정 → z.ai-only
    `opencode.jsonc`(provider 블록 비움) 노출. 사용자 원본 config 의 커스텀 provider 무시.
  - **모델 whitelist**: `effectiveModel` 가 `zai-coding-plan/*` 외 모델을 `FALLBACK_ZAI_MODEL`
    (`zai-coding-plan/glm-5.2`) 로 대체. 프로젝트별 모델 설정도 zai 외이면 fallback.
  - auth.json credential 은 XDG_DATA_HOME 에 있어 격리 config 적용 시에도 유지.
- **부팅 audit** — `enforceCodingPlan=true` 면 서버 시작 시 로그로 명시.
- **`isZaiCodingPlanModel` 순수 함수** — 모델 문자열이 z.ai coding plan 경로인지 판정 (단위 테스트).

### Changed (v1.153.0)

- `server.yml` `opencode.zai.enforceCodingPlan` 주석 확장 — 격리 config / whitelist / env 제어 안내.

### 문서 갱신 (v1.153.0 — 운영 정책 변경, 트리거 C)

- `AGENTS.md` "AI Provider 구조" 섹션에 **z.ai coding plan 강제 모드** 정책 명시 — 격리 config /
  whitelist / 부팅 audit / token 갱신 제약.
- `docker/HUB_README.md` 에 **AI providers** 섹션 추가 (영문) — Claude/Codex/OpenCode 3 provider
  + `VIBECODER_OPENCODE_*` / `VIBECODER_ZAI_ENFORCE_CODING_PLAN` env 표.

### Wire change (v1.153.0)

- 없음 (config-only, `OpenCodeSection`/`ZaiSection` 은 v1.150.0 에 이미 추가). **Android 영향 없음.**

### 제약 (v1.153.0)

- **Settings UI runtime 토글**: config 영속화 시스템 필요로 현재 미구현. server.yml 또는
  `VIBECODER_ZAI_ENFORCE_CODING_PLAN` env 로 제어. runtime 토글은 추후 확정.
...
### Added (v1.152.0 — Phase 2 Milestone 2.3: OpenCode 콘솔 UI 완성)

- **OpenCode 모델 셀렉터** — `WebProjectTemplates` / `ProjectTabsTemplate` 의 `OPENCODE ->
  emptyList()` 를 실제 zai-coding-plan 모델 목록으로 교체(glm-5.2/5.1/5-turbo/4.7/4.5-air/
  5v-turbo). 콘솔 헤더에서 OpenCode provider 선택 시 모델 콤보가 동작.
- **대시보드 OpenCode usage 카드** — `AdminTemplates.renderOpenCodeUsageCard`. credential 상태 +
  토큰 사용량(input/output/cache read/cost). opencode CLI 가 usage % 게이지를 노출하지 않아
  % 게이지 대신 토큰 통계로 구성.
- **OpenCode provider 색상** — `ProjectTabsTemplate` CSS 를 청록(teal) 계열로 정리(claude 주황 /
  codex 파랑 / opencode 청록 3색 구분). background 추가로 가시성 향상.

### Changed (v1.152.0)

- `WebProjectTemplates` 의 OPENCODE 콘솔 헤더 분기가 "provider settings isolated" 안내 대신
  모델 셀렉터를 표시하도록 변경 (CODEX 분기와 대칭).
- `formatOpenCodeTokens` internal 헬퍼로 토큰 수 포맷(1.2K/3.4M)을 카드/summary 공유.

### Wire change (v1.152.0)

- 없음 (UI만). **Android 영향 없음** (provider 선택·모델 선택은 기존 wire 그대로).

### 검증 (v1.152.0)

- 자동화/예약: Phase 0 추상화(`AgentRouter` + `installTurnListeners`)로 OpenCode provider 에서도
  자동화(반복/순차)·TIME 예약이 동작. SESSION_RESET/WEEKLY_RESET 은 opencode 가 usage % 를
  노출하지 않아 보수적으로 보류([ScheduledPromptManager] 가 provider별 usageProviders 맵에서
  OpenCode 누락 → null 처리).
- WS console `?provider=opencode` E2E(prompt → 응답 스트림 → done)는 운영 환경 수동 검증 필요 —
  코드 경로는 Claude/Codex 와 동일한 `AgentRouter` 기반.

## [1.151.0] — 2026-07

### Added (v1.151.0 — Phase 2 Milestone 2.2: OpenCode status / login / quota)

- **`OpenCodeAuthService`** — `opencode providers list` 출력 파싱으로 credential(provider/type)
  확인. `ClaudeAuthService` 와 대칭이나, auth.json 직접 접근 대신 CLI 출력 파싱(민감정보 노출 회피).
- **`OpenCodeStatusService`** — credential 상태 + `opencode stats` 캡처(토큰/cost). `CodexStatusService`
  와 대칭이나, opencode CLI 가 usage % 게이지를 노출하지 않아 토큰 사용량/credential 상태로 구성.
  z.ai coding plan 잔여량(%)은 opencode CLI 미지원 → 추후 z.ai API 직접 연동(Phase 3.1) 시 확장.
- **`OpenCodeUsageMonitor`** — 5분 폴링으로 snapshot 캡처 + 캐시. 임계치 이메일 알림은 usage % 불가로
  비활성(`CodexUsageMonitor` 와 달리 transition 기반 warn/critical 없음).
- **login route** — `EnvSetupService.spawnOpenCodeLogin()` (`opencode providers login` spawn) +
  `POST /env-setup/opencode-login/start` + UI 카드(Codex 카드 하단, message 키 ko/en).
- **quota route** — `GET /api/server/opencode-quota` (ApiPath SSOT 먼저 등록). 인증 필요.

### Wire change (v1.151.0)

- `ApiPath.SERVER_OPENCODE_QUOTA` 상수 추가 — `GET /api/server/opencode-quota`.
- `OpenCodeUsageDto` 신규 DTO (`shared/dto/ConsoleDtos.kt`) — credential 상태 + 토큰 사용량.
- **Android 영향**: additive — 구버전 안드는 영향 없음. 안드가 opencode quota 를 쓰려면 shared/
  ApiPath + DTO 동기 필요(권고).

### Internal (v1.151.0)

- `parseOpenCodeProvidersList` / `parseOpenCodeStats` / `parseTokenCount` 순수 함수 분리.
  부동소수점 오류(`64.6 * 1_000_000`) 방지로 `roundToLong` 적용. `OpenCodeParsingTest` 로
  providers/stats/token 파싱 검증.

## [1.150.0] — 2026-07

### Added (v1.150.0 — Phase 2 사전 조사 + Milestone 2.1: OpenCode 세션 매니저 + config)

- **사전 조사 문서** — `docs/opencode-cli-reference.md` (한국어 internal). opencode 1.17.13
  동작 모델·NDJSON 이벤트 형식·config/auth 구조 정리. Codex 와의 차이 매핑.
- **`OpenCodeSessionManager` 재작성** — `UnsupportedAgentSessionManager` 상속 제거,
  `AgentSessionManager` 직접 구현 (CodexSessionManager 구조 차용, 독립 구현). `opencode run
  --format json --auto` 1회성 exec 모드. sessionID(`ses_...`) 파일로 resume (`-s <id>`).
  Phase 0 turn 관찰 hook + 부팅 reconcile(`opencode-turn-active` 마커) 지원.
- **`OpenCodeEvent` sealed + `OpenCodeJsonParser`** — opencode NDJSON 이벤트 파싱
  (step_start / text / tool_use[pending|completed] / step_finish). 다중 step 모델 —
  `step_finish` reason="stop" 이 전체 turn 완료. `reason="tool-calls"` 는 중간 step.
- **`OpenCodeSection` + `ZaiSection` config** — `model`/`configHome`/`maxResidentSessions`/
  `cmd`/`zai.enforceCodingPlan`. `server.yml` `opencode:` 섹션 + env override
  (`VIBECODER_OPENCODE_*`, `VIBECODER_ZAI_ENFORCE_CODING_PLAN`).
- **`buildOpenCodeExecArgs` + `applyOpenCodeProcessEnv`** — spawn 인자 빌더 + HOME/XDG_CONFIG_HOME/
  OPENCODE_CONFIG_HOME env 주입.
- **`ConsoleRoutes` OPENCODE 501 제거** — opencode CLI 존재 검증(`--version`)으로 전환.

### Wire change (v1.150.0)

- `server.yml` 스키마 확장(config-only). provider 선택값 "opencode" 은 이미 wire 에 존재
  (`AgentProvider.OPENCODE`). **Android 영향**: additive — provider 선택 자체는 안드 변경 없이
  동작. status DTO 는 M2.2 에서 추가 시 shared/ 동기 권고.

### Internal (v1.150.0)

- `OpenCodeJsonParserTest` — 사전 조사에서 확보한 실제 이벤트 샘플로 파싱 검증 (step/text/
  tool_use pending|completed/step_finish stop|tool-calls/exec args).

## [1.149.0] — 2026-07

### Added (v1.149.0 — Phase 1 Milestone 1.3: Codex 부가 패리티)

- **Codex 부팅 reconcile (미완 turn 자동 재개)** — `CodexSessionManager.reconcileInterruptedTurnsAsync()`
  가 서버 재시작으로 끊긴 미완 Codex turn(`codex-turn-active` 마커 잔존)을 같은 thread-id 로
  resume. `ClaudeSessionManager.reconcileInterruptedTurnsAsync` (v1.82.0) 패턴 차용 —
  `MAX_BOOT_RESUME_RETRIES=2` 초과 시 포기 + 수동 안내. `ServerMain` 부팅 시 호출.
- **turn-active 영속 마크** — `startPromptLocked` 가 새 사용자 prompt 시작 시 마커 ON,
  정상 완료/취소/에러/crash 시 OFF. rate-limit 재개 대기 중엔 유지 → 재시작 시 reconcile 잡음.

### Changed (v1.149.0)

- **`CodexJsonParser` reasoning 노이즈 제거** — `item.*` 이벤트에서 모르는 itemType
  (reasoning/thinking 포함) 을 `CodexEvent.Unknown` 폴백 대신 조용히 스킵. 이전엔 reasoning
  토큰이 원시 JSON 으로 콘솔에 뿌려져 노이즈가 됐다. 형식이 확정되면 별도
  `CodexEvent.Reasoning` + 전용 렌더링 추가 예정.
- **`CodexSessionManager` M1.2 회귀 수정** — `TurnFailed`(rate-limit 포함) 후 프로세스가
  exit 할 때 crash 분기가 종료 처리를 덮어쓰던 문제. `sawTurnEnd` 플래그로
  TurnCompleted/TurnFailed 이벤트를 받은 경우 crash 분기 스킵 (rate-limit 자동 재개/usage-limit/
  진짜에러 정책이 정확히 각각 동작).

### Wire change (v1.149.0)

- 없음. **Android 영향 없음.**

### Internal (v1.149.0)

- `startPromptLocked` 플래그 정리 — `resetRateLimitCounter` → `isAutoResume`. rate-limit 재개와
  부팅 재개 모두 rate-limit 카운터 + turn-active 마커를 유지(rate-limit/부팅 자동 재개 중엔
  미완 상태 보존). 일반 사용자 prompt 만 초기화.

### 조사 (v1.149.0)

- **이미지 지원** — Codex CLI `exec` 모드는 명령행 인자로 텍스트만 받아 vision 입력 미지원
  (CLI 제약). `sendPrompt` 의 기존 `codex_images_unsupported` 에러 유지.
- **thinking 블록 전용 렌더링** — Codex CLI 의 reasoning 토큰 출력 형식이 미확정.
  형식 확정 시 별도 `CodexEvent.Reasoning` + 전용 렌더링 추가.

## [1.148.0] — 2026-07

### Added (v1.148.0 — Phase 1 Milestone 1.2: Codex 5상태 busy + 끼어들기 + rate-limit 자동재개)

- **Codex rate-limit 자동 재개** — `CodexSessionManager` 가 `TurnFailed` 메시지를
  ① 일시 rate-limit / ② usage-limit / ③ 진짜 에러로 3-way 분기. 일시 rate-limit
  (429 / "temporarily limiting" / "overloaded" / "(not your usage limit)" 등) 시 지수
  백오프(30/60/120/240/300초, 최대 5회)로 같은 thread-id 에 "이어서 진행" 프롬프트를
  자동 재전송. `ClaudeSessionManager` v1.99.0 동일 정책 차용 — rate-limit 은 turn "종료"가
  아니라 "일시중단 → 재개 대기"라 `fireTurnDone` 을 호출하지 않아 자동화 폭주 차단.
- **`CodexSessionManager.interruptAndSend` 오버라이드** — Codex exec 는 turn 단위
  프로세스라 stdin interrupt 가 의미 없다. destroy 후 새 turn 시작이 자연스럽다는 명시적
  시스템 메시지 + rate-limit 자동 재개 취소를 우선 처리.
- **usage-limit / 진짜 에러 분기** — usage-limit(5h/weekly/plan quota)는 재시도 无효이므로
  STOPPED + `fireInterrupt("usage_limit")` (자동화 다음 프롬프트 차단). 진짜 에러는
  ERROR + `fireTurnDone("error")` 로 기존 자동화 `stopOnError` 분기 유지.

### Changed (v1.148.0)

- **`CodexSessionManager.TurnFailed` 핸들러 3-way 분기** — 기존 단일 경로(ERROR +
  `fireTurnDone("error")`)에서 rate-limit/usage-limit/진짜에러 분기로. `rateLimitRetries` /
  `retryJobs` 상태 맵 추가. `cancelTurn`/`startNew`/`interruptAndSend` 가 rate-limit
  재개 카운터·코루틴을 정리.

### Wire change (v1.148.0)

- 없음. `ConsoleBusyState` / `ProjectBusyChanged` 의 `state.wire` 필드는 이미 5상태
  (ready/responding/waiting/stopped/error) 를 전달 — Codex 는 비대화형 exec 라 `waiting` 을
  emit 하지 않으나 wire 호환은 유지. **Android 영향**: additive (busy state 필드 이미 존재).

### Internal (v1.148.0)

- `isCodexRateLimitMessage` / `isCodexUsageLimitMessage` 순수 함수로 분리 (Codex CLI 가
  구조화된 code 필드 없이 message 텍스트로만 turn 실패를 전달). `CodexRateLimitClassificationTest`
  으로 패턴 매칭 검증 (rate-limit ↔ usage-limit 오탐 분기가 재시도 정책과 정반대이므로 치명적).
- `RATE_LIMIT_RESUME_PROMPT` / `MAX_RATE_LIMIT_RETRIES` / `RATE_LIMIT_BASE_BACKOFF_MS` 상수 —
  Claude 와 동일값이나 독립 상수로 provider별 조정 허용.

## [1.147.0] — 2026-07

### Added (v1.147.0 — Phase 1 Milestone 1.1: Codex 오케스트레이션 연동)

- **`CodexStatusService` → `AgentUsageProvider` 구현** — `usageSnapshot(projectId)` 가
  캡처된 session(5h)/weekly(7d) usage % 를 반환. Codex provider 프로젝트에서도
  SESSION_RESET/WEEKLY_RESET 예약 프롬프트의 한도-해제 판정이 동작한다 (이전: Claude
  전용).
- **`ScheduledPromptManager` provider별 usage provider 맵** — 단일 `AgentUsageProvider?`
  에서 `Map<AgentProvider, AgentUsageProvider>` 로 확장. `AgentRouter.providerFor` 로
  조회한 현재 provider 의 usage 게이지로 한도 해제를 판정. Claude/Codex 각각의
  statusService 가 등록된다.
- **예약 생성 baseline 도 provider별 측정** — `PromptAutomationRoutes.createSchedule`
  이 `ClaudeStatusService` 하드 바인딩에서 provider별 `AgentUsageProvider` 로 전환.
  Codex provider 프로젝트에서 SESSION_RESET/WEEKLY_RESET 예약 생성 시 Codex usage 가
  baseline 으로 측정된다.
- **`CodexUsageMonitor` 임계치 transition 알림** — `ClaudeUsageMonitor` 패턴 차용.
  session/weekly 중 큰 값 기준으로 warn(기본 80%)/critical(기본 95%) 임계치 transition
  시 이메일·웹훅·웹푸시·Android polling 알림을 1회 발송 (같은 레벨 재발송 10분 cooldown,
  임계치 이탈 시 상태 reset).
- **`Notifiers.codexUsageWarn(...)` + `EmailNotifier`/`WebhookNotifier` 대칭 헬퍼** —
  Codex usage 임계치 알림 채널 (빌드 결과/Claude usage 와 동일 facade).
- **`CodexUsageSection` config** — `enabled`/`pollIntervalMinutes`/
  `warnThresholdPercent`/`criticalThresholdPercent`. `server.yml` `codex.usage` 섹션 +
  env override (`VIBECODER_CODEX_USAGE_*`).

### Changed (v1.147.0)

- **`CodexUsageMonitor` 폴링 주기 독립** — 이전 `config.claude.usage.pollIntervalMinutes`
  차용에서 `config.codex.usage.pollIntervalMinutes` 자체값으로. 두 provider 의 usage
  캡처 비용/주기를 독립 조정 가능.

### Wire change (v1.147.0)

- 없음. REST/WS/DTO/`ApiPath` 변경 없이 내부 provider 추상화/알림 확장만. **Android
  영향 없음.**

### Internal (v1.147.0)

- `codexUsageEffectivePercent` / `codexUsageSnapshotFromDto` /
  `codexUsageAlertTransition` / `isLimitReleasedAgainstBaseline` 순수 함수로 분리 —
  임계치 transition 판정과 한도-해제 판정을 단위 테스트로 검증 (`CodexStatusServiceTest`,
  `ScheduledPromptLimitReleasedTest`).

## [1.146.0] — 2026-07

### Added (v1.146.0 — Phase 0: Provider 추상화 기반)

- **`AgentSessionManager` 인터페이스 turn 관찰 hook 승격** — `turnDoneListener` /
  `turnInterruptListener` abstract `var` 를 인터페이스에 추가. 이전에는
  `ClaudeSessionManager` 의 `var` 프로퍼티로만 존재해 Claude 전용이었다.
- **`AgentRouter.installTurnListeners(done, interrupt)`** — 등록된 모든 manager
  (Claude/Codex/OpenCode) 에 동일 합성 리스너를 일괄 주입. `ServerMain` 이 단일
  `sessionManager.turnDoneListener = ...` setter 에서 이 팬아웃으로 전환.
- **`CodexSessionManager` turn 완료 통지** — `TurnCompleted`/`TurnFailed` 이벤트와
  프로세스 crash(exit≠0 / reader exception) 시점에 `fireTurnDone`/`fireInterrupt`
  호출. Codex provider 도 이제 자동화(autopilot)·알림 훅에 닿는다.
- **`AgentUsageProvider` 인터페이스 + `AgentUsageSnapshot`** (Phase 0-C) — provider
  무관 usage 조회 추상화. `ClaudeStatusService` 가 이를 구현해 session/weekly usage %
  를 노출.

### Changed (v1.146.0)

- **`PromptAutomationManager` provider 무관화** — 생성자 `ClaudeSessionManager` →
  `AgentRouter`. `router.sendPrompt(projectId, text)` 로 발사. Codex/OpenCode provider
  에서도 프롬프트 자동화(반복/순차)가 동작한다 (turn 완료 리스너 구현 시).
- **`ScheduledPromptManager` provider 무관화** — 생성자 `ClaudeSessionManager` →
  `AgentRouter`, `ClaudeStatusService` → `AgentUsageProvider?`. SESSION_RESET /
  WEEKLY_RESET 트리거가 usage provider 기반으로 동작. provider 가 usage 를 관측
  불가(null)면 해당 트리거는 보수적으로 보류된다.
- **`ClaudeAgentSessionManager` listener forward** — 래퍼 경유 listener 주입 시
  delegate(`ClaudeSessionManager`)에 전달되도록 get/set 위임. forward 하지 않으면
  router 팬아웃이 Claude manager 의 fire 경로에 닿지 않아 자동화/알림이 무력화된다.

### Wire change (v1.146.0)

- `ApiPath.projectAgentProvider(projectId)` 상수 추가 — `GET/POST
  /api/projects/{projectId}/agent/provider` 가 이전에 `ConsoleRoutes` hardcoded path
  로 단독 운영됐다 (v0.64.0+ SSOT 강제 룰 위반 사례). 경로 문자열은 동일 → **Android
  영향 없음**. additive. shared/ ApiPath 동기 권고(선택).

### Added

- **빌드환경 SSH 서버 설치 카드** — `/env-setup`에 "SSH 서버 (원격 접속)" 선택 카드를 추가.
  `vibe-doctor ssh-server`가 `openssh-server`를 설치하고, 카드에서 저장한 컨테이너 포트로
  `/etc/ssh/sshd_config.d/vibe-coder.conf`를 생성한 뒤 `sshd`를 재시작한다. 기본 포트는
  `2222`이며 compose가 `VIBE_SSH_PORT` → `VIBECODER_SSH_PORT`를 노출한다. "모두 설치"에는
  포함하지 않는다(외부 접속 경계가 열리는 선택 기능).

### Changed

- `docker/compose.yml`에 선택 SSH 포트 매핑(`${VIBE_SSH_PORT:-2222}:${VIBECODER_SSH_PORT:-2222}`)
  과 `VIBECODER_SSH_PORT` env를 추가하고, Dockerfile `EXPOSE`에 `2222`를 추가.

### Wire change

- 없음. SSR 빌드환경 카드/doctor/Docker 설정 변경이며 REST/DTO/`ApiPath`/`WsFrame` 변경 없음.
  **Android 리포 동기화 필요 없음**.

### Fixed

- **프롬프트 자동화·예약·deps/stats 등 pathSeg 라우트 404 (v1.145.18)** — 프롬프트 자동화
  "시작"(반복/순차)이 `automation 404: route_not_found POST /api/projects/{id}/claude/automation/start`
  로 실패하던 문제 수정. 근본 원인은 SSOT 헬퍼 `ApiPath.pathSeg()` 가 라우트 템플릿의
  placeholder 중괄호까지 URL 인코딩(`{`→`%7B`, `}`→`%7D`)한 것. 서버가
  `promptAutomationStart("{projectId}")` 로 라우트를 등록하면 경로가
  `/api/projects/%7BprojectId%7D/...` 가 되는데, Ktor `RoutingPath.parse`(3.1.x)는 세그먼트에
  **리터럴** `{`/`}` 가 있을 때만 path parameter 로 인식하고 없으면 `decodeURLPart` 후
  상수(`{projectId}`)로 등록한다. 따라서 실제 projectId(`caldo`) 요청과 영영 매칭되지 않아
  404. `pathSeg` 가 `%7B`/`%7D` 만 다시 리터럴 중괄호로 되돌리도록 수정 — 실제 식별자는
  regex(`[a-z0-9][a-z0-9._-]{0,63}` 등)로 검증돼 `{`/`}` 를 포함할 수 없으므로 URL 빌드는
  무영향. v1.59.0~v1.145.17 동안 같은 `pathSeg("{...}")` 패턴으로 등록된 모든 라우트가
  동시에 무력화돼 있었고 이번 한 줄 수정으로 일괄 복구된다: 프롬프트 자동화
  start/stop/status·프리셋, 프롬프트 예약(schedule), 빌드 자동화 스케줄(schedules/toggle),
  프로젝트 deps/stats/env-files/wrapper, 백업 자동파일 다운로드(`backupAutoFile`),
  빌드 webhook, sub-agent 콘솔(new/prompt/cancel)·WS 로그. 회귀 방지 테스트
  `ApiPathRouteTemplateTest` 추가(실제 Ktor `RoutingPath.parse` 로 Parameter 등록 검증).
  **Android 영향 없음** — 클라이언트는 URL 빌드에만 `pathSeg` 를 쓰고 실제 식별자는 중괄호를
  포함하지 않아 출력이 동일(라우트 등록은 서버 전용). wire/DTO/ApiPath 경로값 변경 없음.

- **콘솔 전송 버튼 Android Chrome 첫 탭 무시 (v1.145.17)** — 소프트키보드가 올라온
  상태에서 전송(또는 끼어들기) 버튼을 탭하면 첫 탭은 키보드만 내려가고 전송이 안 되던
  문제(두 번째 탭에야 발사) 수정. v1.145.0 의 키보드 모드(`console-keyboard`)가 입력 포커스
  중 보조 컨트롤을 접었다가 blur 시 다시 펼치는데, 버튼 탭 시 textarea blur 의 레이아웃 복원
  + 키보드 dismiss 가 동시에 reflow 를 일으켜 버튼이 손가락 아래에서 벗어나 `click`(→submit)이
  취소된 것이 원인. 액션 버튼 `pointerdown` 에서 `preventDefault` 로 포커스 이동을 막아
  키보드/레이아웃을 유지(첫 탭에 발사), 전송 후 dismiss 는 기존 `input.blur()` 가 담당.
  순수 클라이언트(인라인 JS) 수정 — wire/API 변경 없음, Android 클라이언트 영향 없음.

## [1.145.0] - 2026-06-26 — Codex CLI 설치 카드 (옵션, 로그인 영속)

### Added

- **설정 → 배포환경(`/env-setup`)에 "Codex CLI" 설치 카드** (옵션). OpenAI Codex CLI
  (`@openai/codex`)를 vibe-doctor 로 설치한다 — npm 글로벌 prefix(`/home/vibe/.local`,
  `dev-tools/npm-global` 볼륨)에 설치되어 **이미지 업데이트(재배포) 후에도 유지**된다
  (Gradle/Flutter 와 동일한 `.local` 영속 패턴). "모두 설치/업데이트"에는 포함하지 않고
  개별 카드 버튼으로만 설치 — Android 빌드 필수 도구가 아닌 선택 도구라 Flutter 와 동일 취급.
- **로그인/설정 영속** — `CODEX_HOME=/home/vibe/.config/codex` (Dockerfile ENV). `/home/vibe/.config`
  는 `dev-tools/config` 볼륨에 영속화돼 있어, `codex login` 후 **로그인 상태가 이미지
  재배포 / 컨테이너 재생성에도 유지**된다 (별도 compose 볼륨 추가 불필요 — Git global config 와
  동일 방식).
- 구성 파일: `docker/doctor/lib/codex.sh`(`install_codex`) + vibe-doctor `codex` 서브커맨드 +
  `SetupComponent.CODEX`(EnvSetupService) + 카드 렌더(EnvSetupTemplates) + i18n(EN/KO).
  콘솔/터미널에서 `vibe-doctor codex`(설치) → `codex login`(인증) 순.

### Changed

- `docker/Dockerfile` — `ENV CODEX_HOME=/home/vibe/.config/codex` 추가(기존 `.config` 볼륨 재사용,
  새 마운트 없음).

### Wire change

- 없음. EnvSetup 컴포넌트는 동적 리스트(`ComponentStateDto` 구조 불변, codex 는 enum 항목 추가)라
  REST/DTO/`ApiPath`/`WsFrame` 무변경 → **vibe-coder-android 영향 없음**.

## [1.144.8] - 2026-06-26 — 터미널 세션 복원 원천 봉쇄 해제 (REST 직렬화 500 수정)

### Fixed

- **`GET`/`POST /api/terminal/sessions` 가 항상 500 이던 치명 결함 — "다른 페이지 다녀오면
  터미널이 완전 새로 초기화"의 진짜 원인.** 핸들러가 `mapOf("sessionId" to String, … ,
  "alive" to Boolean)` 처럼 **값 타입이 섞인 Map** 을 그대로 `respond` 했는데,
  kotlinx.serialization 은 `Map<String, Any>` 를 직렬화하지 못한다("Serializing collections
  of different element types is not yet supported"). 그래서 세션 목록 조회가 **무조건 500**
  이었고, 클라이언트 init 의 `fetch(...).then(r => r.ok ? … : {sessions:[]})` 가 매번 빈
  목록을 받아 **항상 `newSession()`** 을 호출 → 기존 세션 복원이 원천적으로 불가능했다.
  idle reaper(v1.144.7)·환경(모바일/데스크탑)과 무관하게 모든 사용자에게 재현됐다.
- **수정** — `@Serializable` DTO(`TerminalSessionView`/`…ListResponse`/`…Created`/`…Limit`)
  로 교체. 응답 JSON 형태(`{sessions:[{sessionId,workdir,createdAt,alive}]}`,
  `{sessionId,workdir}`, `{error,max}`)는 동일 — 클라이언트 무수정. 세션 한도 초과 응답도
  이제 의도대로 `429`(이전엔 같은 직렬화 실패로 500)로 나간다.
- **다른 PC/기기에서 세션 이어짐** — 세션 owner 는 device 가 아니라 **user(admin)** 기준
  (`list(ownerUserId)`)이고, 단일 admin 이라 모든 device 의 `user_id` 가 동일하다. 따라서
  위 수정만으로 어느 기기/브라우저로 로그인하든 같은 터미널 세션 목록이 복원된다.

### Wire change

- 없음. REST 응답의 JSON 구조는 그대로(타입 섞인 Map → 동일 형태 DTO). `ApiPath`·`WsFrame`
  무변경 → **vibe-coder-android 영향 없음**(터미널은 현재 웹 전용 UI).

## [1.144.7] - 2026-06-24 — 터미널 세션 유지 (다른 화면 다녀와도 안 날아감)

### Fixed

- **다른 화면(메뉴)으로 갔다가 터미널로 돌아오면 세션이 "완전 새 터미널"로 초기화되던 문제.**
  cd 했던 위치·실행 중이던 프로세스가 모두 사라져 이어서 작업할 수 없었다. 터미널 탭을 여러 개
  열어둬도 마찬가지였다.
- **근본 원인 — idle reaper 30분 하드코딩.** PTY bash 는 서버에 상주하고 재방문 시 scrollback
  replay 로 복원되도록 v1.34.0 에 설계됐지만, idle reaper 가 **30분** 무활성 세션을 강제 종료했다.
  vibe-coder 를 주로 Claude 콘솔로 쓰다 보니 터미널을 떠나 있는 시간이 30분을 넘기 일쑤라(운영
  로그상 `reaping (idle 30m)` 빈발), 돌아오면 세션이 이미 죽어 새 bash 가 떴다. WS 가 끊긴
  동안엔 입력·출력이 없어 idle 시계만 흘렀다.
- **수정.**
  - **활성 WS 연결 보호** — 세션에 WS 연결 카운터(`attach`/`detach`)를 두어, 보고 있는(WS 연결된)
    터미널은 idle 과 무관하게 절대 회수하지 않는다.
  - **idle 유예 30분 → 설정값(기본 24h)** — `security.terminalIdleTimeoutMinutes` (0 = 무제한).
    WS 가 끊긴 세션만 이 시간 초과 시 정리. 다른 작업을 한참 하고 돌아와도 세션이 살아 있다.
  - **WS 자동 재연결** — 페이지는 살아 있는데 WS 만 끊긴 경우(모바일 백그라운드·네트워크 변동·
    프록시 idle)를 지수 백오프(1s→최대 10s)로 자동 재연결하고, 화면 복귀(`visibilitychange`)/
    네트워크 복구(`online`) 시 즉시 재연결한다. 재연결 시 scrollback replay 로 직전 화면이
    복원된다. 서버가 정상 종료(세션 없음/프로세스 exit, close code 1000)로 닫은 경우엔
    재연결하지 않는다.

### Added

- **`security.terminalIdleTimeoutMinutes` 설정** (server.yml, 기본 1440 = 24h, 0 = 무제한).
  터미널 PTY 세션이 WS 끊긴 뒤 정리되기까지의 유예. 활성 WS 연결 세션은 항상 보호.

### Wire change

- 없음. `WsFrame`(TerminalOutput/Input/Resize/Exit)·`ApiPath`·REST 응답(`GET
  /api/terminal/sessions`) 모두 그대로. 서버 내부 lifecycle 정책 + 클라이언트 정적 자산(SSR
  인라인 JS) 변경 → **vibe-coder-android 영향 없음**(터미널은 현재 웹 전용 UI).

## [1.144.6] - 2026-06-22 — 음성 입력 에코잉/중복 누적 수정 (interim prefix collapse)

### Fixed

- **음성 타이핑(콘솔 🎤 Web Speech API) 결과가 중복 누적되던 결함.** "안녕하세요. 음성녹음
  테스트 중입니다." 라고 말하면 textarea 에 `안녕하세요안녕하세요안녕하세요 음성안녕하세요…`
  처럼 같은 조각이 겹쳐 쌓였다.
- **근본 원인.** ko-KR / 모바일 Chrome 음성 엔진은 **같은 발화 하나에 대해 누적 prefix 를 담은
  여러 개의 result item**(`"안녕하세요"` / `"안녕하세요"` / `"안녕하세요 음성"`)을 동시에 들고
  있는데, v1.94.0 의 전체 재구성 로직이 interim 결과를 전부 `interim += r[0].transcript` 로
  이어붙여 그 누적 prefix 들이 그대로 중복돼 보였다. (v1.94.0 은 final 의 인덱스 리셋 재전송
  중복만 해결했고 interim 의 다중 prefix 중복은 남아 있었다.)
- **수정** — `collapsePrefix()` 도입. 인접한 result 조각이 prefix 관계면(한쪽이 다른 쪽으로
  시작) 더 긴 쪽으로 흡수해 하나로 합치고, 서로 다른 발화일 때만 이어붙인다. final·interim 을
  각각 정규화하고, 화면 표시값은 final+interim 통합 배열에 collapse 를 적용해 final/interim 경계의
  중복(직전 final 을 interim 이 다시 포함하는 경우)까지 제거한다. 정상적인 여러 발화 누적과
  v1.94.0 이 잡던 final 재전송 중복은 그대로 유지된다.
- **캐시.** `voice-input.js` 는 그동안 `?v` 캐시버스트 없이 로드돼 SW(`sw.js`)가 cache-first 로
  구버전을 박제할 수 있었다. 로드 태그를 `?v=1.144.6` 으로 바꿔(다른 정적 자산과 동일 패턴)
  SW 우회 + 브라우저 HTTP 캐시 무효화를 보장한다.

### Wire change

- 없음. 순수 클라이언트 정적 자산(`voice-input.js`) + SSR 자산 버전 쿼리 변경. REST/WS/DTO/
  `ApiPath` 무관 → **vibe-coder-android 영향 없음**(음성 입력은 웹 전용 기능).

## [1.144.5] - 2026-06-19 — 디버그 빌드에 릴리즈 서명키가 주입되던 회귀 수정

### Fixed

- **디버그 빌드(`assembleDebug`)인데도 릴리즈 서명키(`<pkg>.keystore`)가 주입되던 결함.**
  빌드 버튼으로 디버그 APK 를 만들면 릴리즈 키스토어로 서명돼, 디버그/릴리즈 빌드가 같은
  서명을 갖는 문제가 있었다(운영자 규약 "디버그/릴리즈 분리 v1.5.0+" 위배).
- **근본 원인.** `BuildService.resolveSigning` 이 빌드 variant 와 무관하게 항상
  `KeystoreService.loadSigning(packageName)` 을 호출했고, 이 함수는 `<pkg>-keystore.properties`
  의 `storeFile`(=릴리즈 `<pkg>.keystore` 경로로 하드코딩)을 그대로 반환했다. 그 결과 DEBUG
  variant 빌드에도 릴리즈 자격이 `GradleBuilder` 로 전달돼 `-Pandroid.injected.signing.*` 4종이
  주입됐다. AGP 의 injected signing 은 build.gradle.kts 설정과 무관하게 **빌드되는
  variant(debug task 포함)에 서명을 그대로 override** 하므로 디버그 APK 가 릴리즈 키로 서명됐다.
  (`GradleBuilder` 주석의 "debug 는 signingConfigs.debug 가 필요" 설명이 잘못된 전제였다.)
- **수정** — `KeystoreService.loadSigning(packageName, debug)` 에 `debug` 분기를 추가했다.
  `debug=true` 면 `<pkg>-debug.keystore` 를 강제 사용(properties 의 릴리즈 경로 무시), 비밀번호/
  alias 는 release 와 공유한다(`create()` 가 양쪽을 동일 정보로 생성). `BuildService.resolveSigning`
  은 variant 를 받아 DEBUG → debug 키스토어, RELEASE/BUNDLE → release 키스토어를 선택한다.
  디버그 키스토어가 없는 프로젝트(과거 release 만 업로드 등)는 null → 미주입(AGP 표준 debug 서명).
- **효과.** 디버그 빌드는 고정된 `<pkg>-debug.keystore` 로 서명돼 디버그 SHA 가 일관되게 유지된다
  (Firebase/Google Sign-In 디버그 등록이 빌드 머신과 무관하게 동작). `fingerprints()` 가 이미
  노출하던 debug 키스토어 SHA 와도 정합. 릴리즈/번들 빌드는 기존과 동일(`<pkg>.keystore`).
- 기존 프로젝트 영향: 키스토어/소스 변경 없음. 다음 디버그 빌드부터 디버그 키로 서명될 뿐이며,
  릴리즈 빌드 산출물의 서명은 변하지 않는다.

## [1.144.4] - 2026-06-18 — 일괄 전송 시 응답중 세션 무한 증가 → OOM/PC 셧다운 수정 (게이트 우회 차단)

### Fixed

- **일괄 프롬프트 전송 후 "응답중" 프로젝트 수가 동시 한도(`maxConcurrentTurns`, 기본 3)를
  넘어 계속 늘어나다 결국 메모리 고갈로 호스트 PC 가 셧다운되던 치명적 결함** — 운영 점검
  결과 PostgreSQL 이 정상 종료 없이 강제 중단된 기록(`database system was not properly shut
  down`, 오전 09:07 KST)으로 OOM 셧다운이 확인됐다.
- **근본 원인 — "permit 없는 유령 turn".** claude 는 백그라운드 작업(`Bash run_in_background`/
  `Task`) 완료 후 host stdin 없이 **자발적으로 turn 을 재개**할 수 있다. 이때
  `handleStdoutLine` 의 활동 프레임(assistant/tool) 분기가 `busy=false → RESPONDING` 으로
  전이시키는데, 이 경로는 `sendPrompt` 를 거치지 않아 **게이트 permit 을 확보하지 않았다.**
  `busy=false` 에서 자발적 재개가 발생하는 경우는 ① bg task 추적 실패(파서가 `task_started`
  를 못 잡아 `outstandingBgTasks` 가 비어 Done 이 정상 완료로 처리 → `release`) ② watchdog
  조기 종료(v1.144.3 `finalizeStalledBgTurn` 이 `release`+`READY`) 직후 지연 재개다. 이렇게
  태어난 유령 turn 은 `busy=true` 인데 게이트 장부(`heldKeys`)엔 없어 **두 안전장치를 동시에
  우회**했다: ① 게이트 `inFlight` 에 안 잡혀 "슬롯이 비었다"고 오판 → 일괄 전송/자동화가 새
  세션을 계속 spawn(응답중 한도 무력화), ② 상주 캡(`enforceResidentCap`)의 victim 필터가
  `!isBusy(...)` 라 `busy=true` 유령 turn 은 **회수 대상에서 영구 제외** → claude+MCP 사이드카
  트리(~900MB)가 무한 누적 → OOM → PC 셧다운.
- **수정** — `ClaudeConcurrencyGate.adopt(key)` 추가. 이미 진행 중인 자발적 재개 turn 을
  **비대기(suspend 안 함)** 로 게이트 장부에 흡수한다: `heldKeys` 에 등록해 `inFlight` 카운트에
  포함시키고, permit 은 가능하면 확보, 없으면 over-subscribe 를 허용하되 `pendingReduce++` 로
  적어 해당 turn 의 `release` 가 permit 을 풀에 잘못 반환하지 않게 한다(회계 정합성 유지).
  `handleStdoutLine` 의 자발적 재개 분기(`busy != true` 첫 활동 프레임)에서 `markTurnActive`
  와 함께 `gate.adopt(projectId)` 를 호출한다. `adopt` 는 `heldKeys` 기반이라 `sendPrompt` 의
  `acquire` 와 race 해도 중복 등록되지 않는다. 이로써 유령 turn 도 게이트에 잡혀 추가 spawn 이
  억제되고, 상주 캡도 정상 동작한다.
- **알려진 잔여** — `SubAgentSessionManager` 는 메인 콘솔과 달리 busy/자발적 재개 추적 로직이
  없어 같은 흡수 지점이 없다. 단 sub-agent 는 일괄 전송(broadcast) 대상이 아니고
  `BACKGROUND_TASK_GUIDE` 도 주입하지 않아 이번 사건 경로와 무관하다. 필요 시 별도 사이클에서
  sub-agent busy 추적 도입을 검토한다.

### Changed

- **상주 세션 캡 기본값 6 → 3 (`maxConcurrentTurns` 와 정렬).** vibe-coder 는 비인터랙티브
  (프롬프트 송신 → 비동기 응답)라, turn 이 끝난 세션을 warm 으로 길게 살려둘 실익이 적다 —
  다음 프롬프트는 `--resume` 으로 동일하게 재개된다. 상주 캡을 게이트 한도와 같게 좁히면
  **살아있는 세션 = 응답중 세션**으로 수렴해, "응답중 수 이상으로 프로젝트가 활성화되지
  않는다"는 직관과 메모리 상한(응답중 수 × ~900MB)을 함께 보장한다. 대기 중 프롬프트는
  v1.135.0(gate-before-spawn) 이후 이미 **프로세스를 띄우지 않고** 큐(코루틴 suspend)에서
  FIFO 로 대기하므로 대기로 인한 메모리 점유는 없다. 인터랙티브 연속 작업의 cold start(프로
  세스 + MCP 사이드카 재기동)가 잦으면 `/settings` 에서 상주 캡을 키우면 된다.
- **운영 반영 주의** — 상주 캡은 런타임에 `ConfigHolder.current.claude.maxResidentSessions`
  를 읽는다. 운영 인스턴스가 `/settings` 에서 값을 저장한 적이 있으면 그 저장값(기존 6)이
  코드 기본값보다 우선한다. 배포 후 **`/settings` 에서 상주 세션 수를 3 으로 저장**하거나,
  저장 이력이 없다면 새 기본값(3)이 그대로 적용된다.

### Wire change

- 없음 — 내부 동시성 회계 로직 + 설정 기본값만 변경(`ClaudeConcurrencyGate` 신규 `adopt`
  메서드는 서버 내부 API). 새 REST/WS 엔드포인트·DTO·`WsFrame`·`ApiPath` 추가 없음. **구버전
  Android 클라이언트도 그대로 동작**한다 — 안드 측 동기 불필요.

## [1.144.3] - 2026-06-16 — 백그라운드 작업 종료 후 "대기중" 영구 고착 수정 (watchdog)

### Fixed

- **백그라운드 작업이 끝나고 turn 도 정상 종료됐는데 콘솔/프로젝트 목록 상태칩이 "대기중"
  (WAITING)에 고착되던 문제** — claude 가 turn 중 background task(`Bash run_in_background`
  또는 `Task` 서브에이전트)를 띄우면 서버는 `task_started` 이벤트로 `outstandingBgTasks`
  집합에 taskId 를 추가하고(v1.99.2), turn 종료(Done) 시 이 집합이 비어있지 않으면 "곧 자동
  재개될 것"으로 보고 `WAITING` 으로 전이하며 permit·미완 마크를 유지한다. 그런데 taskId 제거
  경로는 terminal lifecycle 이벤트(`task_notification` 또는 `task_updated` 의 terminal status)
  뿐인데, **stream-json 모드에선 Claude CLI 가 (1) bg task 완료 시 자동 재개되지 않고(호스트가
  stdin 제어) (2) 완료 통지마저 누락**할 수 있다. 그 결과 claude 가 작업을 정상 완료하고
  `Done(success)` 를 보냈는데도 stale taskId 가 집합에 영구히 남아, 재개 turn 도 오지 않으니
  `WAITING` 에서 빠져나올 수 없었다(`idle reap` 기본 30분 전까지 고착, 이후 STOPPED). 운영
  점검 결과 mindraw 외 13개 프로젝트에서 반복 발생한 구조적 문제였다.
- **수정** — `WAITING` 진입 시 **bg watchdog**(`armBgWatchdog`)를 무장한다. 마지막 bg 진행
  이벤트(`task_progress`/`task_updated` 등) 이후 `BG_SUSPEND_WATCHDOG_MS`(3분) 동안 추가 진행/
  재개 활동이 없으면 `finalizeStalledBgTurn` 이 turn 을 정상 종료한다 — stale `outstandingBgTasks`
  정리, permit 반환, 미완 마크 해제, `READY` 전이, 자동화 완료 통지(`fireTurnDone`). bg 진행
  이벤트가 올 때마다 watchdog 가 리셋(연장)되므로 **진짜 진행 중인 작업은 죽이지 않고** "완료
  통지 누락 후 무활동" 케이스만 회수한다. 활동 프레임 재개(RESPONDING)·정상 종료·중단·세션
  종료 시엔 `setBusy`/`terminateSession` 에서 watchdog 가 해제된다. 콘솔에는 `bg_task_watchdog`
  시스템 메시지로 종료 사실을 알린다.

### Wire change

- 없음 — 내부 상태 전이 로직만 변경. 새 REST/WS 엔드포인트·DTO·`WsFrame`·`ApiPath` 추가 없음.
  watchdog 종료는 기존 `ConsoleBusyState`/`ProjectBusyChanged`(READY) + 기존 system 프레임
  (code=`bg_task_watchdog`)으로만 노출되므로 **구버전 Android 클라이언트도 그대로 동작**한다
  (신규 code 문자열은 `ignoreUnknownKeys`/그대로 표시). 안드 측 동기 불필요.

## [1.144.2] - 2026-06-15 — rate limit 메시지를 사용량 한도로 오판하던 문제 수정

### Fixed

- **일시 rate limit 인데 "사용량 한도 도달"로 잘못 표시되고 자동 재시도가 안 되던 문제** —
  Anthropic 의 rate-limit 안내 원문(`API Error: Server is temporarily limiting requests
  (not your usage limit) · Rate limited`)이 **"not your usage limit"** 이라는 부정 문구를
  포함하는데, `ClaudeStreamParser.isUsageLimitMessage` 의 `"usage" + "limit"` 단순 부분문자열
  매칭이 이를 사용량 한도로 오분류했다. 그 결과 파서가 `code="usage_limit"` 으로 정규화하고
  원문 메시지까지 한국어 안내문으로 덮어써, 정작 정확한 `ClaudeSessionManager.isRateLimitError`
  (`"temporarily limiting"` 감지)가 실행될 땐 단서가 사라져 자동 재시도 경로로 못 가고
  STOPPED + "⛔ 사용량 한도에 도달..." 빨간 버블이 떴다. `isUsageLimitMessage` 에 rate-limit
  배제 가드(`temporarily limiting`/`rate limit`/`429`/`overloaded`/`not your usage limit`)를
  선행 추가 — 매칭 시 즉시 false 반환해 원문(code/message)을 보존한다. 두 분류기가 상호배타가
  되며 rate-limit 이 우선(= 자동 재시도, 복구 가능 경로)한다. 실제 원문을 그대로 고정한 회귀
  테스트 추가(`ClaudeStreamParserResultTest`).

## [1.144.1] - 2026-06-13 — 프로젝트 목록 앱 아이콘 stale 캐시 수정

### Fixed

- **앱 아이콘 변경 후에도 프로젝트 목록이 예전 아이콘을 보여주던 문제** — `/projects/{id}/app-icon`
  응답이 `Cache-Control: private, max-age=3600` 이라, 브라우저가 1시간 동안 **재검증 없이**
  캐시본을 사용했다. 서버측 `AppIconCache` 는 원본 (경로/mtime/size) 변경을 감지해 ETag 까지
  갱신했지만, max-age 동안 브라우저가 아예 서버에 묻지 않아 stale 이 남았다. `private, no-cache`
  (저장하되 매 사용 전 `If-None-Match` 재검증)로 변경 — 변경 없으면 304(64px 라 본문도 수 KB),
  아이콘이 바뀌면 즉시 200 신규본. 코드베이스의 `ProjectAssetsRoutes` 와 동일 패턴.

## [1.144.0] - 2026-06-13 — 콘솔 메시지 절단 폐기(전문 표시) + raw JSON 노출 근본 수정

### Fixed

- **큰 tool_use(Write/Edit 등)가 콘솔에서 raw JSON 으로 노출되던 근본 원인 제거** —
  이력 복원용 인라인/더보기 JSON 생성기 `renderInitialHistoryJson` 가 각 행 내용을
  4000자 raw substring 으로 잘랐는데, tool_use 입력 JSON(예: 큰 `content` 의 Write)이
  문자열 중간에서 잘려 JSON 이 깨지면 클라 `tryParseJson` 실패 → `renderToolUse` 가
  깨진 raw JSON 을 본문에 그대로 노출(`{"file_path":…,"content":"…"} …(+N)`)했다.
  데이터(파서·DB·라이브 WS·history JSON API)는 항상 전문이었고 표시 계층만의 절단.

### Changed

- **콘솔 전문 표시(절단 폐기)** — 운영자 요청. (1) `renderInitialHistoryJson` 의 4000자
  절단 제거 → 인라인 초기 이력 + "더보기"/무한 스크롤 응답 모두 전문. `jsLit` 이
  `<`/`>`/`&`/제어문자/surrogate 를 escape 하므로 `</script>` 탈출·UTF-8
  MalformedInput·500 위험 없음. (2) 라이브/이력 tool_result 표시의 `clip(out, 8000)`
  (`…(+N)` 마커) 제거 → 전문 표시. 화면 점유는 모든 메시지 공통 `MD_CLAMP_PX` 높이
  접기(탭 펼침)로 제어하고 복사용 원문(`opts.raw`)은 그대로 보존. 빈 thinking
  signature 경량 마커 + tool_result base64 이미지 스트립(서빙 endpoint 위임)은 전문이
  아닌 노이즈/바이너리라 기존대로 유지.

### Wire change

- No. 웹 SSR/inline JS 전용. 안드로이드는 별도 `/api/projects/{id}/history`
  (`HistoryTurnDto.content` = 절단 없는 full)를 사용하므로 영향 없음.

## [1.143.0] - 2026-06-13 — 웹 콘솔 user 프롬프트 이미지 [image] 라벨 + 탭 펼침

### Changed

- **웹 콘솔 사용자 프롬프트 이미지를 기본 `[image]` 라벨로 접고 탭 시 썸네일 펼침** —
  종전엔 user 프롬프트 첨부 이미지가 항상 보이는 썸네일(`<img class="log-img">`)이라,
  특히 상단고정(`.cur`, v1.104.1) 최신 프롬프트가 썸네일로 콘솔 상단을 계속 차지했다.
  이제 user 행은 `[image]`(다중이면 `[image] ×N`) 클릭형 라벨(`.log-img-toggle`)로 접혀
  표시되고 탭하면 썸네일(`.log-images.collapsed` 토글)을 펼친다. tool 결과(스크린샷 등)
  비-user 이미지는 기존처럼 즉시 썸네일 표시. live WS / 이력 복원 / 상단고정 모두 동일
  `append` 경로라 일괄 적용. 안드로이드 앱(v0.71.0)은 이미 동일 동작이며 서버 측 web
  console 만 누락돼 있던 것을 회수. SSR inline JS 변경이라 별도 캐시버스트 불필요.

## [1.142.0] - 2026-06-13 — 프롬프트 템플릿 관리 다이얼로그 우측 rail 가림 방지

### Fixed

- **프롬프트 템플릿 관리 다이얼로그가 우측 overview rail 뒤로 넘어가던 문제** — 통합 탭
  콘솔은 풀폭 iframe 이고 rail 은 부모 레이어(z-index)라, iframe 안 `<dialog showModal>`
  이 뷰포트 중앙(94vw)에 떠서 우측 ~330px(rail 폭)가 가렸다. `prompt-templates.js` 가
  부모(project-tabs.js)가 inner page `.content` 에 주입한 `padding-right`(=rail 폭+24)를
  읽어 다이얼로그를 가시 영역(뷰포트−rail 폭) 안으로 한정·중앙정렬(`applyRailGutter`).
  열 때 + 창 resize 시 재계산. 단독 콘솔 페이지/rail 숨김(padding≤40) 시엔 gutter=0 →
  기존 중앙정렬 유지. 캐시버스트 `?v=1.142.0`.

## [1.141.0] - 2026-06-13 — 콘솔 무한 스크롤(최상단 도달 시 과거 자동 로드)

### Changed

- **콘솔/채팅 과거 내역 자동 로드** — 초기 30개 표시는 유지하되, 스크롤을 위로 올려
  최상단 근처(≤80px)에 닿으면 과거 30개를 자동으로 불러와 prepend(`loadMoreHistory`).
  기존 "↑ 더보기" 버튼은 수동 폴백 겸 시각 힌트로 유지. prepend 후 scrollTop 보정으로
  보던 위치 유지 → 무한 재트리거 없음. `before=<oldestTurnIdx>&limit=30` 페이지네이션
  재사용(서버 변경 없음).

## [1.140.0] - 2026-06-13 — Claude Fable 5 모델 선택 추가

### Added

- **콘솔 모델 콤보박스에 "Fable 5" 추가** — Claude Code CLI 2.1.17x 의 `fable` alias
  (`claude-fable-5`, Anthropic 최상위 모델, Opus 대비 2배 비용) 를 프로젝트별 모델로
  선택 가능. SSR 콘솔 콤보 + JSON `ConsoleSettingsDto.availableModels` 양쪽 반영
  (안드 앱은 목록을 동적 렌더링하므로 자동 노출). 전역 기본(`claude.model`) /
  env `VIBECODER_CLAUDE_MODEL` 로도 `fable` 지정 가능. 컨텍스트 미터의 윈도우
  추정은 기존 로직(haiku 외 1M)이 Fable 5(1M)에 그대로 적용.

### Wire change

- No. (`availableModels` 목록에 항목 추가만 — DTO 구조 동일, additive.)

## [1.139.0] - 2026-06-12 — 끼어들기 ⚡ 상시 노출 + 동시 한도 무시 강제 전송

### Changed

- **끼어들기 버튼 상시 노출** — 종전엔 응답 중일 때만 표시. 이제 콘솔 입력창 우측에
  항상 노출되며 의미가 "지금 당장 보내기"로 통일: 응답 중이면 현재 turn 을 interrupt
  후 즉시 전송, **동시 작업 한도(`maxConcurrentTurns`)가 가득 차 있어도 대기열을
  무시하고 강제 전송**.
- `ClaudeSessionManager.sendPrompt` 에 `bypassGate` 파라미터(내부) —
  `interruptAndSend` 가 게이트 무시로 전송. permit 미보유라 Done 의 release 는
  idempotent no-op (게이트 회계 영향 없음). 한도 무시는 운영자 명시 액션(⚡)에만 적용.

### Wire change

- No. (interrupt 엔드포인트/DTO 동일 — 서버 동작 + SSR 버튼 노출 변경만.)

## [1.138.0] - 2026-06-12 — HistoryTurnDto.imageCount (안드 클라이언트 이미지 마커용)

### Added

- **`HistoryTurnDto.imageCount`** — user turn 의 첨부 이미지 수(raw 의 image 블록 파싱,
  user 외 role 은 0). 클라이언트가 프롬프트에 `[image]` 마커를 표시하고 탭 시
  `GET /api/projects/{id}/claude/console/image?turn=<turnIdx>&idx=<i>` 로 바이트를
  로드하는 용도. additive default(0) — 기존 클라이언트 영향 없음.
- **`ApiPath.claudeConsoleImage`** — v1.133.0 에 hardcoded 로 추가됐던 이미지 서빙
  endpoint 의 SSOT 등록 회수 (§8.A).

### Wire change

- **Yes.** `HistoryTurnDto.imageCount`(additive) + `ApiPath.claudeConsoleImage`.
  **vibe-coder-android shared/ 동기 필요** (같은 cycle 의 안드 v0.71.0 에서 즉시 반영).

## [1.137.4] - 2026-06-12 — 프롬프트 템플릿 콤보 카테고리 헤더 중복 누적 수정

### Fixed

- **템플릿 드롭다운에 카테고리가 여러 번 반복 표시** — `renderPicker` 의 비우기 루프가
  `select.options`(`<option>` 만 포함) 기준 `remove(1)` 이라 **빈 `<optgroup>`(카테고리
  라벨)이 DOM 에 잔류**, 재렌더(템플릿 관리 다이얼로그 열기/닫기 등)마다 카테고리 헤더가
  한 벌씩 누적되던 버그. 모바일 네이티브 select 시트에서 General/★즐겨찾기/... 가 4번씩
  반복된 후 항목이 보였다. `children` 기준으로 placeholder 외 전부 제거하도록 수정.
  `prompt-templates.js?v=1.137.4` + sw `CACHE_VERSION` bump.

### Wire change

- No.

## [1.137.3] - 2026-06-12 — 프로젝트 목록 아이콘 리사이즈 + 캐싱

### Fixed

- **프로젝트 목록 런처 아이콘 전송 최적화** — `/projects/{id}/app-icon` 이 원본
  (최대 density `ic_launcher`, 루트 `icon.png` 폴백 시 업로드 원본 수 MB 가능)을
  매 요청 그대로 보내고 브라우저 캐시도 60초뿐이라, 목록(수십 행 × 30px `<img>`)을
  열 때마다 큰 PNG 들을 반복 전송하던 문제.
  - **64px 다운스케일**(표시 30px 의 2× — HiDPI) PNG 를 `AppIconCache` 가
    **in-memory 캐시**(프로젝트당 수 KB). 원본의 경로/mtime/size 변경 시 자동 재생성.
  - **ETag + `max-age=3600`** — 재방문은 304 재검증(본문 0바이트)으로 처리.
  - WebP 등 ImageIO 미지원 포맷·디코드 실패 시 원본 폴백(기능 회귀 없음).
  - 회귀 테스트 `AppIconCacheTest` (축소·캐시 재사용·mtime 재생성·소형 통과).

### Wire change

- No.

## [1.137.2] - 2026-06-12 — 프로젝트 이동 콤보박스 순서가 서버 재시작 시 리셋되던 현상 수정

### Fixed

- **콤보박스 순서 재시작 리셋** — 콤보박스는 busy(응답중/대기중) 프로젝트를 위로
  올리는데 busy 가 in-memory 라 서버 재시작 시 전부 소멸 → 세션 동안 누적된
  "작업 중 프로젝트 우선" 순서가 매 재시작마다 무너졌다. 영속적인 2차 정렬 키
  **최근 콘솔 활동(프로젝트별 마지막 대화 turn 시각, `lastTsByProject` — 운영 실측
  26ms 단일 GROUP BY)** 을 추가: `busy ↓ → 최근 활동 ↓ → sort_order ↑`. 재시작
  후에도 "최근에 다룬 프로젝트가 위" 순서가 유지된다. 프로젝트 목록 페이지의
  드래그 순서(sort_order)는 영향 없음.

### Wire change

- No.

## [1.137.1] - 2026-06-12 — 설정 페이지 수십 초 로딩 해소 (MCP 탭 npm spawn 제거 + lazy 탭)

### Fixed

- **설정 페이지가 수십 초 걸리던 문제** — 설정 통합 탭이 11개 탭 iframe 을 진입 즉시
  전부 prerender 하는데, 그중 `/env-setup/mcp` 가 **단독 33~36초**(운영 실측)라 설정
  진입 전체가 끌려갔다. 3중 수정:
  - **(A) MCP 설치 판정의 npm spawn 제거** — `McpService.isPackageInstalled` 가 카탈로그
    항목(65+)마다 `npm ls -g` 자식 프로세스를 spawn(1회 실측 ~505ms × 65 = ~33초)하던
    것을 npm 글로벌 root(`npm root -g` 1회 캐시) 하위 패키지 디렉토리 존재 확인(~5ms)으로
    교체. `/env-setup/mcp` 36초 → 수십 ms.
  - **(B) `/backup` 용량 측정 비동기화** — 매 방문마다 워크스페이스 전체를 동기
    walk(cold ~10초)하던 `measureSubdirs` 를 TTL 10분 캐시 + 백그라운드 측정
    (stale-while-revalidate)으로 분리. 첫 방문은 "측정 중…" 표시 후 자동 새로고침.
  - **(C) 설정/도구 통합 탭 iframe lazy 전환** — `src` → `data-src`. project-tabs.js 의
    기존 v1.47.0 lazy 기계(활성 탭 즉시 로드 + 백그라운드 한 장씩 순차 preload)를 그대로
    사용하므로 JS 변경 없음. 앞으로 어떤 탭이 느려져도 설정/도구 진입은 즉시 뜬다.

### Wire change

- No.

## [1.137.0] - 2026-06-12 — mobile-mcp 기본 설치 지정

### Changed

- **mobile-mcp(`@mobilenext/mobile-mcp`)를 기본 설치 MCP 로 지정** (`defaultInstall=true`)
  — 신규 설치(첫 실행, user-scope/legacy 모두 빈 상태)에서 memory / sequentialthinking /
  context7 / playwright 와 함께 자동 등록되고, MCP 카탈로그 체크박스가 기본 선택된다.
  등록 형태는 `npx -y @mobilenext/mobile-mcp@latest` — fast-moving 패키지라 이 항목만
  `@latest` 고정(운영자 요청).
  - **인증 검토 결과**: stdio 모드는 API 키/로그인/계정/토큰 일절 불요. SSE 서버 모드의
    선택적 `MOBILEMCP_AUTH`(Bearer) 만 존재하며 vibe-coder 는 stdio 라 무관. 전제는
    adb(빌드환경 Android SDK 에 포함)뿐. upstream README 의 Node v22+ 표기는 권장 수준
    — 이미지 Node 20 LTS 운영 환경에서 동작 실증.
  - 기존 설치엔 영향 없음(first-run 자동 등록은 빈 상태에서만, 기존 등록 보존).

## [1.136.0] - 2026-06-12 — 프롬프트 일괄 보내기 + 전역 CLAUDE.md Context7 규칙

### Added

- **프롬프트 일괄 보내기 (broadcast)** — 선택한 여러 프로젝트의 콘솔에 같은 프롬프트를
  한 번에 전송. 동시 turn 게이트(`maxConcurrentTurns`)가 한도 초과분을 큐로 순차 처리
  하고, v1.135.0 게이트 선확보 덕분에 대기 중 프로세스/메모리 점유가 없다.
  - **진입점 2곳**: ① 오버뷰 콘솔 우측 rail 프롬프트 자동화 카드 — "⏰ 예약 보내기"
    우측 "📢 일괄 보내기" 버튼, ② 프로젝트 목록 페이지 우상단 — "페이지당" 콤보 좌측.
  - 버튼 → 모달(`vb-bcast-modal`, `BroadcastModalTemplate` self-contained 조각):
    전체 프로젝트 체크박스 목록(`GET /api/projects` 오픈 시 조회, 응답중 표시·전체 선택)
    + 프롬프트 입력 + 전송. 결과로 수락/거부 수 표시.
  - **JSON API**: `POST /api/claude/broadcast` (`BroadcastSendRequestDto(prompt,
    projectIds)` → 202 `BroadcastSendResponseDto(accepted, rejected)`). 1회 최대 100개,
    존재하지 않는/ghost 프로젝트는 `rejected(not_found)`. 전송은
    `ClaudeSessionManager.sendPromptAsync` 비동기 — 응답중 프로젝트는 follow-up 으로
    CLI 내부 큐에 합류.
- **전역 CLAUDE.md 시드에 Context7 규칙 (§15)** — Compose / Flutter / AdMob /
  WindowInsets / Scaffold / SafeArea 관련 코드 수정 전 **Context7 MCP 로 현재 버전
  공식 문서를 확인 후 구현**하도록 강제하는 섹션을 한/영 시드 템플릿에 추가.
  Context7 미등록 시 MCP 카탈로그(`/env-setup/mcp`) 설치 안내. **기존 설치엔 영향
  없음** — 시드는 최초 1회만 기록되고 편집본은 덮어쓰지 않는다(미편집 시드만 언어
  전환 시 교체).

### Wire change

- **Yes.** `ApiPath.CLAUDE_BROADCAST`(`/api/claude/broadcast`) + 신규
  `shared/dto/BroadcastDtos.kt` (`BroadcastSendRequestDto` / `BroadcastRejectDto` /
  `BroadcastSendResponseDto`). additive — 기존 클라이언트 영향 없음.
  **vibe-coder-android shared/ 동기 필요.**

## [1.135.0] - 2026-06-12 — 다중 프로젝트 동시 프롬프트 시 메모리 폭발 해소 (게이트 선확보 + 상주 세션 상한)

### Fixed

- **게이트 대기 중 불필요한 세션 활성화 (메모리 폭발)** — `sendPrompt` 가 동시 turn
  게이트(`maxConcurrentTurns`, 기본 3) **대기에 들어가기 전에** claude 프로세스를 먼저
  spawn 해서, 여러 프로젝트에 프롬프트 큐를 쌓으면 대기 중인 프로젝트도 전부
  claude CLI + MCP 사이드카 트리(운영 실측 세션당 **약 850~900MB**)를 통째로 상주시키던
  문제. 이제 **permit 확보 후에 spawn** 하므로 대기 중엔 프로세스가 아예 없다
  (`ClaudeSessionManager.sendPrompt` + `SubAgentSessionManager.sendPrompt` 동형 적용,
  spawn 실패 시 permit 반환).
- **게이트 30분 이상 대기 시 프롬프트 유실** — 대기 중 세션은 `lastActivity` 가 spawn
  시점에 머물러 idle reaper(30분)가 대기 중 프로세스를 SIGTERM → permit 확보 후
  stdin write 실패로 프롬프트가 유실될 수 있었다. 위 재배치로 대기 중 프로세스 자체가
  없어져 함께 해소.

### Added

- **상주 세션 수 상한 (`claude.maxResidentSessions`, 기본 6, 0=비활성)** — 30분 idle
  reaper 만으로는 다수 프로젝트를 오가며 작업할 때 유휴 세션 트리가 누적되므로,
  상한 초과 시 **가장 오래 유휴인 세션부터 LRU 일시 중지**. session-id 가 보존되어
  다음 프롬프트에서 `--resume` 으로 같은 대화가 이어진다. 진행 중(busy / gate permit
  보유) 세션과 spawn 직후 60초 grace 세션은 절대 회수하지 않는다(전부 진행 중이면
  상한 일시 초과 허용). 메인 콘솔 / sub-agent 풀에 각각 적용. `/settings` → Claude
  섹션에서 변경 가능, **저장 즉시 반영**(ConfigHolder provider). 회수 시 콘솔에
  `resident_cap_paused` 시스템 메시지 표시.

### Wire change

- No. (JSON API / WsFrame / ApiPath 변경 없음 — 서버 내부 동작 + SSR 설정 폼 항목 추가만.)

## [1.134.1] - 2026-06-12 — 이미지 첨부 UI 컴팩트화 (다이얼로그) + 파일선택 박스 노출 수정

### Fixed

- **입력창 우측에 파일선택 박스가 그대로 노출되던 문제** — `admin.css` 의
  `input[type=file] { display:block … }`(구체성 0,1,1)이 UA `[hidden]`(0,1,0)을 이겨
  hidden 속성이 무력화됐다(v1.131.1 rail 카드와 동일 패턴). inline `display:none` 으로
  확실히 숨김.

### Changed

- **이미지 첨부 UI 를 다이얼로그로 이동(우측 공간 컴팩트화)** — 입력창 우측엔 📷 아이콘
  버튼만 남기고, 클릭하면 첨부 다이얼로그(#image-modal)가 열린다: 파일 선택 버튼 +
  미리보기 썸네일 + 개별 제거(✕) + 안내문. 인라인 미리보기 strip 제거.
  - 아이콘 버튼에 **첨부 수 배지** — 붙여넣기/드래그로 첨부해도 한눈에 보임.
  - 붙여넣기(Ctrl+V)/드래그&드롭은 다이얼로그를 열지 않아도 기존대로 동작.

### Wire change

- No.

## [1.134.0] - 2026-06-12 — rail 프롬프트 히스토리: 전체 목록 스크롤 + 하단 페이드

### Changed

- **프롬프트 히스토리 카드(콘솔 우측 오버뷰 rail)** — 최근 7개 제한을 풀고 **현재까지
  보낸 모든 프롬프트**(최신 위, 안전 상한 1000)를 스크롤 목록으로 표시.
  - 각 항목은 기존처럼 **2줄 축약** 유지(표시/hover title 은 300자 미리보기, 클릭 채움용
    `data-prompt` 는 원문 그대로).
  - 목록 높이는 약 7행 — **5개 선명 + 마지막 ~2개 페이드아웃**(마스크 그라데이션).
    스크롤 끝에 닿거나 항목이 적어 스크롤이 불필요하면 페이드 자동 제거(`.at-end`).
  - 콘솔에서 새 프롬프트 전송 시 prepend 후 목록 맨 위로 스크롤(최신 항상 가시).
    기존 7개 절단 + 항목별 opacity 램프 로직 제거.

### Wire change

- No.

## [1.133.0] - 2026-06-12 — 콘솔 이미지: 프롬프트 첨부 전송 + 실제 이미지 렌더 + usage 필터

### Added

- **프롬프트 이미지 첨부 전송** — 콘솔 입력창에서 이미지를 프롬프트와 함께 Claude 에
  보낼 수 있다 (vision). 첨부 경로 3종: 입력창 옆 **이미지 버튼**(파일 선택, 다중) /
  **클립보드 붙여넣기**(스크린샷 Ctrl+V) / **드래그&드롭**. 최대 4장, 장당 base64
  7,000,000 chars(≈5MB). 큰 이미지는 브라우저에서 최대 변 1568px 로 자동 다운스케일
  (토큰 절약, gif 는 원본 유지). 텍스트 없이 이미지만 첨부하면 기본 문구로 전송.
  - 서버: stream-json user message 의 `image` content block (base64) 으로 텍스트 앞에
    배치 — Claude Code TUI 의 이미지 붙여넣기와 동형. `console/prompt` 와
    `console/interrupt`(끼어들기) 둘 다 지원.
  - 첨부 이미지는 `conversation_turns.raw` 에 JSON 으로 보존 — 콘솔 이력 복원 시에도
    내가 보낸 이미지가 보인다.
- **콘솔 이미지 표시** — tool 결과(이미지 파일 Read / 스크린샷 도구 등)의 base64 이미지
  블록을 기존 `[image]` 텍스트 대신 **실제 썸네일**로 렌더. 클릭으로 확대/축소 토글.
  - live WS 프레임: inline base64 를 그대로 data: URL 로 렌더.
  - DB 이력 복원: base64 는 페이지 inline JSON 에 싣지 않고(수 MB 비대 + 기존 4000자
    절단으로 JSON 깨짐 회피) `images:[{mediaType}]` 메타만 emit, 썸네일은 신규
    `GET /api/projects/{id}/claude/console/image?turn=N&idx=M` 가 DB row 에서 서빙
    (세션 쿠키 인증, 현재 세션 범위). 추출/분리 로직은 `ConsoleImages` + 단위 테스트.
- **토큰 사용량 메시지 필터** — turn 마다 찍히는 `usage: input N · output N ·
  cache-read N · cache-create N` 시스템 메시지가 별도 'usage' 카테고리로 분리되어
  콘솔 메시지 필터(🔍)에서 토글 가능 (이전엔 mandatory 'system' 에 묶여 항상 노출).

### Wire change

- **Yes** — `PromptRequestDto` 에 `images: List<PromptImageDto>? = null` 추가 +
  신규 `PromptImageDto(mediaType, data)` (`shared/dto/ConsoleDtos.kt`).
  - 호환성: 필드 생략 시 기존과 동일(텍스트만) — 구버전 클라이언트(Android 포함) 그대로
    동작. 서버 응답 shape 변경 없음.
  - 신규 이미지 서빙 endpoint 는 admin 세션 쿠키 전용(SSR 콘솔 내부용)이라 ApiPath
    SSOT 대상 아님(`console/history` 와 동급의 SSR 보조 endpoint).
  - **vibe-coder-android shared/ 동기 필요** (PromptImageDto 추가 시 모바일에서도 첨부 가능).

### Fixed

- 콘솔 이력 복원 시 이미지 포함 tool 결과가 4000자 절단으로 JSON 이 깨져 raw 텍스트로
  노출되던 문제 — base64 를 서버측에서 분리(strip)해 절단 자체가 일어나지 않게 됨.

## [1.132.1] - 2026-06-12 — 콘솔 이력 절단 이모지 반토막 500 수정

### Fixed

- **콘솔 페이지 500 (`MalformedInputException: Input length = 1`)** — 콘솔 인라인 대화
  이력 직렬화(`renderInitialHistoryJson`)가 긴 메시지를 4000자(UTF-16 코드유닛) 경계에서
  자를 때 이모지(surrogate pair)가 경계에 걸리면 짝 없는 high surrogate 가 남고,
  `respondText` 의 UTF-8 엄격 인코딩이 예외로 터져 **페이지 전체가 500** 이 되던 문제.
  (실증: kauslim 프로젝트 turn_idx=778 의 🟢 U+1F7E2 가 정확히 4000자 경계에 걸림.)
  - 절단을 surrogate-safe 하게 — 경계 char 가 high surrogate 면 한 칸 당겨 자름.
  - 방어선으로 `sanitizeSurrogates()` 헬퍼 추가 — `jsLit`/`esc` 진입부에서 짝 없는
    surrogate 를 U+FFFD 로 치환해 다른 경로의 lone surrogate 도 500 을 못 일으키게 함.
  - 회귀 테스트 `RenderInitialHistorySurrogateTest` 추가 (4000자 경계 이모지 + 엄격
    UTF-8 인코딩 통과 단언).
...
## [1.132.0] - 2026-06-11 — 프로젝트 백업/복원 (이식형 export/import)

### Added

- **프로젝트 백업/복원** — 프로젝트 단위로 소스 + 서명 키스토어 + 문서 + 설정을 단일
  `.tar.gz` 로 묶어 다운로드하고, 다른 서버에서 업로드 복원하면 **동일한 상태에서 작업을
  이어갈 수 있다**.
  - **백업(다운로드)**: 프로젝트 화면의 "더보기" 탭에 **백업** 탭 추가
    (`GET /projects/{id}/backup` 페이지 + `GET /projects/{id}/backup/download`). 원본/DB 를
    건드리지 않고 tar.gz 만 생성해 다운로드(전송 후 임시 tar 삭제).
  - **복원(업로드)**: **설정 → 백업** 페이지 상단에 "프로젝트 복원" 업로드 폼 추가
    (`POST /backup/project-restore`, multipart). id/폴더/패키지명 키스토어가 이미 있으면
    덮어쓰지 않고 거부(기존 데이터·서명키 보호).
  - tar 레이아웃(`manifest.json` + `project/` + `vibecoder/` + `keystores/`)은 기존 프로젝트
    **아카이브와 동일** — `ProjectArchiveService` 에 `backupToTar`(원본 보존) / `restoreFromTar`
    (업로드 파일 기반, archived_projects 레지스트리 무관) 추가로 helper 재사용. 아카이브 tar 도
    상호 복원 가능.
  - `manifest.json` 에 `projectType`(kotlin/flutter) 추가(version 2) — 복원 시 프로젝트 타입
    보존. 구 manifest(version 1)는 `ignoreUnknownKeys` + default kotlin 으로 호환. 같은 변경으로
    기존 아카이브 복원도 projectType 보존.

### Changed

- `/backup` 페이지가 `?ok=`/`?err=` flash 를 표시(이전엔 redirect 결과 미표시).
- `backupRoutes` 등록을 아카이브 `run {}` 블록으로 이동(`ProjectArchiveService` 공유).

## [1.131.1] - 2026-06-11 — 자동화 카드 "진행 중" 점이 항상 깜빡이던 버그

### Fixed

- **자동화 진행 중이 아닌데도 카드의 점(`pt-auto-dot`)이 항상 깜빡이던 문제** — `pt-auto-running`
  영역은 `hidden` 속성으로 숨겨야 하는데, `#project-tabs-root .pt-auto-running { display: flex }`
  (구체성 1,1,0)가 UA 의 `[hidden]`(0,1,0)을 이겨 `hidden` 이 무력화되고 있었다. 진행률 텍스트는
  비어 있고 깜빡이는 점만 노출돼 "사용중이 아닌데 깜빡인다"로 보였다. `#project-tabs-root
  .pt-auto-running[hidden] { display: none; }`(구체성 1,2,0)을 추가해 명시적으로 숨김.
  인라인 `<style>`이라 새로고침 즉시 반영(캐시버스트 불필요).

## [1.131.0] - 2026-06-11 — 예약 보내기 진입점 이동 + 자동화 카드 개선

### Changed

- **프롬프트 예약 UI 를 오버뷰 rail 의 "프롬프트 자동화 카드" 상단으로 이동** — v1.130.0 에서
  SSR 자동화 페이지(`/projects/{id}/automation/prompts`) 상단에 넣었던 예약 UI 를, 운영자
  요청대로 **콘솔 우측 rail(`ProjectTabsTemplate` `pt-auto-card`)** 상단의 "⏰ 예약 보내기"
  버튼 → 부모 레이어 모달(`pt-sched-modal`)로 옮김. 모달에서 프롬프트 + 트리거(시간 지정/세션·
  주간 한도 해제) 입력 + 예약 목록 조회/취소. 백엔드(스케줄러·`scheduled_prompts`·JSON API)는
  그대로 재사용 — `ScheduleSendRequestDto.atEpochMs` 는 모달 JS 가 datetime-local 을 epoch 로
  변환해 전송. SSR 페이지의 예약 섹션 + SSR 전용 라우트(`/schedule`·`/schedule/{id}/cancel`)는
  제거(JSON `GET/POST/DELETE /api/projects/{id}/claude/schedule` 은 유지).
- **자동화 카드 "중단" 버튼 제거** — 진행 중 자동화 중단은 콘솔/자동화 설정 페이지에서 수행.
  카드는 상태 표시 + 시작 + 예약에 집중.
- **자동화 카드 인디케이터(badge)를 "사용중" 상태 표시로** — 자동화 진행 중이면 헤더 옆 badge 가
  `● 진행 중`(초록) 으로 사용중을 명확히 표시(이전엔 `· sent/total` 숫자). 진행률 자체는 카드 본문
  유지. 유휴 시 badge 비움.
- `project-tabs.js?v=1.103.0 → ?v=1.131.0` (캐시버스트), i18n `console.schedule.*` 키 추가(ko/en).

## [1.130.0] - 2026-06-11 — 프롬프트 예약 전송 (Phase 99)

### Added

- **프롬프트 예약 전송 (one-shot scheduled prompt)** — 지정한 **시점**이 되면 프롬프트를
  콘솔로 1회 자동 전송. 프롬프트 자동화(`/projects/{id}/automation/prompts`) 페이지 **상단**에
  "⏰ 예약 보내기" 섹션 + "예약 목록" 표 추가. 자동화(turn 완료마다 연쇄)와 달리 단발 예약.
  - **트리거 3종**:
    - `time` — 절대 시각. UI 입력은 **N분 뒤 / N시간 뒤 / 정확한 시각(datetime-local)** 3가지,
      모두 생성 시점에 epoch 로 환산되어 저장.
    - `session_reset` — Claude **세션(5h) 사용 한도 해제** 감지 시.
    - `weekly_reset` — Claude **주간(7d) 사용 한도 해제** 감지 시.
  - 한도 해제 감지는 `ClaudeStatusService.cachedSnapshot` 의 session/weekly UsagePercent
    (계정 전역값, 백그라운드 `ClaudeUsageMonitor` 5분 폴링) 사용. 예약 시점 사용량%(baseline)
    대비 30p 이상 하락 또는 절대 15% 이하면 "리셋됨" 판정. 관측 불가(null)면 보수적 보류.
  - 발사는 항상 콘솔이 **유휴**일 때만 → 진행 중 turn 에 follow-up 으로 끼지 않고 깔끔한 새 turn.
  - 신규: `ScheduledPromptManager`(30초 폴링 스케줄러), `ScheduledPromptRepository`,
    `scheduled_prompts` 테이블. 라우트는 기존 `promptAutomationRoutes` 에 통합
    (SSR `/schedule`·`/schedule/{id}/cancel` + JSON `GET/POST/DELETE`).
  - 프로젝트 삭제/이름변경 시 `ProjectService.delete` / `ProjectRepository.renameId` 의 단일
    transaction 에서 `scheduled_prompts` 도 정리/repoint (다른 자식 테이블과 동일 패턴).

### Wire change

- **Yes** — Android `shared/` 동기 필요.
  - `ApiPath.kt` 신규 2개: `scheduledPrompts(projectId)` =
    `GET/POST /api/projects/{id}/claude/schedule`, `scheduledPrompt(projectId, scheduleId)` =
    `DELETE /api/projects/{id}/claude/schedule/{scheduleId}`.
  - 신규 DTO 파일 `ScheduledPromptDtos.kt`: `ScheduledPromptTriggers`(time/session_reset/weekly_reset),
    `ScheduledPromptStatus`(pending/sent/cancelled/failed), `ScheduledPromptDto`,
    `ScheduleSendRequestDto`, `ScheduledPromptsResponseDto`.
  - 모든 신규 필드는 기본값 보유 + 안드 `ignoreUnknownKeys=true` 라 구버전 클라이언트 안전.
    catch-up 은 vibe-coder-android `shared/` 에 같은 두 파일 사본 추가.

## [1.129.1] - 2026-06-11 — 콘솔 "더보기"를 스크롤 최상단에서만 표시

### Changed

- 콘솔 "더보기" 버튼을 console-log 스크롤 영역 **밖(항상 표시)**에서 **안의 첫 자식**으로 이동.
  스크롤 콘텐츠의 일부가 되어 **최상단으로 올렸을 때만 보이고**, 아래로 내려가면 화면 밖으로
  사라진다. 더보기 클릭 시 과거 30개는 버튼 다음에 prepend(버튼은 최상단 유지).

## [1.129.0] - 2026-06-11 — 콘솔 메시지 개선 (30개+더보기 / 중복 제거 / 자동스크롤 재설계)

콘솔(Claude 대화) 렌더링을 전반 개선. 정밀 분석에서 드러난 "중복 표시"와 "내 프롬프트 누락"
을 history(DB·과거) ↔ WS replay(ring·미래) 경계 분리로 해소하고, 메시지 로딩을 페이지네이션화.

### Added

- **초기 30개 + 더보기 페이지네이션** — 콘솔 진입 시 최근 30개만 로드, 최상단 "더보기" 클릭으로
  과거 30개씩 추가(스크롤 위치 유지). 서버 `GET /api/projects/{id}/claude/console/history?before=&limit=`
  + `ConversationTurnRepository.Filter.beforeTurnIdx`. 초기/더보기 공용 `renderHistoryRow`.

### Fixed

- **메시지 중복 표시** — DB history 복원과 WS ring replay가 같은 메시지를 dedup 없이 둘 다 렌더하던
  문제. WS 연결 시 `since=initialMaxSeq`(history 경계 seq) 전송 → ring 의 과거 프레임을 replay 하지
  않음. 재연결도 `since=lastSeq`. 서버 인프라(`subscribeConsole(since)`, `consoleCurrentSeq`)는
  완비돼 있었고 클라가 since 를 안 보내던 것이 원인.
- **내가 보낸 프롬프트 누락** — 30개 안에 user(현재/마지막 프롬프트)가 없으면(매우 긴 turn) 마지막
  user 1개를 history 맨 앞에 붙여 상단 고정(`.cur` sticky)을 30개 밖이어도 유지.

### Changed

- **자동스크롤 토글 제거** — `📌 자동 스크롤` 체크박스/localStorage 제거. 대신 "최하단 근처에
  있는가"(stickyWanted)만으로 추적: 최하단이면 새 메시지 따라 내려가고, 스크롤 업 하면 해제(과거
  보는 중 화면 고정). 탭 숨김 중 isAtBottom 부정확 문제를 scroll 이벤트 기반 플래그로 회피.

### Notes

- 수정은 SSR inline JS(`WebProjectTemplates`)만 — `console-render.js`(VibeConsole) 미변경이라
  `?v`/SW 캐시버스트 불필요.
- `console history` endpoint 의 `ApiPath` SSOT 등록은 후속(클라는 JS 라 직접 URL 사용).

## [1.128.7] - 2026-06-11 — 버전명 정적 해석기 (변수/보간/version.properties) + v 중복 제거

### Fixed

- **Kotlin 프로젝트 다수의 버전 미표시/깨짐**. `appVersionName` 이 `versionName = "리터럴"` 만
  읽어, 운영 프로젝트의 변수 참조·문자열 보간·`version.properties` 참조 정의를 못 잡았다.
  신규 `VersionNameResolver` 가 정적 해석:
  - 변수 참조 `= pocketmindVersionName` → `val pocketmindVersionName = "0.17.17"`
  - 보간 `= "v$versionMajor.$versionMinor.$versionPatch"` (+ `val versionMajor = 1`)
  - `= "${versionProps["VERSION_MAJOR"] ?: "1"}.…"` (+ rootProject `version.properties`)
  - 다단계(변수→보간→versionProps) 재귀. 못 풀어 `$` 가 남으면 null(깨진 raw 미표시).
  주석의 versionName 설명은 무시. 회귀 테스트 10개(운영 실측 패턴 픽스처).
- **버전 뱃지 `v` 중복**. versionName 이 이미 `v1.7.3` 처럼 `v` 접두를 포함하면 목록 뱃지가
  `vv1.7.3` 으로 표시되던 것 수정(이미 `v`/`V` 로 시작하면 추가 안 함).

## [1.128.6] - 2026-06-11 — Flutter 앱 버전 인식 (pubspec.yaml)

### Fixed

- Flutter 프로젝트의 목록 버전 뱃지 미표시. `appVersionName` 이 Gradle `versionName = "..."`
  리터럴만 읽어, Flutter(android/app/build.gradle 의 `versionName flutterVersionName` 변수)는
  못 잡았다. Gradle 에서 못 찾으면 **`pubspec.yaml` 의 `version: 1.2.3+4` → `1.2.3`**(+ 뒤 build
  number 제외)로 폴백. moduleName 경로도 nested(`:`) 대응.

## [1.128.5] - 2026-06-11 — Flutter 앱 아이콘 인식 (android/app/src/main/res)

### Fixed

- Flutter 레포 추가 시 프로젝트 목록 앱 아이콘 미인식. `resolveAppIcon` 이 `moduleName`
  경로(`app/src/main/res`)만 봐서 Flutter 의 `android/app/src/main/res/mipmap-*/ic_launcher.*`
  를 못 찾았다. Kotlin(`<module>/src/main/res`) + Flutter(`android/app/src/main/res`,
  `android/<module>/src/main/res`) **양쪽 경로를 탐색**하도록 개선. projectType 무관하게 동작.

## [1.128.4] - 2026-06-11 — 프로젝트 목록 타입 뱃지를 별도 컬럼(핸들 좌측)으로

### Changed

- 프로젝트 목록 Kotlin/Flutter 타입 뱃지(v1.128.1)를 패키지명 셀에서 분리해 **순서변경
  핸들(☰) 좌측의 별도 컬럼**으로 우측 정렬 표시. 빈 데이터 행 colspan 4→5.

## [1.128.3] - 2026-06-11 — 콘솔 프로젝트 콤보박스 busy 우선 정렬

### Changed

- 콘솔 상단 프로젝트 이동 콤보박스(`ProjectTabsTemplate` switcher)에서 **유휴 아닌 busy
  (responding/waiting) 프로젝트를 상단에 표시**. SSR 초기 렌더는 `ProjectState.busy` 기준 stable
  정렬(같은 그룹 내 sort_order 유지), JS WS patch(`project_busy_changed`)는 busy 로 전환된
  프로젝트를 리스트 최상단으로 이동 → **마지막 상태 변경이 위에 남음**. idle 전환은 위치 유지
  (다음 새로고침 시 SSR 정렬로 재배치).

## [1.128.2] - 2026-06-11 — 타임스탬프 표시 lang-aware 통일 (마이크로초 제거)

### Changed

- 모든 SSR 화면의 raw ISO 타임스탬프(마이크로초·T·Z 노출)를 공통 헬퍼
  `AdminTemplates.fmtTs(iso, lang)` / `fmtTsEpochMs(ms, lang)` 로 통일. **ko=`2026/06/11
  18:00:00`(yyyy/MM/dd), en=`06/11/2026 18:00:00`(MM/dd/yyyy 미국 순서)**, 둘 다 KST 초 단위.
- 적용 20개 지점(15개 파일): 대시보드 Claude usage / 디바이스 / 백업 / 프로젝트 상세 updated /
  키스토어 / git PAT / 빌드 비교·상세 / 파일 / audit / 자동화 / 프롬프트 / push / passkey /
  히스토리 검색 · 목록 / agent / webhook secret. 기존 `fmtInstant`(lang 무시 단일 패턴)도 `fmtTs`
  로 통합(`buildTsFormatter` 제거).
- 미대상(의도): JSON DTO 직렬화(`expiresAtIso` 등 Android 클라이언트용), 파일명/로그,
  JS 상대시간(relTime)·`toLocaleString`(이미 locale 자동).

## [1.128.1] - 2026-06-11 — 프로젝트 목록 Kotlin/Flutter 타입 뱃지

### Added

- 프로젝트 목록(`/projects`) 각 행의 패키지명 우측에 **프로젝트 타입 뱃지**(Kotlin=퍼플 /
  Flutter=브랜드 블루) 표시. `ProjectDto.projectType` 기반 — 한눈에 빌드 타입 구분.

## [1.128.0] - 2026-06-11 — clone 프로젝트 타입 불일치 확인 다이얼로그

기존 레포 추가(clone) 시 사용자가 고른 타입(Kotlin/Flutter)과 서버가 감지한 실제 타입이
다르면 등록을 중단하고 **확인 페이지**를 띄운다. "감지값으로 진행" / "선택값 그대로 진행"
두 버튼으로 사용자가 결정한다. Flutter 프로젝트를 Kotlin으로(또는 반대로) 잘못 추가하는 사고 방지.

### Added

- **타입 불일치 확인 페이지** — clone 후 `PackageNameDetector.detectProjectType`(루트
  `pubspec.yaml`→flutter, gradle settings/build→kotlin, 불명확→null)이 사용자 선택과 다르면
  `register` 가 `409 project_type_mismatch` 로 중단. `WebProjectRoutes` 가 이를 잡아
  `projectTypeMismatchPage`(감지값 수용 / 선택값 강제 두 버튼) 렌더.
- **직전 clone 재사용** — 확인 후 재제출(`projectTypeAck=true`)은 이미 받아둔 clone 을
  재사용(`.git` 존재 검사)하여 재clone 하지 않는다. 큰 레포에서 중복 clone 방지.
- 회귀 테스트 1개 추가(`detectProjectType` 우선순위, 총 8개).

### Wire change

**Yes** — `RegisterProjectRequestDto.projectTypeAck: Boolean = false` 추가
(`shared/dto/Dtos.kt`). additive default → 구버전 클라이언트 호환(미전송 시 false = mismatch
검사 정상 동작). `ApiPath` 변경 없음. **vibe-coder-android shared/ 동기 권고**(미동기여도 기존
동작 유지 — Android 클라이언트는 아직 clone 폼 미구현).

### Notes

- 감지가 불명확(pubspec/gradle 둘 다 없음)하면 경고하지 않고 사용자 선택을 따른다.
- mismatch 시 clone 폴더는 보존되며, 확인 페이지에서 어느 버튼을 눌러도 재clone 없이 진행.

## [1.127.2] - 2026-06-11 — Kotlin/Flutter 분기 정밀 리뷰 후속 수정

P1~P4 Kotlin/Flutter 분기 코드의 정밀 리뷰에서 발견된 결함 회수.

### Fixed

- **[#1] FlutterToolchain 의 flutter 미설치 안내가 dead code** — `ProcessRunner.run` 의
  `pb.start()` 가 try/catch 밖이라 flutter 미설치 시 `exit 127` 이 아니라 IOException 으로
  전파됐다. 사용자에게 친절한 `/env-setup` 설치 안내 대신 raw IOException 만 노출됨.
  GradleBuilder 가 gradlew 를 `Files.exists` 로 선체크하듯, `isFlutterAvailable()`(flutter
  --version 선확인)을 추가해 미설치 시 친절 안내 + 127 반환. 기존 dead 분기는 제거.
- **[#2] PackageNameDetector 의 한 줄 다중 include 사각지대** — `include\s*\(?...` 정규식이
  `include(":core", ":app")` 한 줄에서 **첫 인자만** 캡처해, app 모듈이 둘째 이후면
  `detectAppModule` 이 null 이 됐다(→ moduleName 이 기본 "app" 으로 오폴백). 콜론-prefixed
  모듈 path 를 전부 추출하는 정규식(`["']:([...])["']`)으로 교체 — 한 줄 다중/멀티라인/연속
  include 모두 커버. 회귀 테스트 1개 추가(총 7개).
- **[robustness] ProjectTypes.normalize 의 case 민감성** — `"Flutter"`/` flutter ` 같은 비정규
  입력(JSON API 직접 호출)이 kotlin 으로 떨어지던 것을 `trim().lowercase()` 로 흡수.

### Notes (정밀 리뷰에서 P5 로 미룬 항목)

- [#3 Minor] FlutterToolchain.findArtifact 는 단일 `app-{debug,release}.apk` 가정 —
  `--split-per-abi`/flavor/모노레포(서브디렉토리 Flutter 앱) 산출물 미지원. 현재 빌드 명령이
  split 을 안 주므로 기본 경로는 정상. P5 에서 flavor/split 대응.
- [#4 Nit] Flutter debug 빌드도 키스토어 가드(requireKeystoreOrThrow)에 막힘 — 기존 Kotlin
  정책과 일관이나 Flutter debug 는 키스토어 불필요. P5 에서 debug 가드 면제 검토.

## [1.127.1] - 2026-06-11 — 패키지명(applicationId) 자동 감지 버그 수정 (멀티모듈 namespace 오인)

### Fixed

- **clone 패키지명 감지가 라이브러리 서브모듈 namespace 를 applicationId 로 오인**하던 버그.
  이전 `detectPackageFromClonedRepo` 는 `Files.walk` 로 전체를 훑어 `applicationId|namespace`
  중 **첫 매치**(파일 순회 순서는 비결정적)를 채택했다. 멀티모듈에서 라이브러리 서브모듈의
  `namespace`(예: `:service:session-service` 의 `com.siashell.service`)가 app 모듈의
  applicationId(`com.siashell.app`)보다 먼저 걸리면 그 서브모듈 namespace 가 packageName 으로
  잘못 등록됐다 → 키스토어/AdMob 가 틀린 prefix 로 발급(운영 사고: siashell, Calculator).
- **정정**: 감지를 `PackageNameDetector`(신규)로 이관하고 우선순위 명문화:
  1. `com.android.application` app 모듈의 `defaultConfig.applicationId`(release base —
     `applicationIdSuffix` 매칭 제외).
  2. 없으면 그 app 모듈의 `namespace` 폴백.
  3. app 모듈 미식별 시 전체에서 **`applicationId` 만**(namespace 제외) 검색 → 라이브러리
     namespace 오인이 구조적으로 불가능. AndroidManifest `package=` fallback 은 제거.
- 회귀 테스트 6개 추가(`PackageNameDetectorTest`): siashell 구조 모사 / suffix 제외 /
  namespace 폴백 / 라이브러리 전용 → null / 단일 모듈 / nested gradle path.

### Notes

- 기존에 잘못 등록된 프로젝트(siashell `com.siashell.service`, Calculator)의 DB packageName 과
  오발급 키스토어는 **본 코드 수정으로 자동 정정되지 않는다**(신규 clone 감지만 고침). 운영자
  확인 후 별도 마이그레이션(패키지명 변경 + 키스토어 백업 후 재매핑) 필요.

## [1.127.0] - 2026-06-11 — 프로젝트 생성 폼 타입 선택 + Flutter CLAUDE.md (Flutter 지원 완결)

Flutter(Android) 지원 로드맵의 **Phase 4** — 신규 프로젝트 생성 폼에 Kotlin/Flutter 타입
선택을 추가하고, Flutter 선택 시 첫 `CLAUDE.md` 를 Android 전용 Flutter 규칙으로 생성한다.
P1(데이터)·P2(SDK)·P3(빌드) 위에 폼 UI 가 얹혀 **웹에서 Flutter 프로젝트 생성 → 빌드까지 완결**.

### Added

- **생성 폼 프로젝트 타입 라디오**(`/projects` 의 "새 프로젝트" 폼) — `Kotlin (Android)` 기본 /
  `Flutter (Android-only)`. empty/clone 양쪽 공통. `WebProjectRoutes` 가 `projectType` 파라미터를
  `RegisterProjectRequestDto` 로 전달(register 의 `ProjectTypes.normalize` 가 최종 검증).
- **Flutter CLAUDE.md 템플릿**(`ClaudeMdTemplate.CONTENT_FLUTTER` + `renderFlutter`) — Dart/Flutter
  규칙 + **Platform Restriction(Android only)** 섹션(`flutter create --platforms=android`,
  iOS/web/desktop 빌드 금지) + `flutter build apk/appbundle` 빌드 룰 + key.properties 서명 안내.
  `ProjectInfo.projectType` 으로 분기. Kotlin body 는 기존과 동일.
- i18n 키 5개 ×2(en/ko) — `projects.new.projectType` / `typeKotlin` / `typeFlutter` (+Desc).

### Notes

- **기존 프로젝트 영향 없음** — Flutter body 는 신규 생성 시에만(`notExists()` 가드). 기존 Kotlin
  프로젝트의 CLAUDE.md 는 불변.
- clone 시에도 사용자 선택 우선(기본 Kotlin). `pubspec.yaml` 자동감지 힌트/타입 변경 UI 는 후속.
- Wire/DB 변경 없음(P1 의 projectType 필드 재사용) → Android 동기 불필요.

## [1.126.0] - 2026-06-11 — 빌드 파이프라인 toolchain 추상화 + Flutter 빌드

Flutter(Android) 지원 로드맵의 **Phase 3** — 빌드 실행을 `BuildToolchain` 인터페이스로
추상화하고 `project_type`(P1)으로 Gradle/Flutter 구현을 선택한다. 이제 `projectType=flutter`
프로젝트는 `flutter build apk/appbundle` 로, `kotlin` 은 기존 Gradle 로 빌드된다.
Kotlin 빌드 경로는 **동작 동일**(회귀 없음) — 추상화만 추가.

### Added

- **`BuildToolchain` 인터페이스 + `BuildVariant` enum**(`build/BuildToolchain.kt`).
  `BuildVariant`(DEBUG/RELEASE/BUNDLE)가 wire 문자열·확장자·artifact type 메타를 보유
  (값은 v1.107.0 과 동일 — wire/artifact 호환).
- **`GradleToolchain`**(`build/GradleToolchain.kt`) — 기존 `GradleBuilder` + `ApkFinder`
  위임. `releaseTaskFor`(assembleDebug→assembleRelease)를 BuildService 에서 이관.
- **`FlutterToolchain`**(`build/FlutterToolchain.kt`) — **Android 앱 빌드 전용**.
  debug→`flutter build apk --debug`, release→`flutter build apk --release`,
  bundle→`flutter build appbundle --release`. 산출물 `build/app/outputs/flutter-apk/*.apk`
  (+ `bundle/release/app-release.aab`). `flutter` 미설치 시 exit 127 → 설치 안내 로그.

### Changed

- `BuildService.enqueueBuild` 가 variant·gradleTask·finder·ext·artifactType 7개 파라미터
  대신 `BuildVariant` 1개를 받고, 실제 빌드/산출물 해석을 `row.projectType` 으로 선택한
  toolchain 에 위임. `GradleBuilder`/`config` 는 이미 주입돼 있어 **DI(ServerMain) 무변경**.

### Notes

- **Flutter 서명**: `flutter build` 는 AGP injected signing(`-Pandroid.injected.signing.*`)을
  직접 전달 못 함 → Flutter Android release 는 `android/key.properties` + `signingConfigs.release`
  표준 사용. signing 후보가 있으면 안내 로그만(실제 서명은 프로젝트 key.properties, P4/P5).
- **키스토어 가드 공통 유지**: Flutter debug 도 현재는 키스토어를 요구(기존 Kotlin 정책 일관).
  Flutter debug 가드 면제는 후속 검토.
- Wire/DB 변경 **없음**(`BuildVariant.wire` 가 기존 variant 문자열과 동일) → Android 동기 불필요.
- 동작하려면 Flutter SDK 설치(P2) + `projectType=flutter` 프로젝트(현재는 JSON API 로만 생성
  가능, 폼 UI 는 P4) 필요.

## [1.125.0] - 2026-06-11 — 프로젝트 타입(Kotlin/Flutter) 데이터 모델 + wire

Flutter(Android) 지원 로드맵(`docs/01-plan/flutter-android-support.md`)의 **Phase 1** —
프로젝트가 Kotlin(Android-Kotlin)인지 Flutter(Android-Flutter)인지 DB 에 영속하는 SSOT 컬럼과
wire 필드를 추가한다. 빌드 파이프라인 분기(`BuildToolchain`, P3)와 생성/clone 폼 타입 선택
+ CLAUDE.md 분기(P4)의 기반. 이 단계만으로는 사용자 가시 동작 변화가 없다(모든 프로젝트가
기본 `kotlin`). 둘 다 **Android 빌드 타깃** — 프레임워크 구분만.

### Wire change

**Yes** — `ProjectDto` + `RegisterProjectRequestDto` 에 `projectType: String` 필드 추가
(`shared/dto/Dtos.kt`). 신규 상수 `ProjectTypes`(object, `KOTLIN`/`FLUTTER`/`ALL`/`normalize`).

- 기본값 `"kotlin"` 의 **additive default** → 구버전 클라이언트 wire 호환(필드 생략 시 kotlin).
- `RegisterProjectRequestDto.projectType` 미전송 시 서버가 `ProjectTypes.normalize` 로 흡수
  (알 수 없는 값/null → `kotlin`). 추후 오류 방지 안전망.
- enum 이 아니라 `const String` — 구버전 클라이언트가 새 값(`flutter`)을 만나도 deserialize 안전
  (메모리 wire 점검법: 안드는 object+const, `ignoreUnknownKeys=true`).
- `ApiPath.kt` 변경 **없음** — 기존 register/list 경로 재사용.
- **vibe-coder-android shared/ 동기 필요**: `ProjectDto`/`RegisterProjectRequestDto` 에 같은
  `projectType` 필드 + `ProjectTypes` object 추가. additive default 라 미동기여도 기존 동작 유지.

### Added

- DB `projects.project_type varchar(16) default 'kotlin'` (`Schemas.kt`). 기존 행은
  `createMissingTablesAndColumns` 가 `ADD COLUMN ... DEFAULT 'kotlin' NOT NULL` 로 자동 마이그
  (sortOrder 선례). 별도 수동 ALTER 불필요.
- `ProjectRow.projectType` + `ProjectRepository` insert/toRow/renameId 3곳 전파.
- `ProjectService.register` 가 `body.projectType` 를 normalize 후 영속, `toDto` 가 노출.

### Notes

- General Chat ghost(`__scratch__`/`__chat_*`)는 빌드 대상이 아니라 default `kotlin` 유지(무의미).
- 폼 UI 타입 선택·clone 시 `pubspec.yaml` 자동감지·CLAUDE.md Flutter body 는 **P4** 에서.

## [1.124.0] - 2026-06-11 — 빌드환경에 Flutter SDK 설치 메뉴 (Android 앱 빌드 전용)

Flutter(Android) 프로젝트 지원 로드맵(`docs/01-plan/flutter-android-support.md`)의 **Phase 2** —
빌드환경 페이지에 Flutter SDK 를 원터치 설치하는 컴포넌트를 추가한다. 다른 Phase 와 독립적이라
단독 릴리즈. 빌드 파이프라인 연동(`FlutterToolchain`)·프로젝트 타입 선택은 후속 Phase.

운영자 정책 **"Flutter 는 Android 앱 빌드 전용"** 을 설치 단계에서 강제: ① `flutter config` 로
iOS/web/desktop 플랫폼 비활성(android 만 enable), ② `flutter precache --android --no-ios --no-web
--no-linux --no-windows --no-macos --no-fuchsia` 로 Android artifact 만 선다운로드 → iOS engine
artifact(1GB+) 등 불필요한 수 GB 다운로드를 차단(디스크 절약).

### Added

- **빌드환경 Flutter 컴포넌트** — `/env-setup` 의 핵심 섹션(Gradle 다음 우선순위)에 "Flutter
  (Android-only)" 카드 추가. 설치 버튼 → `vibe-doctor flutter` 자식 프로세스 → 실시간 WS 로그
  진행 페이지(기존 task 흐름 재사용). `SetupComponent.FLUTTER`(`doctorCmd="flutter"`) +
  `probeFlutter()`(`.local/flutter/bin/flutter` 존재 검사 또는 `flutter --version`).
- **`vibe-doctor flutter` 서브커맨드** — `docker/doctor/lib/flutter.sh`. git stable channel 을
  `/home/vibe/.local/flutter` 에 `--depth 1` clone, `.local/bin/{flutter,dart}` symlink(기존
  npm-global bind mount 로 영속 — Gradle 과 동일 패턴). 재실행 시 fetch/reset 로 갱신.
  `manifest.yml` 의 `flutter:` 블록(channel/version)으로 핀 가능.
- i18n 키 9개(en/ko) — `env.comp.flutter.*`, `env.size.flutter`, `env.action.flutter*`,
  `probe.flutter.*`.

### Changed

- **이미지 크기 0 증가** — Flutter 는 manifest 런타임 볼륨 설치 방식(Android SDK/Gradle 과 동일).
  Dockerfile 슬림 정책 유지. "모두 설치"(install-all)에서는 **제외** — 선택적 컴포넌트(~2.5GB)라
  Kotlin 사용자에겐 불필요, 개별 카드 버튼으로만 설치.

### Notes

- 설치 후 디스크 검증 권장: `du -sh /home/vibe/.local/flutter`.
- 본 변경은 코드/스크립트만 — 동작하려면 새 이미지 빌드·배포 필요(`flutter.sh`/`vibe-doctor`/
  `manifest.yml` 이 `/opt/vibe-doctor` 로 COPY 됨).
- Wire/DB 변경 **없음**(컴포넌트는 동적 리스트, `envSetupInstall(id)` 가 임의 id 수용) →
  vibe-coder-android 동기 불필요.

## [1.123.1] - 2026-06-11 — 자동 /compact 임계 700K → 500K

### Changed

- `claude.autoCompactTokens` 기본값 **700000 → 500000**. 자동 /compact 가 더 일찍·자주
  실행돼 컨텍스트를 작게 유지한다 → 긴 turn 의 매 step cache_read 누적(컨텍스트 × step 수)이
  줄어 토큰 소비 감소. 임계 순서: warn(350K) < **compact(500K)** < critical(600K) <
  sessionReset(권장 900K). 운영 `.env` 의 `VIBECODER_CLAUDE_AUTO_COMPACT_TOKENS` override 가
  있으면 그쪽 우선(현재 운영엔 미설정 → 본 기본값 적용).

## [1.123.0] - 2026-06-11 — 세션 길이 캡 + 2단계 컨텍스트 포화 경고 (토큰 폭증 차단)

운영 콘솔에서 "한 turn에 세션 quota ~20%, 수 회" 대량 소비가 관측됨. 원인은 버그가 아니라
**장기 `--resume` 세션의 컨텍스트 포화**: 세션을 며칠씩 이어가면 컨텍스트가 1M 윈도우 가까이
(관측 max 104만 토큰) 차오르고, 긴 agentic turn 의 매 step 마다 그 거대 prefix 가 cache_read 로
**누적**(컨텍스트 × step 수)되어 한 turn 에 수천만 토큰이 든다. 자동 `/compact`(v1.108.0)는
turn **경계**에서만 동작 + 같은 세션을 이어가므로 다시 차오른다. 이를 끊는 근본 차단막을 추가.

### Added

- **세션 길이 캡 (자동 새 세션)** — `ClaudeSessionManager.maybeSessionCapOrCompact`. turn 정상
  종료 후 직전 cache_read 가 `claude.sessionResetTokens` 초과 **또는** 세션 정상 완료 turn 수가
  `claude.sessionTurnCap` 도달 시 자동으로 `startNew()`(savedId 폐기 → 다음 prompt fresh,
  컨텍스트 완전 리셋). `/compact` 와 달리 누적을 0 으로 끊는다. 작업 파일·CLAUDE.md 는 유지.
  **둘 다 기본 0(비활성)** — 맥락 손실이 크므로 운영자가 의도적으로 켠다(권장 `sessionResetTokens=900000`).
  자동 /compact settle turn 은 카운트/판정에서 제외(루프 방지).
- **2단계 컨텍스트 포화 경고** — 기존 1차(`contextWarnTokens`, `context_large`)에 2차
  `contextCriticalTokens`(`context_critical`, 기본 **600000 활성**) 추가. CRITICAL 도달 시 더 강한
  문구로 즉시 '새 세션' 을 권한다. 세션당 각 1회(`criticalWarned`/`contextWarned`).
- 설정 3개 신규(`server.yml` `claude` 섹션 + env): `contextCriticalTokens`(기본 600000),
  `sessionResetTokens`(기본 0), `sessionTurnCap`(기본 0). env: `VIBECODER_CLAUDE_CONTEXT_CRITICAL_TOKENS`
  / `VIBECODER_CLAUDE_SESSION_RESET_TOKENS` / `VIBECODER_CLAUDE_SESSION_TURN_CAP`.

### Wire change

**No** — 경고/리셋 알림은 기존 `WsFrame.ConsoleSystem` 프레임의 새 code(`context_critical` /
`session_auto_reset`)로 전달. 프레임 구조/`ApiPath`/DTO 불변. Android `shared/` 동기 불필요
(클라이언트는 system 카드로 렌더, unknown code 안전).

## [1.122.1] - 2026-06-10 — 콘솔 상태칩 "유휴" 오표시 회수 (재연결 busy 동기화)

### Fixed

- **콘솔 페이지 상단 상태칩이 응답 중에도 "유휴"로 표시되던 버그.** 프로젝트
  목록/대시보드는 `projectStatus()`(서버 in-memory `busyStateOrNull`, SSOT)로
  정확히 "응답중"을 그렸으나, 콘솔 칩/통합 탭 헤더 칩(`#console-busy-badge`)은
  클라이언트 `inFlight` 변수에만 의존했다. `inFlight` 는 ① 로컬 prompt 전송
  ② WS `console_busy_state`(busy=true) 수신 두 경로로만 켜지는데, 이 프레임은
  `setBusy()` 의 상태 **전이 시점에만** emit 돼 LogHub ring buffer 에 적재된다.
  도구 호출이 많은 긴 turn 이면 busy=true 프레임이 ring sliding window 밖으로
  밀려나, 새로고침·탭전환·재연결 시 replay slice 에 그 프레임이 없어
  (assistant/tool 프레임은 `inFlight` 를 켜지 않음) 콘솔 칩이 "유휴"로 고착됐다.
  - **수정**: WS console 구독 시 replay 직후 서버 in-memory `busyStateOrNull(id)`
    를 1회 무조건 push(`WsRoutes.handleConsoleStream`). ring window 와 무관하게
    프로젝트 목록과 동일한 SSOT 로 수렴. `null`(부팅 직후 turn 이력 없음)이면
    클라이언트 기본값 `ready` 유지.
  - sub-agent 콘솔은 `ConsoleBusyState` 를 emit 하지 않아 영향 없음(수정 불필요).
  - 메모리 기록 "콘솔 history 복원 최근 offset 함정 (v1.104.2)" 과 동일 계열의
    ring window 의존 문제.

### Wire change

**No** — 기존 `WsFrame.ConsoleBusyState` 를 연결 시 1회 더 보내는 **동작** 변경뿐.
프레임 구조/`ApiPath`/DTO 불변. vibe-coder-android `shared/` 동기 불필요(클라이언트가
이미 처리하는 프레임이며, 안드로이드는 status REST 폴링으로 별도 도출).

## [1.122.0] - 2026-06-09 — 프로젝트 이름 변경 JSON API (P2)

정밀점검 P2(기타 SSOT 갭) 중 가장 체감 큰 항목 — SSR 전용이던 프로젝트 표시 이름 변경을
JSON 노출. android 에서 프로젝트 이름 정리 가능.

### Wire change

**Yes** (additive) — `shared/ApiPath.kt` 신규 1개 + DTO 1파일.

- `projectRename(id)` → `POST /api/projects/{id}/rename` (body `ProjectRenameRequestDto {name}`,
  응답 갱신된 `ProjectDto`). 폴더/패키지 무관(표시 이름만). `ProjectService.rename` 재사용.
- 신규 DTO `ProjectRenameDto.kt`.

### Added

- `ProjectRoutes` 에 `rename` 라우트. requireApiWrite + requireProjectAcl. 빈 이름 400.

### P2 잔여(후속 cycle)

- 파일 작업(new/delete/rename/move/copy/zip), automation cron 예약, dependency audit 의
  JSON 노출은 각각 별개 기능 — 본 cycle 은 rename 으로 대표 처리, 나머지는 후속.

### 동기 메모

- vibe-coder-android v0.42.0 에서 catch-up(ProjectList 오버플로 "이름 변경").

## [1.121.0] - 2026-06-09 — Google Play 업로드 JSON API (P1-2, 빌드→배포 완결)

정밀점검 P1-2: SSR 전용이던 Play 업로드 트리거를 JSON 노출. android 가 Release AAB 빌드
(v1.118.0)에 이어 모바일에서 바로 Play Console 업로드까지 완결.

### Wire change

**Yes** (additive) — `shared/ApiPath.kt` 신규 1개 + DTO 1파일.

- `playUpload(projectId)` → `POST /api/projects/{id}/play-upload` (body `PlayUploadRequestDto`
  `{aabPath?, track?, releaseNotes?}`, 응답 `StoreUploadResponseDto`(202)). 서버가 Claude
  콘솔에 "이 .aab 를 Play <track> 에 업로드" 프롬프트 전송(google-play-publisher MCP).
  진행은 콘솔 WS 로. aabPath 기본 = Release AAB 빌드 산출물.
- 신규 DTO `PublishDtos.kt`(PlayUploadRequestDto/StoreUploadResponseDto).

### Added

- `BuildRoutes` 에 `play-upload` 라우트 + `playPublishService` 파라미터(Module wire).
  기존 `PlayPublishService.trigger` 재사용 — SSR `/builds/{buildId}/play-upload` 와 동일 동작.
- TestFlight(iOS)은 android 클라이언트 대상 외라 제외. 키스토어 업로드는 운영자 서버 관리
  정책(CLAUDE.md)이라 클라이언트 노출 제외.

### 동기 메모

- vibe-coder-android v0.41.0 에서 catch-up(BuildScreen Play 업로드 다이얼로그).

## [1.120.0] - 2026-06-09 — 콘솔 토큰/모델 설정 JSON API (Android 노출)

정밀점검 P0-1: SSR 전용이던 콘솔 토큰 비용 컨트롤(v1.106.0 "토큰 사용량 급소모 개선")의
JSON API 노출 → android 콘솔에서도 모델 선택·자동 compact·MCP 최소화 가능.

### Wire change

**Yes** (additive) — `shared/ApiPath.kt` 신규 4개 + DTO 1파일. vibe-coder-android 동기 필요.

- `consoleSettings`(GET → ConsoleSettingsDto), `consoleModel`(POST ConsoleModelRequestDto),
  `consoleMcpStrict`/`consoleAutoCompact`(POST ConsoleToggleRequestDto) — 모두
  `/api/projects/{id}/claude/console/*`. set 계열은 적용 후 갱신된 ConsoleSettingsDto 응답.
- 신규 DTO `ConsoleSettingsDtos.kt`(ConsoleSettingsDto/ConsoleModelOptionDto/
  ConsoleModelRequestDto/ConsoleToggleRequestDto).

### Added

- `ConsoleRoutes.kt` 에 4개 라우트 추가 — 기존 `ClaudeSessionManager` 읽기/쓰기 메서드
  (`readProjectModel`/`effectiveModel`/`isAutoCompact`/`isMcpStrict` + `setProjectModelAndRestart`/
  `setAutoCompact`/`setMcpStrictAndRestart`) 재사용. SSR `/projects/{id}/console/*` 와 동일 동작.
  모델 목록은 기본+sonnet/opus/haiku 4종.

### 동기 메모

- vibe-coder-android v0.39.0 에서 catch-up(콘솔 설정 시트).

## [1.119.0] - 2026-06-09 — 프로젝트 아카이브 + 품질(Lint) JSON API (Android 노출)

정밀점검 Phase 3: SSR 전용이던 두 기능(아카이브 v1.98.0, 품질·접근성 검사 v1.116.0)의
JSON API 를 §8.A SSOT 로 추가 노출 → vibe-coder-android 가 소비. 인스트루먼트 테스트
(에뮬레이터=서버측 화면)는 android 대상에서 제외, Lint(정적 분석)만 노출.

### Wire change

**Yes** (additive only) — `shared/ApiPath.kt` 신규 6개 + DTO 2파일. vibe-coder-android `shared/` 동기 필요.

- 아카이브: `ARCHIVES`(GET 목록), `projectArchive`(POST, idle 가드), `archiveRestore`(POST),
  `archiveDelete`(DELETE), `archiveDownload`(GET). 응답 `ArchivedProjectDto`(신규).
- 품질: `qualityLint`(POST `?module=app` → `LintResultDto` 신규), `qualityFix`
  (POST `QualityFixRequestDto` → `QualityFixResponseDto`, 선택 이슈를 콘솔로 수정요청).
- 신규 DTO 파일: `ArchiveDtos.kt`(ArchivedProjectDto), `QualityDtos.kt`
  (LintIssueDto/LintResultDto/QualityFixRequestDto/QualityFixResponseDto).

### Added

- `JsonArchiveRoutes.kt` — SSR `archiveRoutes` 와 같은 `ProjectArchiveService` 공유(Module 에서
  hoist). 아카이브는 폴더 rename 과 동일 `isProjectIdle` 가드(동작 중 409 `project_busy`).
- `JsonQualityRoutes.kt` — `LintQualityService.lint` 동기 실행 + 결과 JSON. fix 는
  `buildLintFixPrompt`(internal 승격) 재사용해 `sessionManager.sendPrompt` 로 콘솔 전송.
- i18n: `api.common.archiveNotFound` / `api.common.archiveFailed` / `api.quality.noSelection` (en/ko).

### 동기 메모

- vibe-coder-android v0.38.0 에서 catch-up(ApiPath 6 + DTO 2파일 + Archive/Quality 화면).

## [1.118.0] - 2026-06-09 — Release(APK)/AAB 빌드 JSON endpoint (ApiPath SSOT, Android 노출)

vibe-coder-android 정밀점검(짝 리포 `docs/inspection/2026-06-09-server-wire-ui-audit.md`)에서
드러난 격차: v1.107.0 의 Release/AAB 빌드가 SSR 폼(`POST /projects/{id}/builds/{release|bundle}`)
으로만 존재해 Android(JSON 클라이언트)가 호출할 endpoint 가 없었다. §8.A SSOT 룰에 맞춰 JSON
endpoint 를 추가 노출한다.

### Wire change

**Yes** (additive only) — `shared/ApiPath.kt` 신규 2개. vibe-coder-android `shared/` 동기 필요.

- `buildRelease(projectId)` → `POST /api/projects/{id}/build/release` (assembleRelease).
- `buildBundle(projectId)`  → `POST /api/projects/{id}/build/bundle` (bundleRelease, .aab).
- 양쪽 모두 body 없음, 응답 `BuildDto`(202 Accepted). 동작/가드는 기존 SSR 폼과 동일
  (`BuildService.enqueueRelease`/`enqueueBundle` 재사용 — 키스토어 미존재 시 409 `keystore_required`).
  debug endpoint(`build/debug`)와 대칭.

### Added

- `BuildRoutes.kt` — `build/release` · `build/bundle` POST 라우트 2개. `requireApiWrite` +
  `requireProjectAcl` 가드는 debug 와 동일. `BuildRow.toBuildDto()` 헬퍼로 3개 응답 매핑 통일.

### 동기 메모

- vibe-coder-android `shared/ApiPath` 동기 필요(같은 cycle 의 android v0.37.0 에서 catch-up).

### Added
- **인스트루먼트 테스트 카드** (품질 탭 ②). `connectedDebugAndroidTest` 를 빌드환경의
  **컨테이너 에뮬레이터**(빌드환경 > Emulator)에서 실행해 Compose UI Test / Espresso /
  **ATF**(Accessibility Test Framework — Accessibility Scanner 동급 동적 접근성) 결과를 수집한다.
  - `InstrumentedTestService` — `EmulatorService.serial` 을 `ANDROID_SERIAL` 로 주입해
    같은 컨테이너 내 에뮬레이터를 결정적으로 타겟(다중 디바이스 시에도 고정). JUnit
    결과 XML(`androidTest-results/connected/**/TEST-*.xml`)을 XXE 차단 DOM 으로 파싱.
    timeout 900 s. 실행 전 이전 결과 XML 을 삭제해 stale 파싱 방지.
  - **실패 테스트 → 콘솔 수정요청**: 실패/에러 케이스를 선택해 실패 메시지 포함
    구조화 프롬프트로 콘솔 세션(`sendPrompt`)에 전송. ATF 위반은 UI 를 접근성 기준에
    맞춰 고치도록 안내.
- **동시 실행 가드** (`GradleRunGuard`). lint 와 connectedAndroidTest 가 같은 프로젝트에서
  동시에 spawn 되어 Gradle 프로젝트 락/에뮬레이터 충돌을 일으키지 않도록 projectId 단위로
  직렬화(이미 실행 중이면 "잠시 후 다시 시도" 안내, 큐잉/블로킹 없음).
- **중첩 모듈 경로 해석**. `:feature:login` 처럼 중첩된 Gradle 모듈명을 `feature/login`
  디렉토리로 변환해 리포트/결과 XML·prep 점검 경로를 정확히 찾는다.
- **테스트 사전 준비작업** 버튼 + **준비 상태 체크리스트**. 모듈 build.gradle 과
  `src/androidTest` 를 정적 점검해 (androidTest 디렉토리 / @Test 소스 / testInstrumentationRunner /
  Espresso·Compose UI Test 의존성 / ATF) 준비 여부를 ✓/✗ 로 표시. "준비작업" 버튼을
  누르면 Claude 가 androidTest 의존성·러너·`AccessibilityChecks.enable()` 샘플 테스트를
  구성하도록 환경 구성 프롬프트를 콘솔로 전송한다. 준비 미완료 시 "테스트 실행" 비활성화.

## [1.116.0] - 2026-06-09 — 품질·접근성 검사 탭 (Android Lint + 콘솔 수정요청)

### Added
- **프로젝트 "품질" 탭** (`/projects/{id}/quality`, 더보기 드롭다운). Google 공식
  정적 분석기 **Android Lint** 를 헤드리스(에뮬레이터 불필요)로 실행해
  접근성(Accessibility)·정확성(Correctness)·보안(Security)·성능(Performance) 이슈를
  검출하고, 카테고리 필터 칩으로 분류해 보여준다. 접근성 이슈를 상단에 우선 노출.
  - `LintQualityService` — `./gradlew :{module}:lintDebug` 실행 후
    `lint-results-debug.xml` 을 XXE 차단 DOM 파서로 파싱. abortOnError 로 인한 비정상
    종료와 무관하게 리포트 XML 존재 여부로 성공 판정. timeout 300 s.
  - **선택 이슈 → 콘솔 수정요청**: 검출된 이슈를 체크박스로 선택해
    "콘솔로 수정요청 보내기" 를 누르면, 파일·위치·접근성 가이드가 포함된 구조화
    프롬프트를 생성해 해당 프로젝트의 Claude 콘솔 세션(`sendPrompt`)으로 바로 전송한다.
- 인스트루먼트 ATF(Accessibility Test Framework, Accessibility Scanner 동급 동적 검사)와
  Compose UI Test 결과 연동은 후속 단계(phase 2)로 예정.

## [1.115.0] - 2026-06-07 — 프롬프트 템플릿 기능 대폭 개선 (다이얼로그·변수·즐겨찾기·가져오기)

### Fixed
- **편집이 동작하지 않던 버그** 수정. SSR `/prompts` 의 편집 버튼이
  `onclick="prefillEdit(..., "큰따옴표문자열", ...)"` 형태라, `jsLitString` 이 내보낸 큰따옴표가
  `onclick="..."` HTML 속성을 조기 종료시켜 핸들러가 깨졌다 → `data-*` 속성 + 위임 리스너로 교체.
- **카테고리명이 자동으로 대문자로 보이던 문제** 수정. 저장 데이터는 멀쩡했고 목록 섹션 헤더의
  CSS `text-transform:uppercase` 가 표시만 대문자로 바꿔 혼란을 줬다 → 제거(입력 그대로 표시).

### Added
- **카테고리 콤보박스** — 기존 카테고리를 `<datalist>` 로 선택 + 새 카테고리 자유 입력 동시 지원
  (SSR 폼·관리 다이얼로그 양쪽). 오타로 인한 카테고리 중복 양산 방지.
- **관리 UI 를 콘솔 내 `<dialog>` 모달로** — 별도 페이지(`/prompts`) 이동 없이 콘솔 입력창 하단
  "관리" 버튼으로 즉시 열기. 목록/생성/편집/삭제를 JSON CRUD(`/api/prompt-templates`)로 페이지
  리로드 없이 처리(`/static/prompt-templates.js`). 기존 `/prompts` 페이지는 폴백으로 유지(버그 수정 반영).
- **변수 플레이스홀더 `{{변수}}`** — 본문에 `{{화면명}}` 같은 치환자를 넣으면 삽입 시 값 입력
  다이얼로그가 떠 채워 넣는다. 재사용성 향상.
- **즐겨찾기 고정 + 사용 빈도** — 자주 쓰는 템플릿을 ★ 고정해 picker 상단 노출. 삽입 시 사용
  횟수 +1 집계 → 고정 → 사용횟수 → 제목 순 정렬.
- **검색/필터** — 관리 다이얼로그에서 제목·카테고리·본문 즉시 검색.
- **복제 / 내보내기·가져오기** — 템플릿 1-click 복제, 전체 JSON 내보내기(백업·공유) + 가져오기
  (병합 또는 전체 교체).
- 백엔드: `PromptTemplate` 에 `pinned/useCount/lastUsedAt` 추가(구버전 JSON 하위호환), JSON CRUD
  엔드포인트(`POST/PUT/DELETE /api/prompt-templates`, `/{id}/use|pin|duplicate`, `/import`),
  `PromptTemplateStoreTest` 회귀 테스트.

## [1.114.1] - 2026-06-07 — 서명 적용 라우트 idle 가드 누락 수정 (배포 전 정밀 리뷰 후속)

v1.114.0 배포 전 독립 적대적 코드 리뷰(3개 차원)에서 발견한 선재 결함 수정. 핵심 변경은
회귀 0건으로 검증됨.

### Fixed
- **`POST /projects/{id}/keystore/apply`(build.gradle.kts 서명 적용) 에 콘솔 idle 가드 누락**
  수정(선재 결함). 이 라우트는 `sessionManager.sendPrompt` 로 콘솔에 서명 적용 프롬프트를
  쏘는데, 형제 라우트(`admob`/`admob/apply`/`upload`)는 모두 `consoleIdle` 가드가 있는 반면
  이 라우트만 빠져 있어, 콘솔이 작업 중일 때 프롬프트가 충돌할 수 있었다(사용자가 처음 보고한
  AdMob idle 가드 버그와 동일 계열). 가드 추가 + 버튼 disabled/안내문(표시=동작 일치, P2 와 동형).

### Changed
- `project-tabs.js` 콘솔 뱃지 폴백 `d.state || 'idle'` → `'ready'` (서버 wire 토큰 통일, P1 후속).
- `BootReconcileTest` automation 케이스에 `finishedAt` 세팅·종료 run 불변 대칭 검증 보강.

## [1.114.0] - 2026-06-07 — 프로젝트/콘솔 상태관리 정밀 리팩토링 (단일 진실·와이어링·고착 방지)

상태관리(유휴/응답중/대기중/중지/에러) 전반 정밀점검 후 4단계로 개선. wire 직렬화 포맷
무변경 → Android 클라이언트 호환 유지.

### Changed
- **(P1) 상태값 단일 진실화** — `shared` 에 `ProjectState` enum 신설(wire 문자열 + busy 파생 +
  i18n 키). 이전엔 `"ready"/"responding"/"waiting"/"stopped"/"error"/"idle"` 문자열이 6개+
  파일에 리터럴로 흩어져(한 곳만 어긋나도 뱃지가 조용히 깨짐) 있었다. `ClaudeSessionManager.
  setBusy(Boolean, String?)` → `setBusy(ProjectState)` 로 단일화(busy 는 `ProjectState.busy`
  에서 파생 → (busy, state) 불일치 footgun 제거). 프론트 `'idle'` 토큰을 서버 wire `'ready'`
  와 통일(CSS 별칭은 방어적으로 유지).
- **(P2) 상태 판정·idle 가드 일원화** — SSR 상태칩이 별도 3-state(`responding/stopped/ready`)
  를 재계산해 WS frame 의 5-state(`waiting/error` 포함) live 진실과 불일치하던 것을, 라이브
  상태(`busyStateOrNull`) 우선 + 재시작 시 대화이력 폴백의 단일 소스로 통일(새로고침해도
  `waiting/error` 정확). idle 가드(`consoleIdle`/인라인 OR/부분식)를 단일 헬퍼 `isProjectIdle`
  (= `!busy && !buildRunning && !automationActive`)로 통합 — 키스토어·AdMob 저장/삭제, 폴더·
  패키지 변경, 아카이브, 폼 표시(`structuralEnabled`)가 모두 공유.

### Fixed
- **(P2) 폼 표시 ≠ 동작 불일치 수정** — 폴더/패키지 변경 폼의 표시 활성 조건(`structuralEnabled`)
  이 `isActive`(자동화 진행 중)를 빠뜨려, **자동화 중일 때 폼은 활성인데 제출은 차단**되던
  불일치를 `isProjectIdle` 공유로 해소.

### Added
- **(P3) 부팅 reconcile 규약 + 회귀 테스트** — "진행중 상태를 갖는 모든 DB 테이블은 부팅 시
  `reconcileOrphans()` 로 종료 처리한다" 규약을 `ServerMain` 에 명문화(신규 테이블 추가 절차
  포함). `BootReconcileTest` 로 Builds·PromptAutomationRuns reconcile 회귀 검증(v1.113.2 의
  빌드 고착 버그 재발 방지).
- **(P4) 상태머신 SSOT 테스트·문서** — `ProjectStateTest`(wire 값 고정·유일성·busy 파생·
  라운드트립·오염 프레임 방어), `ClaudeSessionManager` 에 상태 전이 표 단일 권위 주석.
  회귀 위험이 큰 코어 동시성 엔진은 보수적으로 유지.

## [1.113.2] - 2026-06-07 — 재시작으로 고아가 된 빌드 때문에 idle 가드가 영구 차단되던 버그 수정

### Fixed
- **서버 재시작/크래시로 RUNNING/PENDING 에 고착된 빌드가 idle 가드를 영구 차단하던 버그**
  수정(운영 보고: "local-ai-client 프로젝트가 유휴 상태인데 AdMob 광고 ID 저장 시
  '콘솔이 작업 중입니다 — 유휴 상태가 되면 저장(앱 적용)할 수 있습니다' 에러"). 빌드는
  in-process 큐(`BuildService`)로 실행되므로 서버가 빌드 도중 재시작/크래시되면
  `onSuccess`/`onFailure`/`onCancel` 종료 콜백이 하나도 실행되지 못해 `builds` row 가 영원히
  `RUNNING`/`PENDING` 으로 남았다. 그러면 `isBuildRunning()` 이 영구히 `true` 를 반환해
  `consoleIdle()` 가 항상 false → 키스토어/AdMob 저장·업로드·아카이브 등 모든 idle 가드
  라우트가 무한 차단됐다(서버 재시작으로도 해소 안 됨 — DB 영속 상태).
- **부팅 reconcile 추가**: `BuildRepository.reconcileOrphans()` 신설, `ServerMain` 부팅 시
  automation run / interrupted turn reconcile 과 동일 지점에서 호출. 부팅 직후엔 진행 중인
  빌드가 있을 수 없으므로 모든 `RUNNING`/`PENDING` 빌드를 고아로 보고 `FAILED`
  (`error_message='orphaned_by_restart'`) 로 정리한다.

## [1.113.1] - 2026-06-07 — 응답 중 추가 전송 후 영구 "응답중" 고착 수정

### Fixed
- **응답(turn) 종료됐는데 상태칩이 영구 "응답중" 으로 고착되던 버그** 수정(운영 보고). v1.113.0
  에서 follow-up(응답 중 추가 전송)을 추적하려 도입한 `ProjectSession.queuedUserPrompts`
  카운터가 race 에 취약했다: `sendPrompt` 의 `isFollowUp = isBusy()` 판정과 카운터 증가가
  원자적이지 않아, 그 사이 직전 turn 의 `Done` 이 처리되면(빠른 turn / 응답 종료 직전 전송)
  카운터가 실제 `Done` 개수보다 1 커진다. 그 결과 마지막 turn 의 `Done` 이 와도 "아직 하나 더
  남았다" 며 종료 처리(setBusy(false) 등)를 삼켜 busy 가 영구 true 로 남았다.
  - **종료 카운팅 메커니즘 자체를 제거.** follow-up 은 stdin write 만 하고, 각 turn 의 `Done` 은
    항상 정상 처리한다. busy 상태는 기존 자발적 재개 경로(다음 turn 첫 활동 프레임 도착 시
    `setBusy(true)`)가 정확히 관리하므로 카운팅이 불필요했다(애초에 "추가 프롬프트 1개 =
    추가 Done 1개" 가정이 깨지기 쉬웠다).
  - 영향: follow-up turn 사이에 busy 가 매우 짧게(수십 ms) idle 로 깜빡일 수 있으나, 다음 turn
    활동 프레임에서 즉시 복구된다. 영구 고착(치명적) 대비 무해.

## [1.113.0] - 2026-06-06 — 응답 중 전송 = TUI 동형(클라이언트 대기열 제거)

### Changed
- **콘솔 응답(turn) 중 프롬프트 전송 시, 클라이언트 인위적 "대기열"을 제거**하고 TUI 와 동일하게
  동작하도록 변경(사용자 요청). 기존엔 응답 중 전송하면 브라우저 JS 가 `pendingPrompts` 큐에
  쌓고 "대기열에 추가했습니다" 시스템 메시지 + 상태칩 `(대기 N)` 을 띄운 뒤 turn 종료 후 발사했다.
  이제는 응답 중에도 바로 서버로 전송하고, 서버가 진행 중 turn 에 follow-up 으로 이어붙여
  **stream-json CLI 내부 큐가 순차 처리**한다(Claude Code TUI 에서 Esc 없이 Enter 한 것과 동형).
  - 서버: `ClaudeSessionManager` 가 `sendPrompt` 시 `isBusy` 면 follow-up 으로 판정 →
    새 gate permit/turn 마크/bg 추적 초기화를 하지 않고 `ProjectSession.queuedUserPrompts`
    카운터만 증가. `Done` 이 와도 이 카운터가 양수면 종료 처리(gate.release / setBusy(false) /
    fireTurnDone / clearTurnActive)를 **보류**하고 1 감소(백그라운드 작업 `outstandingBgTasks`
    suspended 패턴과 동형). interrupt(중지·끼어들기) 시 카운터 리셋.
  - 클라이언트: `pendingPrompts` 큐 + 큐 관련 시스템 메시지/상태칩/`vibeClearQueue` 전면 제거.
    미사용 i18n 키(`console.queue.*`, `console.busy.responding.queued`) 정리.
  - 근거: stream-json 입력 모드는 turn 중 메시지를 mid-turn 합류시키지 않고 CLI 내부 큐로
    받아 turn 종료 후 처리함이 실측됨(v1.112.0). 따라서 클라이언트 큐는 불필요한 중복 레이어였다.
  - 기존 "끼어들기(↦)" 버튼(현재 turn 중단 후 즉시 전송)과 일반 전송(이어붙이기)의 역할이
    명확히 분리된다.

## [1.112.0] - 2026-06-06 — 콘솔 "끼어들기"(turn interrupt) + 입력창 상단 이동

### Wire change

**Yes** — `shared/ApiPath.kt` 신규 endpoint 1개. vibe-coder-android `shared/` 동기 필요.

- 신규: `claudeConsoleInterrupt(projectId)` → `POST /api/projects/{id}/claude/console/interrupt`.
  - 요청 body: `PromptRequestDto`(기존 prompt 와 동일), 응답: `PromptAcceptedDto`(202).
  - 진행 중 turn 을 interrupt 로 중단하고 곧바로 새 prompt 를 전송(끼어들기). 진행 중이 아니면
    일반 prompt 와 동일 동작.
- `claudeConsoleCancel(projectId)` — **엔드포인트/요청 형태 불변**, 내부 구현만 변경(아래 Changed).
  구버전 Android 클라이언트도 그대로 동작(호환 유지).

### Added
- **콘솔 "끼어들기" 버튼** — 응답(turn) 진행 중에만 입력창 옆에 노출(↦ Interrupt). 클릭 시
  진행 중 turn 을 즉시 중단하고 입력창 내용을 새 prompt 로 전송한다. Claude Code TUI 의
  "Esc → 새 입력 타이핑" 과 동형. 같은 세션/컨텍스트를 유지(프로세스 재기동 없음).
  - 서버: `ClaudeSessionManager.interruptTurn()` / `interruptAndSend()`,
    `ConsoleRoutes` 의 `console/interrupt` 라우트, `console.interrupt*` i18n(en/ko).
  - 동작 근거: stream-json 입력 모드는 turn 진행 중 들어온 user 메시지를 **mid-turn 합류시키지
    않고 turn 종료까지 큐잉**한다(실측 확인, CLI 2.1.167). 따라서 진짜 "끼어들기" 는 control
    프레임 `{"type":"control_request","request":{"subtype":"interrupt"}}` 로 turn 을 abort 해야 가능.
  - 일반 전송(Enter)은 기존대로 큐잉(TUI 에서 Esc 없이 Enter 한 것과 동형) — 동작 변화 없음.

### Changed
- **콘솔 '중지(■)' 버튼이 SIGTERM(프로세스 kill) → control_request interrupt 방식으로 교체.**
  같은 프로세스·같은 세션에서 turn 만 abort(cold start 제거, `--resume` 불필요). interrupt
  전송 실패 또는 watchdog(5초) 내 미종료 시 기존 SIGTERM 으로 자동 폴백. 사용자 가시 동작은
  동일(중지 시 "중단됨" 상태)하되 더 빠르고 컨텍스트가 보존된다.
  - interrupt 로 인한 `result(error_during_execution)` 는 "에러"(빨강)가 아니라 "중단됨"(보라)
    으로 정리하고, 자동화에는 완료(turnDone) 가 아닌 interrupt 로 통지(다음 프롬프트 자동
    발사 방지).
- **콘솔 입력창을 메시지 영역 바로 아래로 이동**(사용자 요청). 기존에 입력창 위에 있던
  빠른프롬프트 바와 도구 바(세션/모델/MCP/필터/자동스크롤)를 모두 입력창 하단으로 재배치.
  순서: 메시지 → 입력창 → 빠른프롬프트 → 템플릿/에이전트 → 도구 바.

## [1.111.3] - 2026-06-06 — gradlew 실행 권한 누락(error=13) 빌드 실패 수정

### Fixed
- **`./gradlew` 실행 시 `error=13, Permission denied`(EACCES)로 빌드가 실패하던 문제** 수정.
  프로젝트의 gradlew 가 존재하되 실행 비트(+x)가 없으면(clone/zip 해제/파일 직접 생성 시 umask·
  전송 방식에 따라 누락) POSIX 에서 `./gradlew` 가 실행 거부됐다. 기존엔 wrapper bootstrap(신규
  생성) 경로에서만 chmod +x 하여, **이미 존재하는 gradlew** 는 권한 보장이 없었다.
  - `GradleBuilder`: 매 빌드 직전 `ensureExecutable()` 로 gradlew 에 owner/group/other +x 를
    보장(idempotent, 이미 +x 면 no-op). 비-Windows 에서만 적용.

### Fixed
- **tool 결과가 에러일 때 해당 turn 이 대화 이력 DB 에 적재되지 못하고 유실되던 버그** 수정.
  `ConversationHistoryService` 가 `role="tool_result_error"`(17자)를 쓰는데 `conversation_turns.role`
  컬럼이 `varchar(16)` 이라 `exceeds length (17 > 16)` 로 persist 실패(WARN)했다.
  - `Schemas.kt`: `role` 컬럼 `varchar(16) → varchar(32)`.
  - `Database.kt`: 기동 시 idempotent `ALTER TABLE conversation_turns ALTER COLUMN role TYPE
    varchar(32)` 추가(`createMissingTablesAndColumns` 는 기존 컬럼 길이를 넓히지 않음).
  - 비파괴 변경 — 기존 데이터/읽기 코드(5곳) 영향 없음.

## [1.111.1] - 2026-06-06 — 콘솔 헤더↔콘솔 박스 상단 여백 추가

### Fixed
- **콘솔 상단 헤더와 콘솔 박스가 margin 0px 로 붙어 있던 문제**(사용자 요청). 하단(콘솔 박스↔
  하단 바 버튼) 여백과 동일하게 `.console-log-wrap` 에 `margin-top: 4px`(= 콘솔 margin-bottom 2px
  + 하단 바 padding-top 2px) 적용.

## [1.111.0] - 2026-06-06 — Todo·백그라운드 작업 패널을 우측 오버뷰(컨텍스트 하단)로 이동

### Changed
- **콘솔 입력창 위의 Todo 요약 패널 + 백그라운드 작업 패널을 우측 오버뷰 rail(컨텍스트 카드 하단)로
  이동**(사용자 요청). 자동화 이동(v1.109.0)과 동일한 postMessage 스냅샷 패턴.
  - 콘솔(`WebProjectTemplates`): `#todo-panel` / `#bg-tasks` HTML·CSS·DOM 렌더 로직 제거.
    `todoStore`/`bgTasks` 상태는 계속 계산하되, 변경 시 스냅샷 HTML 을 `vibe:todo` /
    `vibe:bgtasks` postMessage 로 부모에 전달만 한다(DOM 카드 생성 → 데이터 스토어 + broadcast).
  - 오버뷰(`ProjectTabsTemplate`): 컨텍스트 카드 하단에 Todo 카드(`pt-todo-card`)·백그라운드 작업
    카드(`pt-bg-card`) 신설. 내용 있을 때만 표시, rail 접기/펼치기(v1.110.0) 적용.
  - 부모 JS(`project-tabs.js`): `vibe:todo`/`vibe:bgtasks` 스냅샷을 카드에 렌더.
  - 접기/펼치기는 콘솔 내부 패널 토글 대신 rail 카드 헤더가 담당(중복 제거).
  - 단독 콘솔 페이지(비임베드)에는 Todo/백그라운드 패널이 없어진다(오버뷰에서 확인).

## [1.110.0] - 2026-06-06 — rail 카드 접기/펼치기 + 콘솔 세션/모델/MCP 하단 이동

### Added
- **우측 오버뷰 rail 카드 접기/펼치기**(사용자 요청). 개요·컨텍스트·프롬프트 히스토리·자동화·메모
  카드의 헤더(`.pt-rail-h`)를 클릭하면 본문을 접고 펼친다. 좌측 캐럿(▾/▸)으로 상태 표시,
  카드별 상태는 `localStorage`(`vibe.rail.collapsed.<card>`)에 영속. 헤더 내 액션 버튼/입력
  (＋, /compact, 자동 체크박스 등) 클릭은 토글에서 제외.

### Changed
- **콘솔 헤더의 세션 표시 + 모델 셀렉터 + MCP 최소화 토글을 콘솔 하단 바(메시지 필터 버튼 좌측)로
  이동**(사용자 요청). 헤더 우측에는 외부 링크/중지/새 세션만 남는다. 하단 바는 폭 증가에 대비해
  `flex-wrap` 허용.

## [1.109.0] - 2026-06-06 — 프롬프트 자동화를 우측 오버뷰(메모 위) 별개 기능으로 이동

### Changed
- **프롬프트 자동화 UI 를 콘솔 입력창 위 인라인 패널 → 우측 오버뷰 rail(메모 카드 위)로 이동**
  (사용자 요청 — 별개 기능으로 분리, 메인 프롬프트 textarea 와 무관한 자체 입력으로 실행).
  - 콘솔(`WebProjectTemplates`): 인라인 `#auto-panel` HTML + `initAutomation` JS 전부 제거.
    `automation_progress` WS 프레임은 부모로 `vibe:automation` postMessage 포워딩만 한다.
  - 오버뷰(`ProjectTabsTemplate`): 메모 카드 위에 `.pt-auto-card` 신설(모드 repeat/sequence,
    횟수, 프롬프트 textarea, 시작/중지, 프리셋, 관리 링크). 진행 중엔 진행률 + 중지 버튼.
  - 부모 JS(`project-tabs.js`): `/api/projects/{id}/claude/automation/{start,stop,status}` 직접
    호출 + 활성 시 status 3초 폴링(폴백). 콘솔이 포워딩한 `vibe:automation` 프레임으로 즉시 갱신.
  - REST 계약·서버 자동화 매니저는 변경 없음 — UI 위치만 이동.
  - 단독 콘솔 페이지(비임베드)에는 자동화 UI 가 없어진다(오버뷰 rail 또는 자동화 탭 사용).

## [1.108.4] - 2026-06-06 — 상태칩 정리 + 마이크 Material 아이콘 + AdMob 상태 행

### Changed
- **전송/줄바꿈 힌트 좌측 상태칩(busy-badge) 숨김**(사용자 요청). turn 상태는 콘솔 하단
  '응답중' 스피너와 부모 탭 헤더(`#console-busy-badge`)로 충분히 노출되어 힌트 좌측 중복 칩을
  제거. JS(부모 미러)가 dataset/text 를 계속 읽으므로 element/id 는 유지하고 display 만 끔.
- **음성 입력 마이크 아이콘을 이모지(🎤) → Google Material 'mic' 인라인 SVG 로 교체**(외부 CDN
  미사용 §3). 녹음 중 상태는 `.listening` 클래스(빨강+pulse)로 표시하도록 voice-input.js 의
  textContent 이모지 스왑 제거.
- **개요카드에 AdMob 준비 상태 행 추가**(키스토어 하단). `<pkg>-admob.properties` 존재 시
  '✓ 준비됨', 없으면 '✗ 미준비'. 키스토어와 동일한 1회 `keystoreService.get()` 으로 함께 계산.

### Removed
- **음성 입력 '자동 전송' 체크박스 제거**. 발화 종료 시 form 자동 submit 하던 옵션으로, 음성
  입력(🎤) 사용 시에만 의미 있는 보조 기능이었다. 사용자 검토 결과 불필요로 판단 — 음성 입력은
  받아쓰기만 하고 전송은 사용자가 직접 한다. voice-input.js 의 autoSend 로직도 함께 제거.

## [1.108.3] - 2026-06-06 — 콘솔 본문↔하단 바 여백 축소

### Changed
- **콘솔 본문과 하단 상태 바(응답중·메시지 필터·자동 스크롤) 사이 여백 축소**(사용자 요청).
  `.console-log-wrap` 의 `margin-bottom` 16px→2px, `.console-bottom-bar` 상단 패딩 8px→2px
  (min-height 30→26, margin-bottom 12→10)로 바가 콘솔 하단에 바짝 붙도록 조정.

## [1.108.2] - 2026-06-06 — 콘솔 한도 메시지 정규화 + 필터/자동스크롤 토글 하단 이동

### Fixed
- **사용량/요금 한도 종료가 "success: …" 빨간 버블로 중복 노출되던 문제** 근본 수정.
  5시간 롤링 윈도우·월 한도 등으로 turn 이 끝나면 Claude CLI 가 `result` 프레임을
  **`subtype:"success"` + `is_error:true`** 라는 모순 조합으로, 그것도 `result` 필드에
  **직전 assistant 본문(한도 안내문) 복사본**을 실어 보낸다. 기존엔 `code=subtype="success"`
  가 그대로 실려 청록 assistant 버블 바로 아래에 **"success: <동일 본문>"** 빨간 에러 버블이
  글자까지 중복됐다.
  - `ClaudeStreamParser.parseResult`: 한도 메시지(`isUsageLimitMessage`)를 감지해
    `code="usage_limit"` + 명료한 한국어 안내 한 줄로 대체(원문 echo 제거 → 중복 제거).
    한도가 아닌데 `subtype="success"+is_error` 인 모순 프레임은 `code="error"` 로 정규화해
    "success:" 오인을 제거. 일시 429(rate limit)는 기존 자동 재시도 경로가 처리하도록
    한도 판정에서 제외.
  - `ClaudeSessionManager`: `usage_limit` 종료는 크래시성 '에러'(빨강)가 아니라
    '중단됨'(stopped, 보라) 상태칩으로 표시하고, 자동화엔 완료가 아닌 `fireInterrupt` 로
    통지해 다음 프롬프트 자동 발사(한도 재충돌)를 막는다.
  - 콘솔 렌더러: `console_error` 의 `code==='usage_limit'` 은 빨간 버블 대신 명료한
    '🛑 한도' 시스템 카드로 노출.
  - 회귀 테스트 `ClaudeStreamParserResultTest` 추가(한도 정규화·모순 프레임·rate limit 비오분류).
  - 참고: 한도 문구 자체("You've hit your monthly spend limit" 등)는 CLI/Anthropic 이 보낸
    원문으로 서버는 가공하지 않는다 — 실제 5시간 세션 한도를 CLI 가 일반화된 문구로 표기한 것.

### Changed
- **콘솔 상단의 '메시지 필터' 버튼 + '자동 스크롤' 토글을 콘솔 하단으로 이동**(사용자 요청).
  콘솔 본문 바로 아래 `.console-bottom-bar` 한 줄에 배치 — 좌측은 기존 '응답중' 스피너
  (`#console-spinner`, busy 일 때만 노출), 우측은 필터/자동스크롤 토글을 `margin-left:auto`
  로 항상 우측 정렬. 스피너가 숨겨져 있어도 토글은 우측에 고정된다.
  - 트리거 버튼/토글 markup 만 이동했고 ID(`#filter-open` / `#autoscroll-toggle` /
    `#filter-summary`) 와 JS 핸들러(`getElementById`)는 그대로라 동작 변동 없음.
  - 필터 모달(`#filter-modal`)과 그 스타일은 fixed(위치 독립적)라 변경 없음.
  - autoscroll `<label>` 에 `margin:0` 적용 — v1.108.1 에서 다룬 전역 `label{margin-bottom:16px}`
    상속에 의한 flex 중앙정렬 밀림을 새 하단 바에서도 예방.

## [1.108.1] - 2026-06-06 — 라벨형 컨트롤 정렬 근본 수정(전역 label margin)

### Fixed
- **콘솔 헤더 'MCP 최소화' 토글 + 파일 트리 '업로드' 버튼이 위로 틀어지던 정렬** 근본 원인 해결.
  admin.css 전역 `label { margin-bottom: 16px }` 가 **`<label>` 기반 컨트롤**(MCP 토글·업로드
  label.chip·자동 compact 라벨)에 상속돼, flex `align-items:center` 에서 마진박스가 중앙정렬되며
  내용이 위로 ~8px 밀렸다(버튼/select 는 label 이 아니라 정상). 해당 라벨들에 `margin:0` 적용으로
  한 줄 정렬. (정밀 점검: 증상이 정확히 label 류에서만 발생한 이유 = 전역 label margin.)

## [1.108.0] - 2026-06-06 — 자동 /compact 토글 + 키스토어 탭 승격 + 콘솔 헤더 MCP 정렬

### Added
- **자동 /compact (auto-compact) 토글.** 우측 오버뷰 컨텍스트 카드 우상단 `/compact` 버튼 옆에
  "자동" 체크박스(기본 **ON**). ON 이면 Claude CLI 처럼 **컨텍스트가 임계(`autoCompactTokens`,
  기본 700K)를 넘으면 turn 종료 후 서버가 자동으로 `/compact` 실행 후 작업을 이어간다.** OFF 면
  사용자가 직접 `/compact` 버튼을 누를 때까지 compact 하지 않음. 프로젝트별 설정
  (`.vibecoder/auto-compact-off`), 루프 방지(직전 turn 이 /compact 면 1회 skip). env
  `VIBECODER_CLAUDE_AUTO_COMPACT_TOKENS`.

### Changed
- **프로젝트 키스토어/AdMob 탭을 '더보기' 드롭다운 → 상단 탭바(빌드 우측)로 승격.**

### Fixed
- **콘솔 헤더 'MCP 최소화' 토글이 다른 버튼보다 약간 위로 틀어지던 정렬** — 체크박스 크기 고정
  (13×13) + line-height 정리로 한 줄 정렬.

## [1.107.5] - 2026-06-06 — 컨테이너 전역 CLAUDE.md 시드 템플릿(ko/en) 보충

### Changed
- **첫 설치 시드되는 전역 CLAUDE.md 기본 템플릿(`container-global-claude.ko/en.md`)을 운영 라이브
  파일과 동일 수준으로 보충.** 운영 파일에만 있고 템플릿에 누락돼 있던 항목 추가:
  - §2 빌드 테스트 정의 명확화(컴파일·빌드·단위 테스트, 에뮬레이터 제외 — §14)
  - §5 **AdMob 광고 ID 파일 표준 키 스키마**(admobAppId/<type>AdUnitId, 테스트 ID 금지)
  - §11 "에뮬레이터/스크린샷 자율 실행 금지" 항목
  - **§13 Android Compose UI 함정**(WindowInsets 이중 적용 등) / **§14 에뮬레이터 자율 실행 금지**
  - 목차(13·14) 갱신. (en 템플릿은 동일 내용 영문화.)
- 신규 설치(빈 볼륨 first-run) 부터 완전한 규칙으로 시드된다. 기존 설치의 라이브 파일은 영향 없음.

## [1.107.4] - 2026-06-06 — AdMob ID 섹션: 키 스키마 무관 일반 파싱 + 테스트 ID 숨김

### Fixed / Changed
- **키스토어 AdMob ID 섹션이 광고 ID 를 표시하지 못하던 문제(재발) 근본 해결.** 임포트 레포를
  relocate 할 때마다 Claude 가 서로 다른 키 형식을 써서(`bannerAdUnitId` / `release.admob_banner_id`
  / `admob.bannerId` / `admob.test.*` 등) 화이트리스트 파싱이 계속 빗나갔다. → `readAdmob` 를
  **키 이름 무관 일반 파서**로 교체: 값 패턴(`ca-app-pub-…~…`=App ID, `…/…`=unit) + 키 키워드
  (banner/appOpen/native/interstitial/rewarded[Interstitial])로 분류. 어떤 스키마든 인식.
- **테스트 ID 는 섹션에 표시하지 않음.** 키에 `test`/`debug` 포함 또는 값이 Google 공식 테스트
  publisher(`ca-app-pub-3940256099942544`) 면 제외하고 **운영(실) ID 만** 노출.
- (파일은 읽기만 — 프로젝트 build.gradle 호환 위해 덮어쓰지 않음.)

## [1.107.3] - 2026-06-06 — 콘솔 헤더 / 파일 트리 툴바 한 줄 정렬(높이 통일)

### Fixed
- **콘솔 헤더(세션·모델 콤보·MCP 최소화·새 세션)가 컨트롤 높이 차이로 위아래로 틀어져 보이던
  문제.** 모델 select·MCP 토글·버튼을 모두 30px 높이 + 세로 가운데 정렬로 통일.
- **파일 트리 툴바(⬆ 업로드 / ＋ 새 파일 / 📁＋ 새 폴더 / 안내문)도 동일하게 한 줄 정렬.**
  툴바 내 form/chip 높이를 30px 로 통일(스코프 인라인 `<style>` — 정적 css 캐시 무관).

## [1.107.2] - 2026-06-06 — 모델 기본값 'CLI 기본' + 선택 모델 콤보박스 고정(버그 수정)

### Changed
- **전역 기본 모델을 `default`(CLI 기본)로 변경.** "CLI 기본"은 `--model` 을 전달하지 않아
  `claude` CLI 가 자체 설정/계정 기본으로 모델을 직접 선택한다. 모델은 **프로젝트별로** 콘솔
  콤보박스에서 개별 관리(`.vibecoder/claude-model`). (토큰 절감은 프로젝트별 Sonnet 선택 또는
  env `VIBECODER_CLAUDE_MODEL=sonnet`.)

### Fixed
- **모델 콤보박스가 선택값으로 고정되지 않던 버그.** "CLI 기본" 선택 시 `setProjectModel` 이
  override 파일을 **삭제**해 전역 기본(당시 sonnet)으로 폴백 → 콤보박스가 선택과 다른 값(Sonnet)
  으로 표시됨. 이제 "default" 포함 선택값을 파일에 **명시 저장**해 선택이 그대로 고정 표시된다.

## [1.107.1] - 2026-06-06 — 컨텍스트 미터 누적치 오표시(1.7M) 수정 + 빌드 히스토리 시간 표시 + AdMob 외부형식 파싱

### Fixed
- **컨텍스트 점유율 미터가 세션 누적 토큰(예: 1.7M)을 현재 컨텍스트로 잘못 표시하던 버그.**
  `ClaudeStreamParser` 가 `assistant` 메시지(이번 turn 단일 값)와 `result` 프레임(turn 종료
  **누적치**) 양쪽에서 usage 를 emit 하는데, 누적치가 turn 끝에 마지막으로 도착해 per-turn 값을
  덮어써 미터가 윈도우(1M)를 초과하는 값으로 표시됨. `UsageReport.cumulative` 플래그 추가 →
  미터는 cumulative=false(assistant per-turn)만 사용. 과거 오염된 스냅샷은 sanitize(cacheRead >
  limit 이면 숨김)로 방어, 다음 turn 에 정상 값으로 자동 보정. (운영 compass 1.73M 오표시 회수)

### Added
- **빌드 히스토리 시간 표시 단순화**: 시작 시간 `yyyy-MM-dd HH:mm:ss`(24시간), 종료 컬럼을
  **"빌드 시간"(초, 예: 5분 → `300s`)** 으로 변경. i18n `builds.col.duration` (ko/en).
- **AdMob 광고 ID 외부 형식 파싱**: 임포트 레포를 relocate 할 때 흔히 쓰는 debug/release 분리
  snake_case 키(`release.admob_banner_id` 등)도 `readAdmob` 가 인식 → 키스토어/광고ID 섹션에
  표시. 실 운영값(`release.*`) 우선. (프로젝트 build.gradle 호환 위해 파일은 덮어쓰지 않고 읽기만)

## [1.107.0] - 2026-06-06 — 빌드 메뉴에 Release(APK) / AAB 번들 빌드 버튼 추가

### Added
- **프로젝트/빌드 메뉴의 "Debug 빌드 큐 등록" 우측에 "Release 빌드 (APK)" + "AAB 번들 빌드"
  버튼 추가.**
  - Release: `assembleRelease` → 서명된 release APK (`POST /projects/{id}/builds/release`).
  - AAB: `bundleRelease` → Play Console 업로드용 서명된 `.aab` (`POST /projects/{id}/builds/bundle`).
  - 둘 다 키스토어 서명 자동 주입(`-Pandroid.injected.signing.*`) + Debug 와 동일 SSOT 키스토어
    가드(미준비 시 버튼 비활성 + 차단). 산출물은 `<pkg>-release-v<ver>.apk` /
    `<pkg>-release-bundle.aab` 로 저장.
- `BuildService.enqueueRelease`/`enqueueBundle` (공통 `enqueueBuild` 헬퍼로 debug/release/bundle
  통합), `ApkFinder.findLatestReleaseApk`/`findLatestReleaseBundle`,
  `ArtifactService.storeBuildArtifact`(확장자/타입 일반화) 추가.
- i18n: `builds.queue.release`(.hint), `builds.queue.bundle`(.hint) (ko/en).

## [1.106.3] - 2026-06-06 — 컨텍스트를 독립 카드로 분리 + 우상단 /compact 버튼

### Changed
- **컨텍스트 점유율 미터를 프롬프트 히스토리 카드 내부 → 독립 "컨텍스트" 카드로 분리**하여
  우측 오버뷰 rail 의 히스토리 카드 **위**에 배치. 데이터 없을 땐 "대화 시작 후 표시" 안내.

### Added
- **컨텍스트 카드 우상단에 `/compact` 버튼.** 클릭 시 콘솔 iframe 으로 `vibe:send-prompt`
  postMessage → 콘솔 정상 전송 경로로 `/compact` 전송(busy 면 큐, 에코·미터 갱신 일관).
  확인 다이얼로그 + 전송 중 표시. (운영 확인: vibe 비인터랙티브 모드에서 `/compact` 동작)
- i18n: `tabs.rail.context`, `tabs.rail.context.empty`, `tabs.rail.compact.hint` (ko/en).

### Notes
- /compact 후 다음 turn 의 usage 가 줄면 미터가 자동으로 축소 반영(점유/사용/남음).

## [1.106.2] - 2026-06-06 — 컨텍스트 점유율 미터를 우측 오버뷰 rail(프롬프트 히스토리 상단)로 이동

### Changed
- **컨텍스트 점유율 미터를 콘솔 본문 상단 → 부모 ProjectTabs 의 우측 고정 오버뷰 rail
  (프롬프트 히스토리 상단)로 이동배치.** 모든 탭에서 상시 보이고 iframe 스크롤과 무관.
  콘솔 iframe 이 `vibe:context-usage` postMessage 로 매 turn 부모 rail 미터를 갱신
  (busy 뱃지 미러링과 동일 패턴). 페이지 로드 시 직전 turn 스냅샷으로 초기 표시.
- standalone 콘솔(`/projects/{id}/console` 직접) 과 General Chat(`/chat`) 은 부모 rail 이
  없으므로 기존처럼 콘솔 본문 상단에 인라인 미터를 유지(임베드 시에만 rail 로 이관).

### Notes
- 미터는 매 turn 실제 usage 기반이라 `/compact` 등으로 컨텍스트가 줄면 다음 turn 에 자동 반영.
  (운영 확인: vibe-coder 비인터랙티브 모드에서도 `/compact` 슬래시는 동작함 — 대부분의 다른
  슬래시는 미동작.)

## [1.106.1] - 2026-06-05 — 콘솔 컨텍스트 점유율 그래픽 미터(상시) + Claude CLI /context 유사 시각화

### Added
- **콘솔 프롬프트 히스토리 상단에 컨텍스트 점유율 미터 상시 표시.** 매 turn usage 를 분해해
  (`cache_read`=재사용 / `cache_creation`=신규캐시 / `input`=비캐시) 모델 윈도우 한도 대비
  스택 바 + `사용 / 한도 (%) · 남음` 텍스트 + 범례로 표시. 70%↑ 주황 강조.
- 신규 WS 프레임 `console_context_usage` 로 매 turn **live 갱신**. 페이지 로드 시 직전 turn
  스냅샷(`.vibecoder/claude-context-tokens`, 서버 재시작 후에도 복원)으로 초기 렌더.
- 윈도우 한도는 모델로 추정(opus/sonnet 1M, haiku 200K; used 가 base 초과 시 1M 보정).

### Notes
- 미터는 매 turn 실제 usage 로 계산되므로 컨텍스트가 줄면(예: '새 세션' 리셋) 자동 반영된다.
- ⚠ vibe-coder 의 비인터랙티브 stream-json 모드는 Claude Code 슬래시 커맨드(`/compact` 등)를
  **지원하지 않는다**(`/console/slash` 는 v0.75.0 부터 no-op). 컨텍스트 축소는 현재 '새 세션
  (컨텍스트 리셋)' 으로 수행 — 리셋 시 미터가 비워진다. (요약 기반 compact 는 후속 과제)

## [1.106.0] - 2026-06-05 — 토큰 사용량 급소모 개선 (모델 선택·컨텍스트 관리·MCP 최소화·자동재개 절제)

운영 진단: vibe 세션이 전부 **Opus** + `--resume` 로 한 세션 컨텍스트가 **1M 토큰 상한**까지
누적(다수 세션 ~1M cache_read/turn) + 콜드 캐시 재생성으로 토큰을 매우 빠르게 소진. 4개 레버로 개선.

### Changed
- **Claude 기본 모델을 Opus → Sonnet 로 전환** (`claude.model`, 기본 `sonnet`). 동일 작업
  토큰 사용량 약 1/5. `--model` 로 전달하며, env `VIBECODER_CLAUDE_MODEL` / 프로젝트별
  콘솔 셀렉터로 override 가능. (P0-a, 가장 큰 레버)

### Added
- **콘솔 헤더 모델 셀렉터** (Sonnet/Opus/Haiku/CLI 기본). 변경 시 유휴면 즉시 같은 대화를
  새 모델로 resume, busy 면 다음 turn 부터. 프로젝트별 `.vibecoder/claude-model` 저장. (P0-a)
- **컨텍스트 크기 표시 + 임계 경고** (P0-b). 직전 turn `cache_read` 를 콘솔 헤더에 `ctx ~NK`
  배지로 표시(임계 초과 시 주황 ⚠), 임계 초과 시 1회 system 경고로 "새 세션(컨텍스트 리셋)"
  유도. 기존 "새 세션" 버튼으로 리셋 시 누적 토큰/경고도 초기화. `claude.contextWarnTokens`
  (기본 350K, env `VIBECODER_CLAUDE_CONTEXT_WARN_TOKENS`).
- **MCP 최소화 토글** (P1-a). on 이면 전역 `~/.claude/.mcp.json`(playwright 등 5종) 을 무시하고
  (`--strict-mcp-config`) 프로젝트 `.mcp.json`(있으면)만/없으면 0개 → 매 세션 캐시 프리픽스 축소.
  콘솔 헤더 체크박스, 프로젝트별 `.vibecoder/mcp-strict`.

### Improved
- **자동 재개 절제** (P1-b). 컨텍스트가 `contextWarnTokens` 이상이면 부팅 자동 재개 + rate-limit
  자동 재개를 보류(사용자 부재중 거대한 맥락을 반복 과금하는 것 방지) — 안내 후 수동 재개 유도.
- **프로젝트 CLAUDE.md 템플릿 슬림화** (P2). 매 turn 캐시 프리픽스에 들어가는 기본 CLAUDE.md
  를 ~176→~55줄로 압축(모든 실질 규칙 보존, 중복/장황 산문 제거). 신규 프로젝트에 적용.

### Notes
- 기존 대형 세션은 모델만 바꿔도 즉시 절감되나, 누적 컨텍스트(~1M)는 '새 세션' 으로 리셋해야
  per-turn 비용이 크게 줄어듭니다. 전역 `~/.claude/CLAUDE.md`(운영자 15.7KB)도 슬림화 권장.
...
- **`EnvSetupTemplates` 전체 i18n 적용** — 모든 사용자 가시 한국어 문자열
  ...
  - `renderClaudeAuthActions` — 현재는 옵션 0 웹 OAuth 단일 경로만 노출.
...
`docker exec` 터미널 접근이 불가능한 환경에서도 Claude 인증을 완료할 수 있게
웹 OAuth 로그인 경로를 추가했다. 현재 빌드환경 UI는 이 OAuth 경로만 노출한다.

- **UI** — `EnvSetupTemplates.CLAUDE_AUTH` 카드는 현재 옵션 0 웹 OAuth 단일 경로만
  노출한다.
