...
## 18. Phase 9 — iPhone UI 와이어링 (빌드환경 통합 + 진입점 완결)

> 추가일: 2026-07-22. Phase 0~8 백엔드/템플릿 구현(v1.163.0)이 끝난 뒤 **MacBook 로컬
> 설치**로 실사용을 시작하자 드러난 격차를 회수하는 후속 phase. Phase 8까지와 달리 이 절은
> **미착수 상태**(`[ ]`)로 시작한다.

### 18.0 문제 정의 (실사용 발견)

macOS 에 Vibe Coder Server 를 로컬 설치했으나, **빌드환경(`/env-setup`) 페이지에 iPhone 을
위한 항목이 하나도 없다.** Android SDK/Gradle/Flutter/Codex 카드는 보이지만 Xcode, Command
Line Tools, iOS Simulator runtime, SwiftLint/SwiftFormat 을 감지·안내·설치할 카드가 없어,
사용자가 "iPhone 빌드환경을 어디서 준비하는지" 자체를 알 수 없다.

근본 원인 — **백엔드는 있으나 전역 UI 진입점이 비어 있다:**

| 영역 | 백엔드 | 전역 UI 배선 |
|---|---|---|
| per-project iPhone rail (Xcode build/signing/Simulator/TestFlight) | 있음 | **있음** (`ProjectTabsTemplate`, 79 참조) |
| iPhone/macOS preflight (`IosPreflightService`) | 있음 (`/api/ios/preflight`, mode=linux/mac_local/mac_ssh, Xcode/simctl/SDK/codesigning/blockedReason) | **없음** — 어떤 SSR 템플릿도 미호출, JSON 전용 |
| Swift 도구 설치 (`IosSwiftToolsInstallService`) | 있음 (`/api/ios/swift-tools/install`, Homebrew) | **없음** — env-setup 카드 미노출 |
| 빌드환경 컴포넌트(`SetupComponent`) | Java/Android/Gradle/Flutter/Codex/… 15종 | **iOS 계열 0종** |
| EnvSetup 의 Mac(darwin) 인식 | `IosPreflightService` 는 판정, `EnvSetupService` 는 미인식 | **없음** — 전 컴포넌트가 Linux 컨테이너 + vibe-doctor 가정 |

목표: per-project 레일 수준으로 배선된 iPhone 기능을 **전역 빌드환경/진입점까지 이어붙여**,
Mac 로컬 설치 사용자가 "빌드환경 → iPhone 준비 → 프로젝트 → 빌드/시뮬레이터"까지 화면만으로
완결하게 한다.

### 18.1 설계 원칙 (기존 패턴과의 차이)

- **캡슐화 유지**: iPhone 컴포넌트의 detect/install 로직은 `ios` 패키지 / `IPhonePlatformEngine`
  이 소유한다. `EnvSetupService` 는 `SetupComponent` 등록 + 상태 매핑만, `EnvSetupTemplates`
  는 카드 렌더만 담당한다. `if (component == XCODE)` 류 분기가 공통 서비스 전역으로 퍼지지
  않게 한다.
- **표준 env-setup 패턴에서 벗어나는 점을 명시**: 기존 컴포넌트는 `doctorCmd`(vibe-doctor
  서브커맨드) + `lib/<tool>.sh` 로 설치한다. iPhone 계열은 다르다.
  - Xcode / Command Line Tools / Simulator runtime 은 **자동 설치 불가**(App Store /
    `xcode-select --install` / `xcodebuild -downloadPlatform iOS`). → `doctorCmd = null`
    **info-only 카드** + 안내 명령 표시.
  - SwiftLint / SwiftFormat 만 **자동 설치 가능** → vibe-doctor 가 아니라 기존
    `IosSwiftToolsInstallService`(Homebrew) 를 재사용하는 **전용 install action**.
  - detect 는 `lib/*.sh` probe 가 아니라 **`IosPreflightService` 결과를 SSOT** 로 사용.
- **플랫폼 조건부 노출**: iPhone 컴포넌트는 `install-all`(원터치 모두 설치)에서 제외한다.
  Linux Docker 단독 / 비-Mac 에서는 카드를 렌더하되 상태를 `mac_required` blocked 로 표시하고
  설치 버튼을 숨긴다. Mac 로컬(`mac_local`) 또는 원격 Mac agent(`mac_ssh`) preflight 통과
  시에만 조작 버튼을 노출한다 (Phase 5/§12 조건부 노출 규칙과 동일).
- **Wire/DB 무변경 목표**: `SetupComponent` 는 동적 리스트(`byId`/`entries`)이고 env-setup
  JSON(`/api/env/components`)은 목록을 그대로 직렬화하므로, 컴포넌트 추가는 기존 안드로이드
  클라이언트에 additive 하위호환이다. 새 REST 는 이미 있는 `/api/ios/preflight` /
  `/api/ios/swift-tools/install` 을 재사용해 **신규 endpoint 없이** 배선하는 것을 1차 목표로
  한다. (신규가 불가피하면 `ApiPath` 선등록 후 §8.A 절차를 따른다.)

### 18.2 구현 티켓

#### A. 빌드환경(/env-setup) iPhone 섹션 (핵심 — 보고된 격차)

- [x] `EnvSetupService` 에 **실행 모드 인식** 도입:
  - `mac_local` / `mac_ssh` / `linux` 판정을 `IosPreflightService` mode(또는 동일 로직)로
    재사용. `StatusService` 가 이미 `os.name` 을 읽으므로 공통 helper 로 추출.
- [x] `SetupComponent` 에 iPhone 계열 추가 (모두 조건부, `install-all` 제외):
  - [x] `XCODE` — `doctorCmd=null`, info-only. `xcode-select -p` / `xcodebuild -version`
    감지. 미설치 시 App Store 설치 안내.
  - [x] `XCODE_CLT` (Command Line Tools) — info-only. `xcode-select -p` 존재 확인 +
    `xcode-select --install` 명령 안내(비대화식 자동설치 불가).
  - [x] `IOS_SIMULATOR_RUNTIME` — info-only. `xcrun simctl list runtimes` 에서 iOS
    runtime 존재 여부. 미설치 시 `xcodebuild -downloadPlatform iOS` 안내.
  - [x] `SWIFT_TOOLS` (SwiftLint + SwiftFormat) — **자동 설치 가능**. 카드 버튼 →
    기존 `POST /api/ios/swift-tools/install` 호출. Homebrew 미설치는 `homebrew_missing`
    안내.
  - [x] (선택) `COCOAPODS` — `pod --version` 감지, `gem install cocoapods` 안내/설치.
- [x] `EnvSetupService.detect()` 가 iPhone 컴포넌트는 `IosPreflightService` 스냅샷을 진단
  소스로 사용하도록 위임(직접 shell probe 금지, 캡슐화 유지).
- [x] `EnvSetupTemplates.renderAction()` `when` 분기(exhaustive)에 iPhone 컴포넌트 액션 추가:
  info-only 카드는 "감지 결과 + 안내 명령", `SWIFT_TOOLS` 는 설치 버튼.
- [x] iPhone 컴포넌트를 별도 "iPhone (macOS)" 섹션으로 그룹핑. Mac 판정 시 기본 확장,
  Linux 단독이면 접힘 + "Mac 필요" 안내.
- [x] i18n `MessagesEn`/`MessagesKo` 키 추가(`env.comp.xcode.*`, `env.comp.xcodeClt.*`,
  `env.comp.iosSimRuntime.*`, `env.comp.swiftTools.*`, `env.size.*`, `env.section.iphone`).

#### B. iPhone/macOS preflight 를 SSR 로 노출

- [x] 빌드환경 iPhone 섹션 상단에 **preflight 요약 배너**: mode(mac_local/mac_ssh/linux),
  Xcode 버전, `xcode-select` 경로, simctl 사용 가능 여부, iOS/iPad device type 수,
  codesigning identity 수, `blockedReason`.
- [x] iPhone 프로젝트 개요/rail 에도 preflight 상태 카드 노출(현재 JSON only → SSR).
  `blocked` 와 `failed` 를 구분해 표시(§2.7).
- [x] preflight 실패/blocked 시 다음 행동 안내(“Xcode 설치”, “`xcode-select --install`”,
  “Mac agent 연결”)를 카드 안에 인라인.

#### C. 프로젝트 생성 / 타입 UI 완결성 (대부분 배선됨 — 확인·보강)

- [x] 신규 프로젝트 폼의 `iPhone (SwiftUI)` 선택 시, 현재 서버가 Mac 이 아니면
  "이 서버에선 빌드/시뮬레이터 불가 — 생성·AI·파일 편집만 가능" 경고 노출(생성은 허용).
- [x] iPhone 프로젝트 개요에 "iPhone 빌드환경 설정"(`/env-setup#iphone`) 진입 링크 추가.
- [x] `PlatformUiCapabilities` / `ProjectPlatformEngine.uiCapabilities()` 에 빌드환경
  진입점 노출 여부를 capability 로 추가(하드코딩 분기 금지).

#### D. 내비게이션 / 발견성

- [x] 빌드환경 페이지 섹션 순서에 "iPhone (macOS)" 그룹 삽입(Android/Flutter 섹션과 병렬).
- [x] Mac 로컬 설치 감지 시 대시보드/네비에 "iPhone 빌드환경 준비" 힌트(1회성) 노출 검토.
- [x] Linux 단독 설치에서는 iPhone 섹션에 "원격 Mac agent 연결" 안내만 표시(설치 버튼 숨김).

#### E. 문서 / 회귀 테스트

- [x] env-setup detect/snapshot 테스트: `mac_local` 모드는 iPhone 카드 노출 +
  preflight 배너, `linux` 모드는 `mac_required` blocked + 버튼 숨김.
- [x] `renderAction` exhaustive `when` 컴파일 가드(신규 enum 누락 시 컴파일 실패)로 회귀 방지.
- [x] 문서 갱신 트리거(§8.C 운영/설치): `CHANGELOG.md` `### Added` (v1.164.0) 완료 +
  Wire 무변경 명시(§8.A 해당 없음). **공개 `Wiki/Build-Environment.md` "iPhone (macOS)
  빌드환경" 섹션은 운영자 SSH push 단계로 남김**(외부 공개 액션).

### 18.3 캡슐화 금지/허용 (이 phase 한정 재확인)

금지:

- `EnvSetupService`/`EnvSetupTemplates` 에 `xcodebuild`/`xcrun`/`brew` 를 직접 실행하는 코드
  추가(진단·설치는 `ios` 패키지 서비스 위임).
- iPhone 컴포넌트를 `install-all` 원터치 대상에 포함.
- Linux 단독 환경에서 iPhone 설치 버튼을 활성으로 노출.
- Xcode/CLT/Simulator runtime 을 자동 설치 가능한 것처럼 안내.

허용:

- 공통 env-setup 카드 그리드가 engine capability/컴포넌트 상태만 보고 iPhone 섹션 렌더.
- 기존 `/api/ios/preflight` / `/api/ios/swift-tools/install` 재사용.
- preflight 스냅샷을 `SetupComponent` 상태(INSTALLED/MISSING/PARTIAL/UNKNOWN)로 매핑.

### 18.4 완료 조건

- Mac 로컬 설치에서 빌드환경 페이지에 "iPhone (macOS)" 섹션이 보이고, Xcode/CLT/Simulator
  runtime 감지 상태와 SwiftLint/SwiftFormat 설치 버튼이 뜬다.
- preflight 결과(모드/Xcode/simctl/서명)가 빌드환경과 프로젝트 화면에 표시된다.
- Xcode 미설치 등 자동 설치 불가 항목은 실패가 아니라 안내(blocked)로 표시된다.
- Linux Docker 단독 설치에서는 iPhone 섹션이 "Mac 필요"로 비활성 표시되고 설치가 실행되지
  않는다.
- Android/Flutter 기존 빌드환경 동작과 `install-all` 결과가 변하지 않는다.
- 신규 REST/Wire 없이(또는 최소 additive 로) 배선되어 기존 안드로이드 클라이언트가 깨지지
  않는다.
